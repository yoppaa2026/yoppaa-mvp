'use client'
//
// /commander/cancel?token=...
//
// Page d'annulation depuis lien email confirmation commande C&C.
// Lien typique : https://www.yoppaa.app/commander/cancel?token=<annulation_token>
//
// Flow :
//   1. Lecture token URL
//   2. Affichage écran confirmation "Tu veux annuler ?"
//   3. Click "Confirmer" → POST /api/commande/cancel avec { token }
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              {resultat?.numero_commande && (
                <p style={{ color: T.muted, fontSize: '0.85rem', marginBottom: 12 }}>
                  Commande <strong style={{ color: T.main }}>#{resultat.numero_commande}</strong>
                </p>
              )}
              <p style={{ color: T.deep, fontSize: '0.95rem', lineHeight: 1.55, fontWeight: 600 }}>
                {resultat?.message || 'Ta commande est annulée.'}
              </p>
            </div>
