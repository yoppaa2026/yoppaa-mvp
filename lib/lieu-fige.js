...
// ⚠️ `lieuId` L'EMPORTE SUR TOUT quand il est fourni. C'est le choix EXPLICITE
// du commerçant, qui a rattaché cette plage de réservation à cet endroit-là.
// Le déduire de l'heure serait le contredire : deux plages peuvent se suivre au
// même endroit, ou se tenir à deux adresses sans que les horaires le disent.
export async function champsLieuPour(client, commercant, { jour, heure, lieuId = null } = {}) {
  ...
  try {
    ...
    const lieux = data || []
    if (lieuId) {
      const choisi = lieux.find(l => l.id === lieuId)
      if (choisi) return champsLieu({ ...choisi, libelle: choisi.libelle, adresse: choisi.adresse })
    }
    return champsLieu(lieuALHeure({ commercant, lieux, jour, heure }))
  } catch {
    return {}
  }
}
