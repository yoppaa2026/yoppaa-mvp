...
import { capacitePrestation, premierePlaceLibre, rangLibre, estParCouverts, couvertsValides, occupationDe, dureeSelonCouverts, estCoursCollectif, COLONNES_COUVERTS } from './cours-collectifs'
import { enModeInventaire, formatLibrePour, dureeDuGroupe, estJointure, plafondCadence, etatCadence } from './inventaire-salle'
...
// ⚠️ LES COLONNES DE LA RÈGLE DES COUVERTS VIENNENT DE LA RÈGLE (16/09). Ce
// module les avait toutes ; deux routes voisines en avaient oublié une, et le
// banc ne pouvait pas le voir puisque chacune recopiait sa propre liste.
const COLONNES_PRESTATION_DECIDE =
  `id, nom, tva_taux, duree_minutes, commercant_id, duree_paliers, quantite, actif, jointure_de, jointure_tables, ${COLONNES_COUVERTS}`
 ...
 * 'praticien_hors_prestation', 'couverts_invalides', 'groupe_trop_grand',
 * 'salle_complete', 'cadence_atteinte', 'place_prise', 'ecriture_impossible'.
 */
export async function creerReservationRdv(db, {
  ...
  if (champs?.source !== 'commercant') {
    const { data: creneauxCom } = await db
      .from('rdv_creneaux')
      // 🔴 `praticien_id` MANQUAIT, ET C'EST TOUT LE DÉFAUT DU 18/09 (Alex :
      // « pourquoi il refuse alors qu'il la propose ? »). Depuis que
      // `tranchesReservees` distingue une plage NOMMÉE d'une plage COMMUNE,
      // cette colonne décide si une plage dédiée réserve l'heure de sa seule
      // praticienne ou celle de toute la maison. Absente du select, elle vaut
      // `undefined` partout : le serveur retombait sur l'ancien comportement et
      // refusait exactement ce que l'écran venait de proposer.
      //
      // ⚠️ C'EST LE DÉFAUT LE PLUS FRÉQUENT DE CE DÉPÔT, et il ne dit jamais
      // rien : une colonne absente ne lève pas, elle vaut `undefined`, et la
      // règle s'applique à l'envers en silence.
      .select('id, jour_semaine, date_specifique, heure_debut, heure_fin, pause_debut, pause_fin, actif, praticien_id')
      .eq('commercant_id', commercantId)
      .is('deleted_at', null)
  ...
  if (estParCouverts(prestation)) {
    ...
    //
    // 🔴 UNE TABLE LIBRE NE SUFFIT PLUS. Si trop de personnes arrivent déjà sur
    // ce quart d'heure, la cuisine ne suit pas, et c'est ce que le restaurateur
    // a réglé. Le serveur compte avec la règle de la fiche, `etatCadence` :
    // l'écran propose, le serveur décide, et les deux lisent la même règle.
    //
    // ⚠️ TOUTES LES RÉSERVATIONS DU JOUR, SANS FILTRE DE PRESTATION : ce sont des
    // personnes qui arrivent, et la fiche compte les mêmes, puisque la fonction
    // publique les rend toutes. Pas seulement les tables de la salle.
    //
    // ⚠️ LA CADENCE SE LIT ICI, À PART, et seulement pour une table : un salon ne
    // paie aucune lecture de plus, et une colonne demandée à la lecture du lieu,
    // plus bas, ferait tomber TOUTES les réservations si elle manquait.
    //
    // ⚠️ UNE LECTURE EN ÉCHEC NE FERME RIEN, comme les plages et les praticiens
    // plus haut : une cadence est un confort de cuisine, pas une garde d'argent,
    // et un incident de lecture ne doit pas refuser une salle vide. On le dit
    // dans les journaux.
    //
    // ✅ LE RESTAURATEUR EST PRÉVENU SANS ÊTRE BLOQUÉ (décision d'Alex du 10/09
    // pour la salle, la même ici) : son agenda reste le sien.
    const cuisineAConsulter = champs?.source !== 'commercant'
    if (cuisineAConsulter) {
      const { data: reglage, error: errReglage } = await db
        .from('commercants')
        .select('rdv_cadence_couverts')
        .eq('id', commercantId)
        .maybeSingle()
      if (errReglage) console.error('[rdv/creation] cadence illisible, non appliquée', errReglage.message)
      const plafond = errReglage ? null : plafondCadence(reglage)
      if (plafond !== null) {
        const { data: duJour, error: errJour } = await db
          .from('rdv_reservations')
          .select('id, heure_debut, couverts')
          .eq('commercant_id', commercantId)
          .eq('date_rdv', dateRdv)
          .in('statut', STATUTS_OCCUPENT)
          .is('deleted_at', null)
        if (errJour) console.error('[rdv/creation] arrivées illisibles, cadence non appliquée', errJour.message)
        const cadence = errJour ? null : etatCadence({ plafond, couverts: couvertsRetenus, reservations: duJour || [], debutMin })
        if (cadence?.depasse) {
          return { ok: false, code: 'cadence_atteinte', restants: cadence.restants, plafond: cadence.plafond }
        }
      }
    }
