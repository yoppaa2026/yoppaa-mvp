...
const MOTS_TABLE = {
  ...
  // 🔴 L'ÉCRAN D'ANNULATION ET L'ESPACE DU CLIENT (11/09). La route
  // d'annulation rendait « Ton RDV est annulé. » au client d'un restaurant,
  // et son espace lui proposait « Annuler ce RDV ». Ces phrases sont rendues
  // telles quelles par la route, affichées telles quelles par l'écran.
  ecranAnnule: 'Ta réservation est annulée.',
  ecranDejaAnnule: 'Ta réservation est déjà annulée.',
  ecranAvant: 'avant ta réservation',
  ecranDejaEuLieu: 'Ta réservation a déjà eu lieu : il n’y a plus rien à annuler.',
  ecranNonHonore: 'Ta réservation a été marquée non honorée : elle ne peut plus être annulée.',
  annulerLien: 'Annuler cette réservation',
  annulerTitre: 'Annuler cette réservation ?',
  annulerBouton: 'Annuler la réservation',
  // ─── L'AIDE DE L'ÉCRAN DE RÉGLAGE ─────────────────────────────────────────
  //
  // 🔴 LES ONGLETS PARLAIENT LE MÉTIER, L'AIDE NON (Alex, 16/09). Sous des
  // onglets « Tables · Salles · Services », un restaurateur lisait « laisse la
  // capacité à 1 pour un rendez-vous en tête à tête », « ajoute ton équipe » et
  // « le lundi 10h à 11h, rien que le yoga ». Le vocabulaire du salon, mot pour
  // mot, sur l'écran d'un restaurant : celui qui est seul devant son écran
  // referme et téléphone.
  //
  // ⚠️ CHAQUE ÉTAPE DIT QUOI FAIRE, POURQUOI, ET DONNE UN EXEMPLE DE SON MONDE.
  // Le `**gras**` est rendu par l'écran.
  aide: {
    resume: 'Tes tables, tes salles, tes services et jusqu’à quand on réserve : l’ordre à suivre en 4 étapes',
    etapes: [
      {
        titre: 'Tes formats de table, d’abord',
        texte: 'Un format, c’est une taille de table et le temps qu’on y passe : « Table de 4 · 2 h ». Crée-les tous, du duo à la grande tablée, et dis **combien tu en as** de chaque. Yoppaa placera ensuite la plus petite qui convient, et gardera les grandes pour les grands groupes.',
      },
      {
        titre: 'Tes salles, ensuite',
        texte: 'Une seule salle ? Crée-la quand même : tu n’y penseras plus. Sinon, ajoute la terrasse, l’étage, la véranda. Tu pourras leur ouvrir **des services différents** et dire quelles tables s’y trouvent.',
      },
      {
        titre: 'Tes services : quand on peut réserver',
        texte: 'Un service dit à quelles heures on prend des réservations, **et dans quelle salle**. Midi de 12 h à 14 h, soir de 18 h 30 à 22 h : deux services, et le reste de la journée reste fermé.',
      },
      {
        titre: 'Jusqu’à quand on peut réserver',
        texte: 'Deux mois par défaut, et c’est ce qu’il faut pour un restaurant. Monte plus haut si tu prends des groupes longtemps à l’avance, comme les fêtes de fin d’année.',
      },
    ],
    // ⚠️ DEUX PIÈGES VRAIS, ET VÉRIFIABLES. Le second est celui du 16/09 : tant
    // que Stripe n'encaisse pas, la carte n'est jamais demandée, et rien ne le
    // disait à celui qui venait d'allumer l'empreinte.
    pieges: 'Tes services ne peuvent pas déborder de tes horaires d’ouverture (Paramètres → Profil) : ce qui dépasse ne sera jamais proposé. Et la carte bancaire ne se demande que si ton compte Stripe encaisse : tant qu’il n’est pas actif, tes grandes tables se réservent sans garantie.',
  },
}
...
const MOTS_RDV = {
  ...
  // ⚠️ Les trois premières et les trois dernières sont les phrases d'AVANT,
  // lettre pour lettre : un salon ne doit rien voir changer.
  ecranAnnule: 'Ton RDV est annulé.',
  ecranDejaAnnule: 'Ce RDV est déjà annulé.',
  ecranAvant: 'avant ton RDV',
  // Celles-ci remplacent « ce RDV est au statut "no_show" », une valeur de
  // base de données qui s'affichait telle quelle au client, chez tous.
  ecranDejaEuLieu: 'Ce RDV a déjà eu lieu : il n’y a plus rien à annuler.',
  ecranNonHonore: 'Ce RDV a été marqué non honoré : il ne peut plus être annulé.',
  annulerLien: 'Annuler ce RDV',
  annulerTitre: 'Annuler ce RDV ?',
  annulerBouton: 'Annuler le RDV',
  // ─── L'AIDE DE L'ÉCRAN DE RÉGLAGE ─────────────────────────────────────────
  // Le texte d'origine, qui était juste pour un métier de service : il vit
  // désormais ici, à côté de celui de la table, pour qu'aucun des deux ne
  ...
  aide: {
    ...
    etapes: [
      {
        titre: 'Tes prestations, d’abord',
        texte: 'Ce que tu proposes, sa durée et son prix. **La capacité change tout** : laisse-la à 1 pour un rendez-vous en tête à tête, monte-la pour un cours collectif. C’est elle qui décide de la suite, ici et sur ta fiche.',
      },
      {
        titre: 'Ton équipe, ensuite',
        texte: 'Tu travailles seul ? Passe cette étape. Sinon, ajoute chacun, puis dis sur quelle prestation il intervient. Sans rien préciser, tout le monde peut tout faire.',
      },
      {
        titre: 'Tes plages : quand, pour qui, pour quoi',
        texte: 'Une plage dit à quelles heures on peut réserver, **avec qui**, et **ce qui s’y donne**. Un cours à heure fixe mérite sa propre plage : le lundi de 10 h à 11 h, rien que le yoga. Le reste de la journée peut rester ouvert à tout.',
      },
      {
        titre: 'Jusqu’à quand on peut réserver',
        texte: 'Deux mois par défaut. Si tu vends des carnets ou des abonnements, monte à trois ou six mois : dix séances par semaine couvrent déjà plus de deux mois.',
      },
    ],
    pieges: 'Tes plages ne peuvent pas déborder de tes horaires d’ouverture (Paramètres → Profil) : ce qui dépasse ne sera jamais proposé. Et un cours laissé sur une plage ouverte à tout devient réservable à n’importe quelle heure, pour une personne seule.',
  },
}
...
export function objetReservation({ prestation_nom = '', table = false, couverts = null } = {}) {
  return groupeReservation({ table, couverts }) || prestation_nom || ''
}

// Combien ils sont, en personnes ; `null` quand ce n'est pas une table ou que
// le nombre ne se lit pas. ⚠️ LA RÈGLE VIT ICI SEULEMENT : le fichier calendrier
// en avait sa propre copie (`lib/ical.js`), et deux copies d'une règle finissent
// toujours par dire deux choses.
export function groupeReservation({ table = false, couverts = null } = {}) {
  const n = Math.floor(Number(couverts))
  if (table && Number.isFinite(n) && n >= 1) return `${n} personne${n > 1 ? 's' : ''}`
  return null
}

// Le même objet, quand rien ne l'annonce à côté. Sous l'heure du rappel de la
// veille, « 6 personnes » seul ne dit pas ce qu'on a réservé : « Table pour 6
// personnes », c'est aussi le titre de l'événement que le client a déjà dans son
// calendrier.
export function intituleReservation({ prestation_nom = '', table = false, couverts = null } = {}) {
  const groupe = groupeReservation({ table, couverts })
  return groupe ? `Table pour ${groupe}` : (prestation_nom || '')
}
