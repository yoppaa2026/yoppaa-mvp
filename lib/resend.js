...
export function emailRdvConfirme({
  ...
  return layout({
    ...
    intro: deplace
      ? `<strong>${yopper_prenom}</strong>, ton rendez-vous chez <strong>${commercant_nom}</strong> a été <strong>déplacé</strong>. Voici la nouvelle date à retenir 🟣`
      : `<strong>${yopper_prenom}</strong>, ton rendez-vous chez <strong>${commercant_nom}</strong> est bien confirmé. À très vite 🟣`,
    body: `
...
export function emailRdvAnnule({
  ...
  const raisonLabel = lieuChange
    ? `Le lieu a changé chez <strong>${commercant_nom}</strong>`
  ...
  return layout({
    ...
    intro: lieuChange
      ? `<strong>${yopper_prenom}</strong>, ton rendez-vous du <strong>${formatDateFr(date_rdv)} à ${heure_debut?.slice(0,5) || '?'}</strong> chez <strong>${commercant_nom}</strong> ne peut pas être maintenu : l’endroit a changé. Reprends ta place en un clic, tu retrouveras les horaires à la nouvelle adresse.`
      : `<strong>${yopper_prenom}</strong>, ton rendez-vous du <strong>${formatDateFr(date_rdv)} à ${heure_debut?.slice(0,5) || '?'}</strong> chez <strong>${commercant_nom}</strong> est annulé.`,
    body: `
...
export function emailRappelRetrait({
  ...
  return layout({
    ...
    intro: `<strong>${yopper_prenom || 'Yopper'}</strong>, ${texte}`,
    body: `
...
export function emailAbonnementConfirme({
  ...
  return layout({
    ...
    intro: `${yopper_prenom ? `${yopper_prenom}, ton` : 'Ton'} abonnement chez <strong>${commercant_nom}</strong> est confirmé. Garde cet email : c'est ta preuve d'achat.`,
    body: `
      ...
      <p style="margin:10px 0 0;font-size:11.5px;color:${C.muted};line-height:1.55;">
        Une question sur ton abonnement ? Contacte directement ${commercant_nom}, c'est lui qui le gère.
      </p>
    `,
