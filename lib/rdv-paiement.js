...
export function etatPaiementClient(commande = {}) {
  ...
  if (commande.paye_en_ligne) {
    return {
      cle: 'paye', ton: 'paye',
      libelle: 'Payé en ligne',
      detail: [avantagesClient, 'Rien à sortir au comptoir.'].filter(Boolean).join(' '),
    }
  }
  ...
  if (reste !== null && reste <= 0 && (bon > 0 || recompense > 0)) {
    if (recompense > 0 && bon <= 0) {
      return {
        ...
        detail: `Ta récompense de ${euros(recompense)} couvre tout. Rien à sortir au comptoir.`,
      }
    }
    ...
    return {
      ...
      detail: `${avantagesClient} Rien à sortir au comptoir.`,
    }
  }
