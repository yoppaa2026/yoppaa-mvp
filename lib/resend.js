...
export function emailCommandeConfirmee({
  ...
  const titreQuand = estExpedition ? 'Expédition' : estLivraison ? 'Livraison prévue' : estBoutiqueRetrait ? 'Retrait en boutique' : 'Retrait prévu'
  // Ligne horaire : créneau pour le C&C/livraison ; la boutique détail n'a PAS
  // de créneau (philosophie différente : dès que c'est prêt, aux heures d'ouverture)
  const ligneHoraire = estExpedition
    ? 'Ton colis part dès que la commande est préparée'
    : estBoutiqueRetrait
      ? 'Dès que ta commande est prête, pendant les heures d\'ouverture'
  ...
  return layout({
    ...
    body: `
      ...
      ${estBoutique ? `
      <p style="margin:0 0 14px;font-size:12px;color:${C.muted};line-height:1.55;">
        Un souci avec ta commande ? Contacte directement la boutique, elle trouvera une solution avec toi.
      </p>` : ''}
      ${offrir_mdp ? `
