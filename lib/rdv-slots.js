...
export function ajusterPlagePourJour(plage, horaireJour) {
  ...
  if (!(f > d)) return { statut: 'ignoree', debut: null, fin: null }

  // ⚠️ HORAIRES INCONNUS : ON NE TOUCHE À RIEN. Ajuster sur une ignorance
  // raccourcirait des plages parfaitement valables.
  if (!horaireJour) return { statut: 'inchangee', debut: minutesToTime(d), fin: minutesToTime(f) }
  if (horaireJour.ouvert === false) return { statut: 'ignoree', debut: null, fin: null }
  ...
  if (plages.length === 0) return { statut: 'inchangee', debut: minutesToTime(d), fin: minutesToTime(f) }
  ...
  if (!meilleur) return { statut: 'ignoree', debut: null, fin: null }
