...
export async function envoyerPush({
  ...
  // Envoi différé (rappel programmé) : OneSignal accepte un instant ISO 8601.
  // On garde la file (idempotence de délivrance côté OneSignal).
...
/**
 * Annule une notification OneSignal PROGRAMMÉE (send_after) pas encore envoyée.
 * Utilisé quand une commande / un RDV est annulé : on retire le rappel en file
 * pour ne pas notifier « n'oublie pas ton retrait » sur une commande annulée.
 * Best-effort : un échec (déjà envoyée, id inconnu) est non bloquant.
 *
 * @param {string} notificationId  id renvoyé par envoyerPush au moment du schedule
 * @returns {Promise<{ok:boolean, error?:string}>}
 */
export async function annulerPush(notificationId) {
  const { appId, apiKey } = getConfig()
  if (!appId || !apiKey) return { ok: false, error: 'OneSignal env non configuré' }
  if (!notificationId) return { ok: false, error: 'notificationId requis' }

  try {
    const res = await fetch(
      `${ONESIGNAL_API}/notifications/${encodeURIComponent(notificationId)}?app_id=${appId}`,
      { method: 'DELETE', headers: { 'Authorization': authHeader(apiKey) } }
    )
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      // 400/404 = déjà envoyée ou introuvable : non bloquant.
      return { ok: false, error: body?.errors?.[0] || `HTTP ${res.status}` }
    }
    return { ok: true }
  } catch (e) {
    console.error('[onesignal] annulerPush exception', e)
    return { ok: false, error: e?.message || String(e) }
  }
}
...
/**
 * Helper : push à une LISTE de Yoppers ciblés par leurs external_id (= clients.id).
 * Robuste au multi-appareils : OneSignal livre à toutes les subscriptions de
 * chaque external_id, SANS dépendre des tags (donc immunisé au 409 de tag-sync
 * quand un même compte est logué sur plusieurs appareils). Remplace le ciblage
 * par tag `favori:*` pour les pushs deals/actus/GMY.
 *
 * include_aliases est borné par requête → on découpe en lots de 2000.
 *
 * @param {string[]} clientIds  liste de clients.id
 * @param {Object}   content     { headings, contents, url, data, high_priority }
 * @returns {Promise<{ok:boolean, recipients:number, error?:string}>}
 */
export async function envoyerPushParExternalIds(clientIds, content) {
  const ids = [...new Set((clientIds || []).filter(Boolean).map(String))]
  if (ids.length === 0) return { ok: true, recipients: 0, skipped: 'aucun_destinataire' }

  const CHUNK = 2000
  let okAll = true, recipients = 0, lastErr = null
  for (let i = 0; i < ids.length; i += CHUNK) {
    const lot = ids.slice(i, i + CHUNK)
    const res = await envoyerPush({ ...content, include_aliases: { external_id: lot } })
    if (res.ok) recipients += res.recipients || 0
    else { okAll = false; lastErr = res.error }
  }
  return { ok: okAll, recipients, error: lastErr }
}
...
export async function setYopperTags(externalId, tags) {
  ...
  // Timeout dur : sans ça, un appel OneSignal lent peut faire dépasser le temps
  // d'exécution de la fonction serverless et renvoyer un vrai 502 côté Vercel.
  ...
  try {
    ...
    console.error('[onesignal] setYopperTags exception', e?.name === 'AbortError' ? 'timeout 6s' : e)
