...
import { normaliser as normaliserTva, ventiler as ventilerTva, imputerRemise } from '@/lib/tva'
import { referenceCommande } from './numero-commande'
import { libelleOptions } from './options-ligne'
import { adresseRendezVous } from './lieu-fige'
...
export async function envoyerEmailsCommande(commandeId, supabase) {
  ...
  const { data: cmd, error: errCmd } = await supabase
    ...
    .select(`
      ...
      paye_en_ligne, encaisse_mode, encaisse_montant,
      client_email, client_nom, client_telephone,
      annulation_token, mode_retrait, adresse_livraison, frais_livraison, tva_taux_livraison, bon_cadeau_montant,
      fidelite_remise,
      ...
      articles:commande_articles(quantite, prix_unitaire, options, tva_taux, article_nom, article:articles(nom))
    `)
    .eq('id', commandeId)
    .single()
  if (errCmd || !cmd) {
  ...
  const articlesFlat = (cmd.articles || []).map(a => ({
    // ⚠️ LE NOM FIGÉ À LA VENTE D'ABORD. Le ticket ne venait que de la jointure :
    // un « Lot de 3 » s'affichait sous le nom de l'article de base, et un article
    // retiré du catalogue laissait un « — » sur le reçu du client. Un justificatif
    // d'achat doit rester lisible quoi qu'il advienne du catalogue.
    nom:            a.article_nom || a.article?.nom || '—',
    quantite:       a.quantite,
  ...
  // Les frais de livraison rejoignent la ventilation, au taux figé à l'achat.
  // Ce sont des frais accessoires à la vente : ils suivent le taux des
  // marchandises, et le plus bas de la commande quand elle en mélange.
  const tauxLivraison = normaliserTva(cmd.tva_taux_livraison)
  const fraisLivraison = Number(cmd.frais_livraison || 0)
  if (fraisLivraison > 0 && tauxLivraison !== null) {
    parTauxTicket[tauxLivraison] = (parTauxTicket[tauxLivraison] || 0) + fraisLivraison
  }
  // 🔴 LA RÉCOMPENSE SORTAIT DU TOTAL SANS SORTIR DE LA BASE TVA (Alex, 28/08).
  // Le ticket disait « Plus rien à payer 0,00 € » puis « TVA 21 % · 1,39 € » :
  // il se contredisait lui-même, sur un document qu'il appelle justificatif.
  //
  // ⚠️ ET LE BON CADEAU NE S'IMPUTE SURTOUT PAS ICI : c'est un moyen de
  // paiement, pas une remise. Le prix convenu ne bouge pas, la TVA due non
  // plus. La règle et son pourquoi vivent dans `lib/tva.js`, avec l'export
  // comptable, pour que les deux documents ne puissent plus diverger.
  const parTauxNet = imputerRemise(parTauxTicket, cmd.fidelite_remise)
  const ventilationTva = Object.entries(parTauxNet)
    // Une ligne à zéro n'apprend rien et ferait afficher « base 0,00 € » sous
    // un total nul : quand tout est offert, il n'y a pas de TVA à annoncer.
    .filter(([, ttc]) => Number(ttc) > 0)
    .map(([taux, ttc]) => {
  ...
  if (cmd.client_email) {
    try {
      ...
      const html = emailCommandeConfirmee({
        ...
        tva_taux_livraison:      tauxLivraison,
        total:                   cmd.total,
        ...
        // 🔴 LA REMISE DE FIDÉLITÉ NE SORTAIT PAS D'ICI. Le ticket annonçait le
        // total BRUT : Alex a lu « 36,00 € » sur un écran de commande qui venait
        // d'afficher 26,00 €, sa récompense ayant pourtant bien été consommée.
        // La colonne n'était pas dans le `select` ci-dessus : aucune erreur,
        // aucun log, juste un chiffre faux. C'est le défaut le plus fréquent de
        // ce projet, et il porte cette fois sur ce que le client va PAYER.
        ...
        // ⚠️ LE TICKET NE DISAIT PAS SI C'ÉTAIT PAYÉ (Alex, 17/08). Une commande
        // réglée en ligne et une commande à payer au comptoir sortaient d'ici
        // rigoureusement identiques. `paye_en_ligne` est écrit par le webhook
        // AVANT cet appel (route.js ligne 702 puis 770), l'ordre est donc sûr.
        paiement:                cmd,
      })
      await envoyerAuCommercant({
        to: cmd.client_email,
        subject: `Ta commande ${chezLeCommerce(cmd.commercant?.nom || 'le commerçant')} est confirmée`,
        html,
      })
    } catch (e) {
  ...
  if (cmd.commercant?.notif_mode === 'chaque' && cmd.commercant?.email) {
    try {
      const html = emailNouvelleCommandeCommercant({
        ...
        // ⚠️ LES DEUX REMISES, sinon le commerçant prépare la commande en
        // croyant devoir encaisser le brut. Le bon cadeau manquait ici depuis
        // toujours, la récompense depuis sa création.
        fidelite_remise:  cmd.fidelite_remise,
        bon_cadeau_montant: cmd.bon_cadeau_montant,
      })
      await envoyerAuCommercant({
        to: cmd.commercant.email,
        subject: `Nouvelle commande #${referenceCommande(cmd) || ''} · ${prenom}`,
        html,
      })
    } catch (e) {
