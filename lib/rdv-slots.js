...
import { creneauxDuJour, franchitMinuit, finApresMinuit, timeToMinutes } from './deplacement-rdv'
// ⚠️ LA SALLE SE COMPTE EN TABLES DANS UN SEUL FICHIER, et ce moteur le lit
// (10/09). Sans cet import, la grille de la fiche comptait une salle comme un
// agenda de coiffeur : une table de deux réservée à 19h fermait 19h à la table
// de quatre, comme si le restaurant n'avait qu'une personne à servir.
import { enModeInventaire, conflitSalle, etatCadence } from './inventaire-salle'
...
// ⚠️ `timeToMinutes` VIT DANS `deplacement-rdv` depuis le 10/09 : ce fichier
// l'importe et la republie plus bas, avec `finApresMinuit`, pour la même raison.
...
//
// 🔴 LE BAR QUI FERME À 02:00 N'AVAIT AUCUN CRÉNEAU, ET AUCUNE ERREUR. En
// minutes, `02:00` vaut 120 et `14:00` en vaut 840 : la fermeture tombait AVANT
// l'ouverture, le clip `Math.min(fin, fermeture)` ramenait la fin de journée à
// deux heures du matin, et la boucle des créneaux ne tournait pas une fois.
//
// Chez une brasserie qui ferme à minuit ou à 2h — c'est-à-dire chez la plupart
// des restaurants — SIX JOURS SUR SEPT étaient muets. Le commerçant voyait ses
// plages enregistrées, son client ne voyait rien, et rien ne disait pourquoi.
//
// ⚠️ `00:00` EST LE PIRE DES CAS, et c'est le plus fréquent : il vaut ZÉRO. Une
// fermeture « à minuit » n'était pas seulement mal ordonnée, elle annulait la
// journée entière.
//
...
// ⚠️ LA RÈGLE VIT DÉSORMAIS DANS `deplacement-rdv`, ET C'EST UNE CONTRAINTE DE
// SENS DE DÉPENDANCE, pas un caprice. L'agenda du commerçant en a besoin lui
// aussi, et ce fichier-ci importe déjà `creneauxDuJour` de là-bas : la ramener
// dans l'autre sens aurait fermé un cycle. Une règle, un seul endroit où elle
// est écrite, et le module bas la porte.
//
// ⚠️ `import` PUIS `export`, JAMAIS `export … from` : la forme republiée ne lie
// pas le nom localement, et ce fichier s'en sert quatre fois plus bas. C'est
// exactement le défaut rencontré dans `lib/resend.js`.
export { franchitMinuit, finApresMinuit, timeToMinutes }
...
// 🔴 ET LE QUATRIÈME ARGUMENT RÉPARE LE COMPTEUR DE PLACES (Alex, 18/09) :
// « le nombre de places restantes ne se met à jour que sur le praticien, sans
// préférence n'actualise pas ». Vérifié en exécutant : sur Emily, une place
// prise sur trois ; sans préférence, zéro.
//
// LA CAUSE. Cette fonction sert DEUX questions que rien ne distinguait :
//   • « ce créneau est-il bloqué ? » → oui seulement si TOUTES les praticiennes
//     éligibles sont prises. Une inscription chez Emily ne bloque pas Carole,
//     donc elle était écartée du résultat ;
//   • « combien reste-t-il de places à ce cours ? » → là, TOUTE inscription
//     compte, quelle que soit la praticienne. Trois places, c'est trois places.
//
// Écartée pour la première question, l'inscription disparaissait aussi de la
// seconde, et le cours s'affichait complet alors qu'il ne l'était pas — ou
// l'inverse, ce qui est le cas vu par Alex.
//
// ⚠️ ET SEULEMENT POUR UN COURS. Ajouter les réservations non bloquantes à un
// rendez-vous INDIVIDUEL fermerait des créneaux à tort : un Reiki chez Carole à
// 10h n'empêche pas d'en prendre un chez Emily à la même heure. C'est pour ça
// que l'appelant passe l'identifiant seulement quand la prestation EST un
// cours, et rien sinon.
export function filtrerReservationsPourSlots(reservations, praticienChoisi, praticiensEligibles, { prestationCours = null } = {}) {
  ...
  // 🔴 LES INSCRIPTIONS À CE COURS COMPTENT TOUTES, bloquantes ou non. Sans
  // cette reprise, le compteur de places ne voyait que les heures où toutes les
  // praticiennes sont occupées, c'est-à-dire presque jamais chez un studio à
  // deux professeurs.
  //
  // ⚠️ ON DÉDOUBLONNE, et ce n'est pas une précaution de style : une inscription
  // peut être bloquante ET porter ce cours. Comptée deux fois, elle prendrait
  // deux places sur trois à elle seule, et le cours se fermerait plus tôt qu'il
  // ne doit. Le défaut serait alors l'inverse de celui qu'on corrige.
  if (prestationCours) {
    const dejaLa = new Set(bloquants)
    for (const r of list) {
      if (String(r.prestation_id) === String(prestationCours) && !dejaLa.has(r)) {
        bloquants.push(r)
        dejaLa.add(r)
      }
    }
...
//
// 🔴 SAUF POUR UN COURS : PAS DE PLAGE, PAS DE DISPO (arbitrage d'Alex, 08/09,
// et il a raison). « Vide = toutes » est juste pour une prestation SOLO : le
// client choisit son heure dans la plage ouverte, et c'est ce qui protège tous
// les commerces qui n'ont jamais touché ce réglage. Un COURS, lui, est une
// heure que le commerçant FIXE : le proposer partout, c'est laisser le premier
// client décider qu'un cours a lieu à 13h, à une personne, que le professeur
// n'a jamais prévu. C'est le défaut d'origine du 07/09, que je n'avais corrigé
// qu'à moitié : j'avais rendu possible de rattacher un cours à une plage, et
// je continuais à le proposer ailleurs. L'avertissement DÉCRIVAIT le défaut au
// lieu de l'empêcher.
export function creneauAccepte(creneauId, prestationId, liaisons, { estCours = false } = {}) {
  ...
  // UNE PLAGE SANS RIEN DE COCHÉ ACCEPTE TOUT LE CATALOGUE ORDINAIRE.
  //
  // 🔴 ELLE REFUSAIT CE QUI ÉTAIT COCHÉ AILLEURS, ET ÇA CASSAIT LE MÉTIER DE
  // LA MOITIÉ DU PARC (Alex, 08/09). Deux façons de travailler coexistent :
  //
  //   • le club de sport et le centre de yoga POSENT leurs heures et disent ce
  //     qu'ils y font : zumba lundi 19h, spinning mardi 18h. Chaque plage
  //     nomme, le client ne choisit pas l'horaire ;
  //   • le coiffeur et l'esthéticienne ouvrent de GRANDES plages au pas de 30
  //     ou 60 minutes, et les clients les remplissent comme ils veulent.
  //
  // Avec l'ancienne règle, une esthéticienne qui réservait son jeudi 14h-16h
  // au soin du visage voyait ce soin DISPARAÎTRE de toutes ses autres plages.
  // Elle n'avait rien demandé de tel. Cocher veut dire « ici, c'est ça », pas
  // « ça n'existe que là ».
  //
  // ⚠️ ET LA PLAGE DÉDIÉE RESTE PROTÉGÉE : c'est `tranchesReservees` qui s'en
  // charge, depuis le 07/09. Une plage qui nomme réserve son heure contre tout
  // le reste. Les deux moitiés du travail sont faites, chacune à sa place, et
  // celle-ci était devenue une confiscation, pas une protection.
  if (!prestationId) return true
  // 🔴 UN COURS NE VIT QUE SUR UNE PLAGE QUI LE NOMME. Ici, la plage ne nomme
  // personne : elle accueille l'agenda ordinaire, pas un cours à heure fixe.
  // C'est la seule restriction qui subsiste, et elle est voulue.
  if (estCours) return false
  return true
}
...
export function tranchesReservees(creneauxJour, prestationId, liaisons, { estCours = false } = {}) {
  ...
  for (const c of creneauxJour || []) {
    ...
    const debut = timeToMinutes(c.heure_debut)
    const fin   = timeToMinutes(c.heure_fin)

    // 🔴 UNE PLAGE RÉSERVE L'HEURE DE SA PRATICIENNE, PAS CELLE DE TOUTE LA
    // MAISON (Alex, 18/09, captures de Studio Amandine à l'appui).
    //
    // Lundi, deux plages 09:00-18:00 se superposaient : Carole donnait le
    // pilates, Emily le yoga. Le client qui choisissait Carole voyait ses neuf
    // créneaux ; celui qui ne choisissait personne n'en voyait AUCUN. En
    // « sans préférence », cette fonction reçoit TOUTES les plages du jour, et
    // celle d'Emily, dédiée au yoga, réservait la journée entière contre le
    // pilates. Le moteur croyait la salle prise par un cours, alors que c'est
    // une collègue qui en donne un autre à côté.
    //
    // ⚠️ LE COMMENTAIRE JUSTE AU-DESSUS L'AVOUAIT SANS LE VOIR : « les créneaux
    // reçus sont ceux du praticien choisi, OU TOUS quand il n'y a pas de
    // préférence ». C'est exactement le second cas qui était faux.
    //
    // 🔴 ET ON NE DÉFAIT PAS LA CORRECTION DU 07/09. Une plage dédiée doit
    // continuer d'empêcher qu'un Reiki vienne squatter l'heure du yoga : sans
    // ça, le créneau devenait occupé et plus personne ne pouvait s'inscrire au
    // cours. C'est le praticien qui sépare les deux situations :
    //
    //   • plage COMMUNE (aucune praticienne nommée) → elle prend la maison
    //     entière, elle réserve contre tout le monde. Inchangé.
    //   • plage NOMMÉE → elle ne réserve QUE sa praticienne. Elle ne bloque
    //     donc que s'il ne reste, à cette heure, personne d'AUTRE pour
    //     m'accueillir.
    if (c.praticien_id) {
      const uneAutreMAccueille = (creneauxJour || []).some((autre) => {
        if (String(autre.id) === String(c.id)) return false
        // ⚠️ UNE AUTRE PRATICIENNE, pas la même sous une seconde plage : deux
        // plages d'une même personne sont la même ressource, et elles ne se
        // libèrent pas l'une l'autre.
        if (!autre.praticien_id || String(autre.praticien_id) === String(c.praticien_id)) return false
        if (timeToMinutes(autre.heure_debut) >= fin) return false
        if (timeToMinutes(autre.heure_fin) <= debut) return false
        return creneauAccepte(autre.id, prestationId, liaisons, { estCours })
      })
      if (uneAutreMAccueille) continue
    }
...
export function creneauxPourPrestation(creneaux, prestationId, liaisons, { estCours = false } = {}) {
  if (!Array.isArray(liaisons) || !prestationId) return creneaux || []
  return (creneaux || []).filter(c => creneauAccepte(c.id, prestationId, liaisons, { estCours }))
}

// Un cours qu'AUCUNE plage ne nomme n'a pas d'horaire : il n'est réservable
// nulle part, et la fiche publique ne le liste donc pas (décision d'Alex,
// 08/09). Une prestation que le client peut choisir sans jamais voir un seul
// créneau est une impasse.
//
// ⚠️ SANS LIAISONS CHARGÉES, ON NE CACHE RIEN. Le jour où la table n'est pas
// lue, faire disparaître tous les cours du parc serait le pire des remèdes, et
// personne ne verrait d'erreur.
// CE PRATICIEN FAIT-IL CE MÉTIER ?
//
// ⚠️ MÊME RÈGLE QUE POUR LES PLAGES, ET ELLE EST DÉJÀ CELLE DE L'ÉCRAN : une
// prestation que personne ne réclame est faite par tout le monde. C'est ce qui
// protège les commerces qui n'ont jamais touché ce réglage, et l'immense
// majorité n'a de toute façon qu'un praticien.
//
// ⚠️ `praticienId` vide veut dire « sans préférence » : le commerce assigne
// qui il veut, on ne juge rien.
export function praticienAutorisePourPrestation(praticienId, prestationId, liens) {
  if (!praticienId) return true
  if (!Array.isArray(liens)) return true
  const duMetier = liens.filter(l => String(l.prestation_id) === String(prestationId))
  if (duMetier.length === 0) return true
  return duMetier.some(l => String(l.praticien_id) === String(praticienId))
}

// Une prestation que plusieurs praticiens pourraient faire, sans que le
// commerçant ait dit lesquels. Sert à le prévenir, pas à bloquer.
//
// ⚠️ ET SEULEMENT À PARTIR DE DEUX PRATICIENS : chez un indépendant seul, la
// question n'existe pas, et une alarme qui sonne tout le temps ne protège plus
// rien.
export function prestationSansPraticienDit(prestationId, liens, nbPraticiens) {
  if (!prestationId) return false
  if (Number(nbPraticiens) < 2) return false
  if (!Array.isArray(liens)) return false
  return !liens.some(l => String(l.prestation_id) === String(prestationId))
}

export function coursSansHoraire(prestation, liaisons) {
  if (!Array.isArray(liaisons)) return false
  // ⚠️ UNE TABLE N'A JAMAIS « PAS D'HORAIRE » : elle se réserve sur tout service
  // qui l'accueille. Sans cette ligne, les tables sans plage dédiée s'affichaient
  // « dates à venir » sur la fiche d'un restaurant ouvert.
  if (prestation?.par_couverts === true) return false
  if (!(Number(prestation?.capacite) > 1)) return false
  return !liaisons.some(l => String(l.prestation_id) === String(prestation?.id))
}
...
export function prestationAutoriseeSurCreneaux({
  creneaux, liaisons, prestationId, dateStr, jour, debutMin, finMin, estCours = false,
}) {
  ...
  // ⚠️ ET « PAS DE PLAGE, PAS DE DISPO » SE REFUSE ICI AUSSI (08/09). L'écran
  // ne propose plus un cours sans plage, mais un écran ne décide de rien : sans
  // cette règle au serveur, une requête forgée pose encore un cours à l'heure
  // qu'elle veut.
  const retenus = duJour.filter(c => creneauAccepte(c.id, prestationId, liaisons, { estCours }))
  ...
  // ⚠️ `estCours` VOYAGE JUSQU'ICI, ET IL MANQUAIT (18/09). Sans lui, cette
  // règle jugeait qu'une grande plage ouverte peut accueillir un COURS, ce
  // qu'elle ne peut pas : l'écran et le serveur appliquaient deux versions de
  // la même règle. Une règle écrite une fois mais appelée deux fois avec des
  // arguments différents, c'est deux règles.
  if (Number.isFinite(d0) && Number.isFinite(f0)
      && tombeDansUneTrancheReservee(d0, f0, tranchesReservees(duJour, prestationId, liaisons, { estCours }))) {
    return false
  }
...
export function creneauHorsOuverture({ jour, heureDebut, heureFin, horairesDetail }) {
  ...
  // 🔴 ET LA FERMETURE APRÈS MINUIT COMPTE ICI AUSSI (09/09). Sans elle, une
  // brasserie ouverte de 14:00 à 02:00 voyait CHACUNE de ses plages déclarée
  // « hors horaires » : l'alerte se déclenchait sur tout, donc elle ne
  // protégeait plus rien.
  const plages = []
  if (h.debut && h.fin) {
    const d1 = timeToMinutes(h.debut)
    plages.push([d1, finApresMinuit(d1, timeToMinutes(h.fin))])
  }
  if (h.debut2 && h.fin2) {
    const d2 = timeToMinutes(h.debut2)
    plages.push([d2, finApresMinuit(d2, timeToMinutes(h.fin2))])
  }
...
export function ajusterPlagePourJour(plage, horaireJour) {
  ...
  // 🔴 ET LA FERMETURE APRÈS MINUIT, TROISIÈME ENDROIT (09/09). Sans elle, la
  // copie d'une plage vers un jour qui ferme à 02:00 la déclarait « hors
  // ouverture » et refusait de la poser.
  const plages = []
  if (horaireJour.debut && horaireJour.fin) {
    const a1 = timeToMinutes(horaireJour.debut)
    plages.push([a1, finApresMinuit(a1, timeToMinutes(horaireJour.fin))])
  }
  if (horaireJour.debut2 && horaireJour.fin2) {
    const a2 = timeToMinutes(horaireJour.debut2)
    plages.push([a2, finApresMinuit(a2, timeToMinutes(horaireJour.fin2))])
  }
...
export function coursDejaCoche(idsCoches, prestations) {
  ...
  for (const id of idsCoches || []) {
    ...
    // Une table n'est pas un cours : deux formats de table sur une même plage,
    // c'est une salle normale, pas deux cours qui se chevauchent.
    if (p && Number(p.capacite) > 1 && p.par_couverts !== true) return p
  }
  return null
}

// Un cours collectif qu'AUCUNE plage VIVANTE ne vise nommément.
//
// 🔴 UNE PLAGE ÉTEINTE NE PORTE PLUS RIEN, ET LES DEUX ÉCRANS DIVERGEAIENT
// (Alex, 08/09). Le tableau de bord charge les plages inactives pour permettre
// de les rallumer ; la fiche publique, elle, ne lit que les actives. Un cours
// dont la seule plage était désactivée passait donc « Dates à venir » chez le
// client pendant que le commerçant ne voyait AUCUN avertissement. Il éteint sa
// plage, son cours sort de la vente, et rien ne le lui dit.
//
// ⚠️ `creneaux` est facultatif pour ne casser aucun appelant, mais tout
// appelant qui a la liste DOIT la passer : sans elle, la fonction compte les
// plages mortes. Une garde de banc vérifie les deux appels du tableau de bord.
export function prestationSansCreneauDedie(prestationId, liaisons, creneaux = null) {
  if (!prestationId) return false
  if (!Array.isArray(liaisons)) return true
  const vivants = Array.isArray(creneaux)
    ? new Set(creneaux.filter(c => c && c.actif !== false && !c.deleted_at).map(c => String(c.id)))
    : null
  return !liaisons.some(l =>
    String(l.prestation_id) === String(prestationId)
    && (vivants === null || vivants.has(String(l.creneau_id))))
}
...
//
// 🔴 `salle` : UNE SALLE INVENTORIÉE SE COMPTE EN TABLES, pas en séance (10/09).
// `{ formats, reservations }`, les formats de table du commerce et TOUTES les
// réservations du jour, sans filtre de praticien. Quand elle est là et que
// l'inventaire est complet, la question n'est plus « cette séance a-t-elle de
// la place ? » mais « une table libre peut-elle asseoir ce groupe ? », et la
// réponse vient de `formatLibrePour`, celle du serveur. Absente, rien ne change.
//
// 🔴 `cadence` : LA CUISINE A SON MOT À DIRE APRÈS LA SALLE (lot 5, 12/09).
// `{ plafond, reservations }`, la cadence du restaurant et TOUTES les
// réservations du jour. Une table libre ne suffit plus : si trop de personnes
// arrivent déjà sur ce quart d'heure, le créneau est refusé pour cette raison-là,
// `raison: 'cadence'`. Seulement pour une table ; absente, rien ne change.
// ⚠️ LA RÈGLE EST `etatCadence`, CELLE DU SERVEUR : l'écran qui proposerait ce
// que le serveur refuse enverrait le client remplir un formulaire pour rien.
export function conflitReservation({ cadence = null, ...regle }) {
  const verdict = conflitSansCadence(regle)
  if (verdict.conflit || !cadence || regle.parCouverts !== true) return verdict
  const etat = etatCadence({
    plafond: cadence.plafond,
    couverts: regle.couvertsDemandes,
    reservations: cadence.reservations || [],
    debutMin: regle.debut,
  })
  return etat?.depasse ? { ...verdict, conflit: true, raison: 'cadence', cadence: etat } : verdict
}

function conflitSansCadence({
  debut, fin, prestationId = null, capacite = 1, reservations = [],
  parCouverts = false, couvertsDemandes = 1, salle = null,
}) {
  if (salle && enModeInventaire(salle.formats)) {
    return conflitSalle({
      formats: salle.formats,
      couverts: Math.max(1, Math.floor(Number(couvertsDemandes) || 1)),
  ...
  const plages = (reservations || []).map(r => ({
    ...
    couverts: Number(r.couverts) >= 1 ? Math.floor(Number(r.couverts)) : 1,
  }))
  ...
  //
  // 🔴 MAIS DEUX TABLES NE SONT PAS DEUX SÉANCES (09/09). Un cours de 10h et un
  // de 10h30 ne coexistent pas, d'où l'égalité stricte des bornes. Deux tables
  // qui se chevauchent, elles, coexistent tout le temps : une à 20h00, une à
  // 20h30, et la salle porte les deux. Avec l'égalité stricte, la seconde
  // serait tombée dans « les autres » et aurait été refusée pour occupation.
  // Un restaurant n'aurait pris qu'une table par heure ronde.
  const memeSeance = !(places > 1) ? []
    : parCouverts
      ? plages.filter(p => (prestationId === null || p.prestation_id === prestationId)
          && debut < p.end && fin > p.start)
  ...
  // L'occupation se compte en LIGNES pour un cours, en COUVERTS pour une table.
  const occupes = parCouverts
    ? memeSeance.reduce((s, p) => s + p.couverts, 0)
    : memeSeance.length
  // ⚠️ CE QU'ON DEMANDE COMPTE AUSSI. Une salle où il reste deux couverts n'est
  // pas « libre » pour une table de six : elle l'est pour une table de deux.
  const demandes = parCouverts
    ? Math.max(1, Math.floor(Number(couvertsDemandes) || 1))
    : 1

  const base = {
    inscrits: places > 1 ? occupes : null,
  ...
  if (places > 1 && occupes + demandes > places) {
...
export function genererSlots({ dateChoisie, dureeMinutes, creneaux, reservations, horairesDetail, capacite = 1, prestationId = null, liaisonsCreneaux = null, parCouverts = false, couvertsDemandes = 1, salle = null, cadence = null }) {
  ...
  // 🔴 UNE FERMETURE APRÈS MINUIT SE COMPTE APRÈS L'OUVERTURE, PAS AVANT. Sans
  // ça, une brasserie ouverte de 14:00 à 02:00 n'avait pas un seul créneau.
  const shopClose = horaireJour?.fin
    ? finApresMinuit(shopOpen, timeToMinutes(horaireJour.fin))
    : null
  ...
  if (horaireJour?.debut2 && horaireJour?.fin2) {
    const d2 = timeToMinutes(horaireJour.debut2)
    shopRanges.push([d2, finApresMinuit(d2, timeToMinutes(horaireJour.fin2))])
  }
  ...
  // ⚠️ `estCours` SE LIT SUR LA CAPACITÉ REÇUE, pas sur `places` calculé plus
  // bas : une variable lue avant sa déclaration ferait un écran blanc, et le
  // banc de zone morte le dirait, mais autant ne pas l'écrire.
  // 🔴 ET UNE TABLE N'EST PAS UN COURS (10/09). Sa capacité vaut l'inventaire
  // entier — vingt-quatre pour six tables de quatre —, et lire `capacite > 1`
  // la classait « cours à heure fixe ». Le moteur ne lui offrait alors que les
  // plages qui la NOMMENT, et un groupe de quatre ne trouvait aucun horaire sur
  // un service ouvert à « toutes mes prestations ».
  const estCours = Number(capacite) > 1 && parCouverts !== true
  const creneauxRetenus = creneauxPourPrestation(creneauxJour, prestationId, liaisonsCreneaux, { estCours })
  ...
  // ⚠️ `estCours` VOYAGE AVEC, depuis le 18/09 : la fonction doit savoir ce
  // qu'une AUTRE plage accepterait à la même heure, et un cours ne vit que sur
  // une plage qui le nomme. Sans lui, une grande plage ouverte passerait pour
  // capable d'accueillir un cours, et libérerait une heure qui ne l'est pas.
  const reservees = tranchesReservees(creneauxJour, prestationId, liaisonsCreneaux, { estCours })

  // ── Reservations existantes (en minutes depuis minuit) ─────────────────────
  // 🔴 LES COUVERTS VOYAGENT AUSSI (10/09). Cette copie ne gardait que les
  // heures, la prestation et la place : `conflitReservation` recevait donc des
  // tables sans leur nombre de convives, et comptait une table de six pour UN
  // couvert. La salle en couverts paraissait plus vide qu'elle ne l'est, et la
  // grille proposait ce que le serveur allait refuser. Une colonne perdue dans
  // un `map` est la colonne absente d'un select, par une autre porte.
  ...
  for (const cr of creneauxRetenus) {
    ...
    // 🔴 LE PAS RESTE CELUI DE LA PLAGE, ET C'EST UNE DÉCISION DU 18/09.
    // Il a bien failli suivre la DURÉE de la prestation, pour retirer au
    // commerçant un réglage qu'il n'a aucune raison de comprendre. Mesuré sur
    // les vrais commerces, c'était une perte :
    //
    //   • Salon Nathalie règle un pas de 30 et vend des prestations de 30 à
    //     90 minutes. Un pas égal à la durée lui aurait retiré LES DEUX TIERS
    //     de ses départs sur les longues : 9h00 et 10h30, là où elle propose
    //     aujourd'hui 9h00, 9h30, 10h00, 10h30. Pouvoir commencer une couleur
    //     à la demie n'est pas un défaut de réglage, c'est son métier.
    //
    //   • Et elle est la seule à avoir une PAUSE : la grille serait repartie
    //     du début de la plage sans se recaler après, donc 13h00 devenait
    //     inréservable alors qu'il est libre.
    //
    // ⚠️ CE QUI CHANGE EST L'ÉCRAN, PAS LE MOTEUR : la granularité disparaît
    // de la configuration des services et ne reste que là où elle a un sens,
    // chez un restaurant. La valeur en base continue de décider ici.
    ...
    for (let t = debut; t + dureeMinutes <= fin; t += pas) {
      ...
      const c = conflitReservation({
        ...
        // 🔴 ET LA SALLE SE COMPTE EN COUVERTS (09/09). Sans ces deux-là, une
        // table de six passait pour libre dans une salle où il restait deux
        // places : le client allait jusqu'au paiement pour lire « complet ».
        parCouverts, couvertsDemandes,
        // 🔴 ET UNE SALLE INVENTORIÉE SE COMPTE EN TABLES (10/09), avec la
        // fonction du serveur. Sans elle, la table de deux montrée au client
        // était la seule regardée, et une table de quatre libre ne comptait pas.
        salle,
        // 🔴 ET LA CUISINE (lot 5) : trop d'arrivées sur ce quart d'heure le
        // ferment à ce groupe, même quand une table y est libre.
        cadence,
      })

      // ⚠️ UN QUART D'HEURE PLEIN SE LIT « COMPLET », comme une salle pleine : le
      // client n'a pas à connaître la cadence de la cuisine, seulement qu'il
      // doit choisir une autre heure.
      const motif = !c.conflit ? null
        : c.raison === 'complet' || c.raison === 'trop_grand' || c.raison === 'cadence' ? 'complet'
