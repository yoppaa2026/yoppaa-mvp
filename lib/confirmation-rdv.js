...
import { resteAEncaisser, retoursAnnulation, libelleRetours, libelleRetoursFaits } from './rdv-paiement'
...
export function questionRdv(action, rdv) {
  ...
  if (action === 'no_show') {
    // ⚠️ LE FRÈRE, TROUVÉ EN CORRIGEANT L'AUTRE. « Tu gardes son acompte » est
    // faux dès que l'acompte vaut zéro et que 40 € de bon cadeau ont payé la
    // prestation : le commerçant garde surtout le bon, et ne le savait pas. On
    // nomme ce qui reste chez lui, et rien d'autre : la récompense de fidélité
    // n'est pas de l'argent qu'il encaisse, c'est une remise qu'il a consentie.
    const r = retoursAnnulation(rdv)
    const garde = [
      r.acompte > 0 ? `${euros(r.acompte)} d’acompte` : '',
      r.surBon > 0 ? `${euros(r.surBon)} de bon cadeau` : '',
      r.produits > 0 ? `${euros(r.produits)} de produits déjà payés` : '',
    ].filter(Boolean)
    const message = garde.length
      ? `Il sera prévenu, et tu gardes ce qu’il a déjà versé : ${garde.length > 1 ? `${garde.slice(0, -1).join(', ')} et ${garde[garde.length - 1]}` : garde[0]}. Le créneau est resté bloqué pour lui.`
...
export function confirmationRdv(action, { rdv, raison, retours = null } = {}) {
  const qui = nomClient(rdv)
  if (action === 'annule_commercant') {
    ...
    return `${base}${retours ? libelleRetoursFaits(retours) : ''}`
  }
