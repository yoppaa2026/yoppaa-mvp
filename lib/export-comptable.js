...
export function construireLignes({
  ...
  for (const c of commandes) {
    ...
    //
    // ⚠️ ET ON EXIGE LE REMBOURSEMENT ENREGISTRÉ. Sans lui, on ne sait pas si
    // l'argent est reparti : une commande payée puis annulée dont le
    // commerçant a GARDÉ l'argent est un autre cas, et il reste ouvert. On ne
    // le traite pas en silence.
    if (!estComptabilisable(c.statut)) {
      const fraisPerdu = arrondi(c.stripe_frais)
      if (fraisPerdu > 0 && Number(c.stripe_refund_amount) > 0) {
        poser({
...
function entete({ commercant, du, au, assujetti, avertissementTaux, tauxManquants, refsIncompletes = 0, remboursements = 0, fraisRetenus = 0, ventesBons = 0 }) {
  ...
  if (fraisRetenus > 0) {
    l.push(ligneCsv([`${fraisRetenus} ligne${fraisRetenus > 1 ? 's' : ''} "Frais retenu" : la vente a ete annulee et remboursee en entier, mais Stripe ne restitue pas ses frais. Seul ce cout subsiste, sans chiffre d affaires ni TVA.`]))
  }
  l.push('')
  return l
}
