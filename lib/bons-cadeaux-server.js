...
export async function regimeBonPourCommerce(supabase, commercant) {
  ...
  try {
    const [{ data: articles }, { data: prestations }] = await Promise.all([
      ...
      // 🔴 J'AI ÉCRIT `prestations` ICI LE 03/09, ET LA TABLE S'APPELLE
      // `rdv_prestations`. Supabase ne lève pas : elle rend `{ data: null }`.
      // La déduction a donc tourné SANS AUCUNE PRESTATION, et classé quinze
      // bons en usage unique sur le seul catalogue d'articles. Chez un salon,
      // c'est précisément la mauvaise moitié du commerce qui manquait.
      supabase.from('rdv_prestations').select('tva_taux').eq('commercant_id', id),
    ])
    return regimeBon({
      ...
      // ⚠️ LA CATÉGORIE TRANCHE LE DOUTE : dans l'alimentaire, un catalogue à un
      // seul taux ne dit pas que le commerce n'en a qu'un, il dit que le
      // catalogue n'est pas fini.
