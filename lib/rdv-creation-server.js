...
import { capacitePrestation, premierePlaceLibre, estParCouverts, couvertsValides, occupationDe, dureeSelonCouverts } from './cours-collectifs'
import { enModeInventaire, formatLibrePour } from './inventaire-salle'
...
export async function creerReservationRdv(db, {
  ...
  const { data: prestation } = await db
    ...
    .select('id, nom, capacite, tva_taux, duree_minutes, commercant_id, par_couverts, couverts_min, couverts_max, duree_paliers')
    .eq('id', prestationId)
    .maybeSingle()
  ...
  if (champs?.source !== 'commercant') {
    ...
    const autorise = prestationAutoriseeSurCreneaux({
      ...
      finMin: timeToMinutes(heure) + dureeSelonCouverts(prestation, champs?.couverts),
      ...
      // ne décide de rien.
      estCours: capacitePrestation(prestation) > 1,
    })
  ...
  if (estParCouverts(prestation)) {
    ...
    // table : le cas du cours ne change pas d'une ligne. C'est la requête
    // suivante qui restreint aux prestations en mode table du commerce.
    const { data: formatsTable } = await db
      .from('rdv_prestations')
      .select('id, nom, actif, par_couverts, couverts_min, couverts_max, quantite')
      .eq('commercant_id', commercantId)
    ...
    const finMin = debutMin + dureeSelonCouverts(prestation, champs?.couverts)
    ...
    if (enModeInventaire(formatsTable)) {
      ...
      // 🔴 ET LA PLACE SE REFAIT AVEC LA TABLE RETENUE. `placeNo` a été calculé
      // plus haut, sur le format que l'écran proposait : si le serveur en a
      // choisi un autre, le rang appartient au mauvais format et l'index unique
      // rejetterait l'insertion — ou pire, laisserait passer une collision sur
      // le format d'à côté. Le rang n'a de sens qu'à l'intérieur d'un format.
      if (String(prestationRetenue.id) !== String(prestation.id)) {
        const capaciteRetenue = capacitePrestation(prestationRetenue)
        if (capaciteRetenue > 1) {
          const { data: dejaLaRetenue } = await db
            .from('rdv_reservations')
            .select('place_no')
            .eq('commercant_id', commercantId)
            .eq('prestation_id', prestationRetenue.id)
            .eq('date_rdv', dateRdv)
            .eq('heure_debut', heure)
            .in('statut', STATUTS_OCCUPENT)
            .is('deleted_at', null)
          placeNo = premierePlaceLibre(prestationRetenue, (dejaLaRetenue || []).map(r => r.place_no)) || 1
        } else {
          placeNo = 1
        }
      }
  ...
  const payload = {
    ...
    // vérifier du tout.
    duree_minutes: dureeSelonCouverts(prestationRetenue, couvertsRetenus),
    heure_fin: minutesToTime(timeToMinutes(heure) + dureeSelonCouverts(prestationRetenue, couvertsRetenus)),
  }
