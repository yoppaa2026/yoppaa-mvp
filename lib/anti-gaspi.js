...
export function offresOuvertes(offres = [], instant = new Date(), { reservable = () => false } = {}) {
  ...
  return liste
    ...
    .sort((a, b) => dabordLeReservable(a, b) || (a.restant - b.restant))
}

/**
 * Ce sur quoi le Yopper peut agir tout de suite passe devant.
 *
 * ⚠️ CETTE COMPARAISON EXISTAIT EN DEUX EXEMPLAIRES, et c'est le harnais de
 * mutation qui l'a dit : impossible de viser l'une des deux sans muter l'autre,
 * `String.replace` prenant toujours la PREMIÈRE occurrence. Une garde qui ne
 * peut pas être visée n'est pas une garde mesurée. Même leçon que le 04/09 sur
 * les statuts qui n'occupent pas un créneau : la duplication ÉTAIT le défaut.
 */
function dabordLeReservable(a, b) {
  if (a.reservable === b.reservable) return 0
  return a.reservable ? -1 : 1
}
...
// ─── COMBIEN IL EN RESTE, ET QUI LE COMPTE ──────────────────────────────────
//
// 🔴 LE DÉFAUT VU PAR ALEX LE 04/09 AU SOIR. Il publie TROIS assiettes à moitié
// prix, et la fiche en propose QUINZE : elle lisait le stock du jour de
// l'article, pas la quantité de l'offre. Quinze assiettes vendues 4,75 € au lieu
// de 9,50 €, c'est soixante et onze euros qui manquent à la caisse, sur une
// offre censée écouler trois restes.
//
// ⚠️ ET RIEN NE L'AURAIT SIGNALÉ. La commande passe, le paiement passe, le
// commerçant le découvre au comptoir. C'est exactement la forme de défaut que ce
// projet traque : pas d'erreur, pas de journal, de l'argent en moins.

/**
 * Le plafond de cette offre, ou `null` quand elle n'en a pas.
 *
 * ⚠️ `null` VEUT DIRE « SANS OBJET », PAS « ZÉRO ». Les lots et les duos
 * ordinaires n'ont pas de quantité propre : ils s'arrêtent au stock de leur
 * article. Confondre les deux les rendrait tous invendables.
 */
export function plafondDeLOffre(offre) {
  const q = Number(offre?.quantite)
  return Number.isFinite(q) && q > 0 ? Math.floor(q) : null
}

/**
 * Combien il en reste sur cette offre. `null` quand elle n'a pas de plafond.
 *
 * ⚠️ JAMAIS NÉGATIF. Une vente de plus que le plafond peut arriver — deux
 * paniers qui partent en même temps — et il ne faut pas que l'écran affiche
 * « -1 restant » à celui qui arrive après.
 */
export function resteSurOffre(offre, dejaVendu = 0) {
  const plafond = plafondDeLOffre(offre)
  if (plafond === null) return null
  const vendu = Number(dejaVendu)
  return Math.max(0, plafond - (Number.isFinite(vendu) && vendu > 0 ? vendu : 0))
}

/**
 * Pourquoi cette commande dépasse l'offre. `null` si elle tient.
 *
 * ⚠️ LE MESSAGE DIT COMBIEN IL EN RESTE, pas seulement que c'est trop. Sans le
 * chiffre, le Yopper retire un article au hasard et réessaie jusqu'à tomber
 * juste, ou abandonne.
 */
export function refusDeQuantite({ titre, offre, dejaVendu = 0, demande = 0 }) {
  const reste = resteSurOffre(offre, dejaVendu)
  if (reste === null) return null
  const veut = Number(demande)
  if (!Number.isFinite(veut) || veut <= 0) return null
  if (veut <= reste) return null
  const quoi = titre || 'Cette offre'
  if (reste === 0) return `${quoi} : tout est parti. L'offre vient de se terminer.`
  return `${quoi} : il n'en reste que ${reste} à ce prix.`
}

/**
 * Ce que la carte annonce : « il en reste 3 ».
 *
 * ⚠️ ON DIT LA RARETÉ, PAS L'URGENCE. « Il en reste 3 » informe ; « plus que
 * 3 ! » presse, et ce n'est pas notre rôle. Même règle que le temps restant.
 */
export function libelleReste(reste) {
  const n = Number(reste)
  if (!Number.isFinite(n) || n <= 0) return ''
  return `il en reste ${n}`
}

// ─── OÙ ÇA S'AFFICHE, ET DANS QUEL ORDRE ────────────────────────────────────
//
...
// s'affichent ? »
//
// La réponse était : AUCUN CRITÈRE. L'accueil charge toute la Belgique publiée,
// et « Rien ne se perd » lisait cette liste entière, triée par temps restant.
// Un Yopper de Mettet voyait donc en PREMIER l'offre d'Arlon qui ferme dans dix
// minutes, et sa boulangerie d'à côté tout en bas, s'il descendait jusque-là.
//
// ⚠️ ET LE TEMPS RESTANT ÉTAIT LE MAUVAIS PREMIER TRI. Cette section existe
// pour qu'on y aille MAINTENANT : ce qui ferme dans dix minutes à quarante-cinq
// kilomètres n'est pas une offre, c'est une frustration. La proximité décide,
// le temps restant départage.
//
// ⚠️ LE COMMERÇANT N'A PAS DE CODE POSTAL DANS LA VUE PUBLIQUE. Le seul signal
// géographique disponible est la distance calculée par l'accueil jusqu'au lieu
// ACTIF le plus proche — celle qui rend une professeure de yoga visible depuis
// chacune de ses salles. On s'appuie sur elle et sur rien d'autre.
//
// ⚠️ SANS POSITION, ON NE FILTRE RIEN. Un Yopper qui a refusé la géolocalisation
// n'a aucune distance : lui masquer la section le priverait de tout au nom d'un
// périmètre qu'on est incapable de mesurer. Il retrouve alors l'ordre d'avant,
// le temps restant, et aucune distance affichée puisqu'on n'en connaît aucune.

/**
 * Au-delà, personne ne traverse la province pour une tarte à quatre euros.
 *
 * ⚠️ C'EST UN PLAFOND, PAS UNE PROMESSE DE PROXIMITÉ. En zone rurale, la seule
 * offre du soir peut être à vingt kilomètres : elle s'affiche, avec sa distance
 * écrite sur la carte, et le Yopper décide. Masquer aurait fabriqué une section
 * vide un mardi soir à Mettet, c'est-à-dire une fonction morte.
 */
export const RAYON_INVENDU_M = 25000

// ⚠️ `INVENDUS_AFFICHES` A ÉTÉ RETIRÉE LE 05/09, quelques heures après avoir été
// écrite. Elle plafonnait à quatre les cartes affichées sur l'accueil, avant un
// « Voir les 9 ». L'accueil n'affiche PLUS AUCUNE carte : il porte une bande
// d'une ligne, et la liste complète vit dans son propre panneau, sans plafond.
//
// 🔴 UNE CONSTANTE SANS APPELANT EST DU CODE MORT, ET SA GARDE EST UNE GARDE
// VERTE QUI NE PROTÈGE RIEN. La laisser « au cas où » aurait ajouté une ligne de
// plus au décompte des vérifications sans rien mesurer du produit.

/**
 * Deux distances, la plus proche d'abord, et l'inconnue en dernier.
 *
 * ⚠️ `null` NE VAUT PAS ZÉRO. `Number(null)` rend 0, et une offre sans position
 * connue serait passée devant la boulangerie d'en face. C'est le piège du zéro,
 * déjà vu six fois dans ce projet.
 */
function comparerDistance(a, b) {
  if (a === b) return 0
  if (a === null) return 1
  if (b === null) return -1
  return a - b
...
/**
 * Les offres ouvertes, ramenées au périmètre du Yopper et classées par
 * proximité.
 *
 * Prend la liste DÉJÀ MISE EN FORME par `offresOuvertes` — on ne refait ni le
 * filtre d'ouverture ni le calcul du temps restant, qui n'ont pas à exister en
 * deux exemplaires.
 *
 * @param {Array}    ouvertes   ce que rend `offresOuvertes`
 * @param {Function} distanceDe (offre) => mètres, ou `null` si on ne sait pas
 * @param {number}   rayon      le plafond, en mètres
 */
export function offresProches(ouvertes = [], { distanceDe = () => null, rayon = RAYON_INVENDU_M } = {}) {
  const liste = Array.isArray(ouvertes) ? ouvertes : []
  // 🔴 `Infinity` VEUT DIRE « PLUS DE PLAFOND », ET IL FALLAIT L'ÉCRIRE (18/09).
  // `Number.isFinite(Infinity)` vaut FAUX : sans cette première branche, un
  // appelant qui demande « montre-moi tout » retombait sur les 25 kilomètres,
  // silencieusement, et le bouton « voir plus loin » n'aurait rien fait du tout.
  // Le piège est le même que celui du zéro noté juste en dessous : une valeur
  // particulière qui traverse un test générique sans être vue.
  const plafond = rayon === Infinity
    ? Infinity
    : (Number.isFinite(Number(rayon)) ? Number(rayon) : RAYON_INVENDU_M)
  return liste
    .map(o => {
      // 🔴 LE PIÈGE DU ZÉRO, ATTRAPÉ PAR LE BANC DANS CE MODULE MÊME, 05/09.
      // `Number(null)` vaut ZÉRO, pas NaN : la première écriture de cette ligne
      // transformait une distance inconnue en « zéro mètre », et l'offre sans
      // position connue passait donc DEVANT la boulangerie d'en face. Le tri
      // plus bas était juste, c'est la mesure qui mentait.
      const brut = distanceDe(o?.offre)
      const d = brut === null || brut === undefined || brut === '' ? NaN : Number(brut)
      return { ...o, distance: Number.isFinite(d) ? d : null }
    })
    // ⚠️ LA DISTANCE INCONNUE PASSE. Voir plus haut : on ne masque pas au nom
    // d'une mesure qu'on n'a pas.
    .filter(o => o.distance === null || o.distance <= plafond)
    .sort((a, b) =>
      dabordLeReservable(a, b)
      || comparerDistance(a.distance, b.distance)
      || (a.restant - b.restant))
}

/**
 * Ce que le Yopper envoie à quelqu'un quand il partage une offre.
 *
 * ⚠️ LE TEXTE DIT L'OFFRE, PAS LA MARQUE. « Yoppaa, ton quartier dans ta
 * poche » convient pour partager un commerce ; ici la personne qui reçoit doit
 * comprendre en une ligne qu'il reste quelque chose, chez qui, et à quel prix.
 *
 * ⚠️ ET IL NE DIT PAS COMBIEN IL EN RESTE. Entre l'envoi et la lecture, tout
 * peut être parti : un chiffre gravé dans un message devient faux tout seul.
 */
export function texteDePartage(offre, nomCommerce) {
  const quoi = offre?.titre || 'Un invendu'
  const chez = nomCommerce ? ` ${chezLeCommerce(nomCommerce)}` : ''
  const remise = remisePourcent(offre)
  const combien = remise > 0 ? ` à -${remise} %` : ''
  return `${quoi}${chez}${combien}, avant la fermeture. ${TITRE_YOPPER}, sur Yoppaa.`
}

/**
 * L'adresse d'une offre : la fiche du commerçant, à hauteur de l'offre.
 *
 ...
 * deal ». Sur une boulangerie à quarante articles, l'invendu est à trois écrans
 * de défilement : le Yopper arrivait sur un catalogue et ne retrouvait pas ce
 * qu'il venait chercher.
 *
 * ⚠️ UN PARAMÈTRE, PAS UNE ANCRE `#`. Le catalogue se peint APRÈS le premier
 * rendu ; une ancre native viserait un élément qui n'existe pas encore et le
 * navigateur ne réessaie jamais. Et la fiche défile dans un conteneur interne,
 * pas dans la fenêtre.
 */
/**
 * Ce que la bande annonce : « 4 offres près de toi ».
 *
 * 🔴 « PRÈS DE TOI » MENT SANS POSITION, et c'est le seul piège de ce libellé.
 * Un Yopper qui a refusé la géolocalisation n'a AUCUNE distance : la liste lui
 * montre alors les offres de tout le pays, classées par urgence. Lui écrire
 * « près de toi » serait une affirmation fausse, sur l'écran d'accueil, à
 * chaque ouverture.
 *
 * ⚠️ ON REGARDE LES OFFRES, PAS UN DRAPEAU DE GÉOLOCALISATION. C'est la même
 * règle que la fenêtre qui fait l'invendu : la donnée décide, pas un booléen
 * posé à côté qui pourrait dire le contraire.
 *
 * ⚠️ ET L'ACCORD SUIT LE NOMBRE. « 1 offres » sur un écran d'accueil se
 * remarque immédiatement et fait douter du reste.
 */
export function libelleDecompte(offres = []) {
  const liste = Array.isArray(offres) ? offres : []
  const n = liste.length
  if (n === 0) return ''
  const situe = liste.some(o => o && o.distance !== null && o.distance !== undefined)
  return `${n} offre${n > 1 ? 's' : ''} ${situe ? 'près de toi' : 'avant la fermeture'}`
}

/**
 * L'adresse de la liste complète.
 *
 * 🔴 CE N'EST PAS UNE ROUTE À PART, ET C'EST DÉLIBÉRÉ (arbitrage Alex, 05/09).
 * Une vraie page devrait recharger les commerces, les lieux, la position, les
 * distances et le relevé des ventes : cinq chargements recopiés, donc cinq
 * occasions de diverger. Or la distance se mesure jusqu'au LIEU ACTIF le plus
 * proche, et une seconde version de ce calcul a déjà produit un défaut.
 *
 * La liste est donc un panneau de l'accueil, ADRESSÉ : le paramètre suffit à
 * l'ouvrir, le bouton retour du téléphone la referme, et `/rien-ne-se-perd`
 * renvoie ici pour la communication.
 */
export const PARAM_LISTE = 'invendus'
export const CHEMIN_LISTE = `/commander?${PARAM_LISTE}=1`

export const PARAM_OFFRE = 'offre'

export function lienVersOffre(slug, offreId) {
  if (!slug) return null
  const base = `/commander/${encodeURIComponent(slug)}`
  if (!offreId) return base
  return `${base}?${PARAM_OFFRE}=${encodeURIComponent(offreId)}`
}
