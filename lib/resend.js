...
// ⚠️ LE NOM DU BON SUIT LE MÉTIER (31/08). Chaque gabarit qui nomme le bon
// reçoit désormais `commercant_categorie`, et un email part sans retour
// possible : ici plus qu'ailleurs, une catégorie oubliée dans un `select` se
// paie comptant. Le repli reste « cadeau », compris partout.
...
// ⚠️ L'ADRESSE D'UNE FICHE VIENT DE SA SOURCE UNIQUE. Elle était recomposée à
// la main ici, et un email ne se corrige pas une fois parti : un slug non
// encodé y fabrique un lien mort chez le client, définitivement.
//
// ⚠️ ET DEUX FONCTIONS, PAS UNE. `lienFiche` est l'adresse générique, juste
// dans tous les cas ; `lienFicheRdv` vise l'agenda, et ces deux emails-ci le
// visent délibérément — on écrit à quelqu'un qui vient de prendre rendez-vous.
import { lienFiche, lienFicheRdv, LIEN_ACCUEIL } from './lien-fiche'
...
export function lignesAvantages(C, { fidelite_remise = 0, bon_cadeau_montant = 0, commercant_categorie = null } = {}) {
  const montant = (v) => {
    const n = Number(v)
  ...
  return [
    ...
    bon ? ligne(`🎁 ${libelleBon(commercant_categorie, { majuscule: true })}`, '#10B981', bon) : '',
  ].join('')
}
...
// ⚠️ LE MÊME ENVOI, SOUS SON VRAI NOM. Les emails destinés aux Yoppers
// partaient par `envoyerAuCommercant`, ce qui est exact techniquement et
// trompeur à la lecture : on finit par croire qu'un email de client va chez le
// commerçant. Rien ne change pour les appelants existants.
export async function envoyerAuYopper({ to, subject, html, attachments }) {
  return envoyer({ to, subject, html, attachments })
}
...
// ⚠️ UN IMPORT, PAS UN `export … from` : la seconde forme ré-exporte sans
// jamais lier le nom dans ce module, et l'appel plus bas serait une variable
// inexistante. C'est `npm run verif:undef` qui l'a dit, et personne d'autre :
// `no-undef` est éteint dans la configuration principale.
import { elisionDe } from './francais'
// 🔴 LE VOCABULAIRE DU MÉTIER DANS LES EMAILS (Alex, 09/09, le soir du jour où
// Le Bistrologue a dit oui). Un client qui vient de réserver une table recevait
// « Ton RDV est confirmé ». L'écran avait été corrigé l'après-midi ; l'email,
// lui, RESTE dans la boîte du client et se relit la veille du repas.
//
// ⚠️ `commercant_categorie` EST FACULTATIF PARTOUT, et son absence rend le
// vocabulaire du rendez-vous. Un email qui part sans la colonne ne change donc
// pas d'un mot : aucune régression possible sur le parc existant.
import { motsReservation, objetReservation, intituleReservation } from './reservation-metier'
...
<body style="margin:0;padding:0;background:${C.bg};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:${C.ink};">
  <div style="max-width:560px;margin:0 auto;padding:24px 16px;">
    <div style="background-color:${C.panel};background-image:linear-gradient(135deg,${C.panel} 0%,#2D0F6B 60%,${C.ink} 100%);border-radius:18px 18px 0 0;padding:28px 28px 22px;">
      ${logoEmailDark(32)}
    ...
    <div style="background:#fff;border-radius:0 0 18px 18px;padding:28px;border:1px solid ${C.pale};border-top:none;">
      ...
      ${ctaUrl && ctaLabel ? `
        <p style="margin:24px 0 0;text-align:center;">
          <a href="${ctaUrl}" style="display:inline-block;padding:13px 28px;background-color:${C.panel};background-image:linear-gradient(135deg,${C.panel},${C.main});color:#fff;text-decoration:none;border-radius:100px;font-weight:800;font-size:15px;letter-spacing:-0.2px;">
            ${ctaLabel} →
          </a>
        </p>
      ` : ''}
...
// LE FILET : DES DOSSIERS ATTENDENT DEPUIS TROP LONGTEMPS.
//
// 🔴 L'alerte de soumission part d'un `fetch` DEPUIS LE NAVIGATEUR. Elle se
// perd sur un 403, sur un 500, ou simplement si l'onglet se ferme entre
// l'enregistrement et l'appel : la fiche passe en attente et aucun email n'est
// même tenté. Ce rappel-ci part du SERVEUR et ne dépend de personne.
//
// ⚠️ IL NE SONNE QUE POUR CE QUI A DÉPASSÉ LE DÉLAI PROMIS au commerçant
// (24 h ouvrées, annoncées sur la page d'inscription). Un rappel quotidien de
// tous les dossiers deviendrait un bruit de fond qu'on cesse de lire.
export function emailDossiersEnAttente({ dossiers = [] }) {
  const adminUrl = 'https://www.yoppaa.app/admin'
  const lignes = dossiers.map(({ fiche, attente }) => `
    <tr>
      <td style="padding:8px 10px;border-bottom:1px solid ${C.pale};font-size:13px;color:${C.ink};font-weight:700;">
        ${echapperHtml(fiche?.nom || 'Sans nom')}
      </td>
      <td style="padding:8px 10px;border-bottom:1px solid ${C.pale};font-size:13px;color:${C.ink};">
        ${echapperHtml(fiche?.type || '—')}
      </td>
      <td style="padding:8px 10px;border-bottom:1px solid ${C.pale};font-size:13px;color:#DC2626;font-weight:800;white-space:nowrap;">
        ${attente?.texte || ''} · ${attente?.ouvres ?? '?'} j ouvrés
      </td>
    </tr>`).join('')
  const n = dossiers.length
  return layout({
    audience: 'admin',
    title: n > 1 ? `${n} dossiers attendent ta validation` : 'Un dossier attend ta validation',
    intro: n > 1
      ? `<strong>${n} commerçants</strong> ont soumis leur inscription et attendent depuis plus que le délai annoncé.`
      : `Un commerçant a soumis son inscription et attend depuis plus que le délai annoncé.`,
    body: `
      <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:${C.ink};">
        La page d'inscription leur promet une réponse <strong>sous 24 h ouvrées</strong>.
      </p>
      <table style="width:100%;border-collapse:collapse;margin-bottom:14px;">${lignes}</table>
      <p style="margin:0;font-size:12.5px;line-height:1.6;color:${C.muted};">
        Ce rappel part du serveur : il arrive même si l'alerte de soumission
        s'est perdue en route, et il s'arrête dès que la file est vide.
      </p>
    `,
    ctaUrl: adminUrl,
...
// 1 ter) CE QU'UN HABITANT NOUS DIT, ET QUE PERSONNE N'APPRENAIT (08/09)
//
// 🔴 Les suggestions de commerces et les signalements de fiche arrivaient bien
// en base, et RIEN NE PRÉVENAIT PERSONNE. Il fallait penser à ouvrir /admin
// pour les découvrir. Une suggestion, c'est une porte à aller frapper : elle
// vaut le jour où elle arrive, pas le mois suivant.
//
// ⚠️ CE TEXTE EST ÉCRIT PAR UN VISITEUR SANS COMPTE et atterrit dans une boîte
// mail signée par le domaine yoppaa.app. Chaque valeur passe par
// `echapperHtml`, sans exception : c'est la règle du haut de ce module.
export function emailSuggestionCommerce({ nom_commerce, adresse, type_commerce, commentaire, deja = 0 }) {
  const ligne = (label, valeur) => valeur
    ? `<tr><td style="padding:5px 0;color:${C.muted};vertical-align:top;">${label}</td><td style="padding:5px 0;color:${C.ink};font-weight:700;text-align:right;">${echapperHtml(valeur)}</td></tr>`
    : ''
  return layout({
    audience: 'admin',
    title: 'Un habitant réclame un commerce',
    intro: `Quelqu&rsquo;un veut voir <strong>${echapperHtml(nom_commerce)}</strong> sur Yoppaa.${deja > 1 ? ` C&rsquo;est la <strong>${deja}<sup>e</sup> fois</strong> qu&rsquo;on le réclame.` : ''}`,
    body: `
      <div style="background:${C.bg};border-radius:12px;padding:16px 18px;border:1px solid ${C.pale};">
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          ${ligne('Commerce', nom_commerce)}
          ${ligne('Adresse', adresse)}
          ${ligne('Type', type_commerce)}
        </table>
        ${commentaire ? `
          <p style="margin:14px 0 0;font-size:13px;line-height:1.55;color:${C.ink};background:#fff;border-radius:10px;padding:10px 12px;border:1px solid ${C.pale};">
            &laquo;&nbsp;${echapperHtml(commentaire)}&nbsp;&raquo;
          </p>` : ''}
      </div>
      <p style="margin:16px 0 0;font-size:12px;color:${C.muted};line-height:1.5;">
        Personne n&rsquo;est nommé : ni celui qui demande, ni son adresse. C&rsquo;est la promesse de la page d&rsquo;accueil.
      </p>
    `,
    ctaUrl: 'https://www.yoppaa.app/admin',
    ctaLabel: 'Voir les commerces réclamés',
  })
}

export function emailSignalementFiche({ motif, description, cible_nom, commercant_id, service_id }) {
  const quoi = commercant_id ? 'un commerce' : service_id ? 'un service public' : 'une fiche'
  return layout({
    audience: 'admin',
    title: 'Signalement sur une fiche',
    intro: `Un habitant signale un problème sur ${quoi}${cible_nom ? ` : <strong>${echapperHtml(cible_nom)}</strong>` : ''}.`,
    body: `
      <div style="background:${C.bg};border-radius:12px;padding:16px 18px;border:1px solid ${C.pale};">
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <tr><td style="padding:5px 0;color:${C.muted};">Motif</td><td style="padding:5px 0;color:${C.ink};font-weight:700;text-align:right;">${echapperHtml(motif || 'autre')}</td></tr>
          <tr><td style="padding:5px 0;color:${C.muted};">Fiche</td><td style="padding:5px 0;color:${C.muted};font-size:11px;text-align:right;font-family:monospace;">${echapperHtml(commercant_id || service_id || '—')}</td></tr>
        </table>
        ${description ? `
          <p style="margin:14px 0 0;font-size:13px;line-height:1.55;color:${C.ink};background:#fff;border-radius:10px;padding:10px 12px;border:1px solid ${C.pale};">
            &laquo;&nbsp;${echapperHtml(description)}&nbsp;&raquo;
          </p>` : ''}
      </div>
      <p style="margin:16px 0 0;font-size:12px;color:${C.muted};line-height:1.5;">
        Le commerçant n&rsquo;est pas prévenu : tu arbitres d&rsquo;abord. Un signalement peut être faux ou de mauvaise foi.
      </p>
    `,
    ctaUrl: 'https://www.yoppaa.app/admin',
    ctaLabel: 'Ouvrir l’administration',
...
// 1 ter) Signalement d'un AVIS, et c'est un autre travail que de signaler une
// fiche. Sur une fiche, on corrige une donnée : des horaires, une adresse. Sur
// un avis, on arbitre un CONTENU écrit par quelqu'un, et la décision est de le
// retirer ou de le laisser. L'email doit donc porter le texte incriminé, sinon
// le modérateur ouvre l'administration pour aller le chercher, et il jugera de
// mémoire une fois sur deux.
//
// ⚠️ L'AUTEUR DE L'AVIS N'APPARAÎT PAS. On juge ce qui est écrit, pas qui l'a
// écrit, et faire circuler une identité par email n'apporte rien à la décision.
export function emailSignalementAvis({ motif_libelle, description, commerce_nom, avis_texte, avis_note, avis_id }) {
  return layout({
    audience: 'admin',
    title: 'Signalement sur un avis',
    intro: `Un habitant signale le contenu d’un avis${commerce_nom ? ` publié chez <strong>${echapperHtml(commerce_nom)}</strong>` : ''}.`,
    body: `
      <div style="background:${C.bg};border-radius:12px;padding:16px 18px;border:1px solid ${C.pale};">
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <tr><td style="padding:5px 0;color:${C.muted};">Motif</td><td style="padding:5px 0;color:${C.ink};font-weight:700;text-align:right;">${echapperHtml(motif_libelle || 'Autre')}</td></tr>
          <tr><td style="padding:5px 0;color:${C.muted};">Note laissée</td><td style="padding:5px 0;color:${C.ink};font-weight:700;text-align:right;">${echapperHtml(String(avis_note ?? '—'))} / 5</td></tr>
          <tr><td style="padding:5px 0;color:${C.muted};">Avis</td><td style="padding:5px 0;color:${C.muted};font-size:11px;text-align:right;font-family:monospace;">${echapperHtml(avis_id || '—')}</td></tr>
        </table>

        <p style="margin:14px 0 4px;font-size:11px;font-weight:800;color:${C.muted};text-transform:uppercase;letter-spacing:0.7px;">L’avis signalé</p>
        <p style="margin:0;font-size:13px;line-height:1.55;color:${C.ink};background:#fff;border-radius:10px;padding:10px 12px;border:1px solid ${C.pale};">
          ${avis_texte ? `&laquo;&nbsp;${echapperHtml(avis_texte)}&nbsp;&raquo;` : '<em>Aucun commentaire, une note seule.</em>'}
        </p>

        ${description ? `
          <p style="margin:14px 0 4px;font-size:11px;font-weight:800;color:${C.muted};text-transform:uppercase;letter-spacing:0.7px;">Ce que dit celui qui signale</p>
          <p style="margin:0;font-size:13px;line-height:1.55;color:${C.ink};background:#fff;border-radius:10px;padding:10px 12px;border:1px solid ${C.pale};">
            &laquo;&nbsp;${echapperHtml(description)}&nbsp;&raquo;
          </p>` : ''}
      </div>
      <p style="margin:16px 0 0;font-size:12px;color:${C.muted};line-height:1.5;">
        Ni le commerçant ni l&rsquo;auteur ne sont prévenus : tu arbitres d&rsquo;abord. Un signalement peut viser un avis parfaitement légitime, et un commerçant mécontent d&rsquo;une note reste un signaleur possible.
      </p>
    `,
    ctaUrl: 'https://www.yoppaa.app/admin',
    ctaLabel: 'Ouvrir l’administration',
    ...
    body: `
      ...
      <div style="background:${C.bg};border-radius:12px;padding:16px 18px;border:1px solid ${C.pale};">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          ...
          <tr><td style="padding:5px 0;color:${C.muted};">Adresse</td><td style="padding:5px 0;color:${C.ink};font-weight:700;text-align:right;">${echapperHtml(adresse) || '—'}</td></tr>
...
export function emailValidationCommercant({ nom, slug, avant_lancement = false }) {
  const ficheUrl     = lienFiche(slug) || LIEN_ACCUEIL
  const dashboardUrl = 'https://www.yoppaa.app/dashboard'
  ...
  return layout({
    ...
    body: `
      <p style="margin:0 0 14px;font-size:14px;line-height:1.6;color:${C.ink};">
        ${avant_lancement
          ? `Tu as jusqu'au <strong>${ouverture}</strong> pour la préparer tranquillement : tes horaires, ton catalogue, tes créneaux, tes deals. C'est ce jour-là que tout s'ouvre au public, et tu seras prêt.`
...
// 3 quater) Email AU COMMERÇANT : son inscription s'est arrêtée en route.
//
// ⚠️ IL N'A RIEN FAIT DE MAL, ET LE TON DOIT LE DIRE. Ce n'est ni un rappel à
// l'ordre ni une relance commerciale : quelqu'un a été interrompu et on lui
// tend la main. Une seule relance, jamais deux (`relance_inscription_envoyee_at`).
//
// ⚠️ ON NOMME CE QU'IL A DÉJÀ FAIT plutôt que ce qui manque. « Ton dossier est
// rempli à 70 % » se lit tout autrement que « il te manque trois champs », et
// c'est la même information.
export function emailRelanceInscription({ nom, avancement = null }) {
  const reprendreUrl = 'https://www.yoppaa.app/signup'
  const dejaFait = Number.isFinite(avancement) && avancement > 0
    ? `Ton dossier est déjà rempli à <strong style="color:${C.main};">${avancement} %</strong>. `
    : ''
  return layout({
    audience: 'commercant',
    title: 'Ton inscription t’attend',
    intro: `Bonjour <strong>${nom}</strong>, tu as commencé ton inscription sur Yoppaa et tu n'es pas allé jusqu'au bout. ${dejaFait}Tout ce que tu as saisi est conservé.`,
    body: `
      <p style="margin:0 0 14px;font-size:14px;line-height:1.6;color:${C.ink};">
        Il ne reste que <strong>quelques minutes</strong> pour terminer. Tu reprends là où tu t'es arrêté, rien n'est à refaire.
      </p>
      <p style="margin:0 0 14px;font-size:13px;line-height:1.6;color:${C.ink};">
        Une fois ton dossier envoyé, on le valide à la main et ta page part en ligne.
      </p>
      <p style="margin:0;font-size:12.5px;line-height:1.6;color:${C.muted};">
        Tu as changé d'avis ou tu t'es inscrit par erreur ? Ignore simplement ce message, c'est le seul que tu recevras à ce sujet.
      </p>
    `,
    ctaUrl: reprendreUrl,
...
// « 0,00 € de récompense ».
// ═══════════════════════════════════════════════════════════════════════════
// 🔴 CE QUI EST RÉELLEMENT PASSÉ PAR LA CARTE (Alex, 01/09)
// ═══════════════════════════════════════════════════════════════════════════
//
// Sa commande #RE4 : 21,90 €, couverts en entier par ses bons. L'email
// d'annulation affichait quand même « Le remboursement est lancé. Tu verras le
// montant revenir sur ton moyen de paiement dans 5 à 10 jours. » **Stripe n'a
// jamais rien encaissé** : il aurait guetté un virement qui ne viendrait pas.
//
// 🔴 LA CAUSE EST UN DRAPEAU QUI DIT AUTRE CHOSE QUE CE QU'ON LUI DEMANDE.
// `paye_en_ligne` vaut `true` sur une commande couverte par des avantages :
// `create-rdv-commande` l'écrit ainsi, exprès, parce qu'elle est « équivalente
// à un paiement en ligne » du point de vue du commerçant. Il répond à
// « le client doit-il encore payer », pas à « la carte a-t-elle été débitée ».
//
// ⚠️ ET LE FRÈRE EST PIRE : le même bloc dit au COMMERÇANT « rembourse
// manuellement depuis ton Stripe Dashboard » sur un paiement que Stripe n'a
// jamais vu. Il chercherait une transaction inexistante.
//
// ⚠️ LE PIÈGE DU ZÉRO, une fois de plus : `null` veut dire « on ne sait pas »,
// et surtout pas « rien ». Un appelant qui ne passe pas le total garde
// exactement le comportement d'avant, plutôt que de taire un vrai virement.
export function partCartePayee({ total, fidelite_remise, bon_cadeau_montant, paye_en_ligne } = {}) {
  if (!paye_en_ligne) return 0
  const t = Number(total)
  if (!Number.isFinite(t)) return null
  const arr = (n) => Math.round((Number(n) || 0) * 100) / 100
  return Math.max(0, arr(t - arr(fidelite_remise) - arr(bon_cadeau_montant)))
}

export function ligneAvantagesEmail({ fidelite_remise, bon_cadeau_montant, nb_bons = 0, commercant_categorie = null } = {}) {
  const montant = (v) => {
    const n = Number(v)
  ...
  // ⚠️ LE PLURIEL SUIT LE NOMBRE DE BONS, pas le montant : une commande peut en
  // porter cinq depuis le 01/09, et « en bon gourmand » devant trois bons est
  // faux pour le commerçant comme pour le Yopper.
  if (bon) bouts.push(`<strong>${euros(bon)}</strong> en ${libelleBon(commercant_categorie, { pluriel: Number(nb_bons) > 1 })}`)
...
export function emailRdvConfirme({
  ...
  // ⚠️ `bon_cadeau_montant` EST ARRIVÉ LE 28/08, quand le rendez-vous a eu le
  // droit d'en porter un. Sans lui ici, `soldeRdv` recevrait `undefined`,
  // vaudrait 0, et l'email annoncerait au client un solde qu'il ne doit plus.
  prix_estime, acompte_paye, acompte_montant, fidelite_remise = 0, bon_cadeau_montant = 0,
  // 🔴 COMBIEN DE BONS (01/09) : la phrase de la politique d'annulation promet
  // le retour « de ton bon », et un rendez-vous peut en porter cinq.
  nb_bons = 0,
  delai_annulation_heures = 24,
  commercant_categorie = null,   // le nom du bon suit le métier (31/08)
  ...
  produits = null,
  // 🔴 UNE TABLE SE DIT EN PERSONNES (email d'Alex, 10/09 tard). `table` : la
  // prestation est une table (`par_couverts`) ; `couverts` : combien ils sont.
  table = false,
  couverts = null,
  // 🔴 LE MONTANT GARANTI N'ÉTAIT NULLE PART DANS CET EMAIL (Alex, 16/09, E6).
  ...
  // gardera, et celui qu'il ressortira le jour d'un débit. Zéro = pas de
  // garantie, et alors rien ne s'affiche.
  empreinte_montant = 0,
  // 🔴 « RAPPEL AUTOMATIQUE 24H AVANT » SUR UNE TABLE DE CE SOIR (même email).
  // L'alarme du fichier calendrier sonne la veille : à moins de 24 heures, elle
  // tombe dans le passé et ne sonne jamais. L'expéditeur dit si elle sonnera ;
  // sans rien savoir, on garde la phrase d'avant.
  rappel_24h = true,
}) {
  ...
  const solde = soldeRdv({ prix_estime, fidelite_remise, bon_cadeau_montant, acompte_montant, acompte_paye })
  // Ce que la table garantit, et s'il y a vraiment eu un acompte à rembourser.
  // ⚠️ LES DEUX SONT DISTINCTS : une table garantie n'a PAS d'acompte, rien
  // n'a été encaissé. Les confondre, c'est promettre un remboursement qui
  // n'aura jamais lieu.
  const garantie = Number(empreinte_montant) > 0 ? Math.round(Number(empreinte_montant) * 100) / 100 : 0
  const aAcompte = !!acompte_paye && Number(acompte_montant) > 0
  const mots = motsReservation({ categorie: commercant_categorie })
  return layout({
    audience: 'yopper',
    commercantNom: commercant_nom,
    title: deplace ? mots.emailDeplace : mots.emailConfirme,
    intro: deplace
      ? `<strong>${echapperHtml(yopper_prenom)}</strong>, ${mots.laSienne} chez <strong>${echapperHtml(commercant_nom)}</strong> a été <strong>${mots.participeDeplace}</strong>. Voici la nouvelle date à retenir 🟣`
      : `<strong>${echapperHtml(yopper_prenom)}</strong>, ${mots.laSienne} chez <strong>${echapperHtml(commercant_nom)}</strong> est bien ${mots.participeConfirme}. À très vite 🟣`,
    body: `
      ...
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;">
        ...
        ${numero_rdv ? `<p style="margin:8px 0 0;font-size:12px;color:${C.muted};">${mots.numeroLabel} <strong style="color:${C.main};">#${numero_rdv}</strong></p>` : ''}
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:13px;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        <tr><td style="padding:10px 14px;color:${C.muted};border-bottom:1px solid ${C.pale};">${mots.prestationLigne}</td><td style="padding:10px 14px;color:${C.ink};font-weight:700;text-align:right;border-bottom:1px solid ${C.pale};">${echapperHtml(objetReservation({ prestation_nom, table, couverts })) || '—'}</td></tr>
        ${praticien_prenom ? `<tr><td style="padding:10px 14px;color:${C.muted};border-bottom:1px solid ${C.pale};">${mots.praticienLigne}</td><td style="padding:10px 14px;color:${C.ink};font-weight:700;text-align:right;border-bottom:1px solid ${C.pale};"><span style="display:inline-flex;align-items:center;gap:6px;"><span style="width:10px;height:10px;border-radius:50%;background:${praticien_couleur || C.main};display:inline-block;"></span>${echapperHtml(praticien_prenom)}${praticien_nom ? ' ' + echapperHtml(praticien_nom) : ''}</span></td></tr>` : ''}
        ...
        ${garantie > 0 ? `<tr><td style="padding:10px 14px;color:${C.muted};border-bottom:1px solid ${C.pale};">Garantie</td><td style="padding:10px 14px;color:${C.ink};font-weight:900;text-align:right;border-bottom:1px solid ${C.pale};">${euros(garantie)}<br/><span style="font-weight:600;font-size:11.5px;color:${C.muted};">Rien n&rsquo;est débité si tu viens</span></td></tr>` : ''}
        ${lignesAvantages(C, { fidelite_remise, bon_cadeau_montant, commercant_categorie })}
      ...
      <div style="background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          ...
          ${(() => {
            // 🔴 « PAYÉ EN LIGNE » ÉTAIT AFFIRMÉ SUR LE TOTAL BRUT, ET C'EST
            // FAUX DÈS QU'UN BON PAIE (01/09). Sur le rendez-vous d'Alex, ses
            // bons couvraient les 21,90 € du shampoing : la carte n'a rien
            // encaissé, et l'email lui faisait chercher un débit bancaire qui
            // n'existe pas.
            //
            // ⚠️ REPLI SUR L'ANCIENNE FORME : les appelants qui ne passent que
            // `total` gardent exactement le comportement d'avant. Un champ
            // absent ne doit pas transformer un email juste en email muet.
            const bonProduits = Number(produits.bon) || 0
            const recompenseProduits = Number(produits.recompense) || 0
            const enLigne = produits.paye_en_ligne === undefined
              ? Number(produits.total) || 0
              : Number(produits.paye_en_ligne) || 0
            const pluriel = Number(produits.nb_bons) > 1
            const l = []
            if (bonProduits > 0) {
              l.push(`<tr><td style="padding:9px 14px;color:${C.muted};border-bottom:1px solid ${C.pale};">${libelleBon(commercant_categorie, { pluriel, majuscule: true })}</td><td style="padding:9px 14px;color:#10B981;font-weight:800;text-align:right;border-bottom:1px solid ${C.pale};">−${euros(bonProduits)}</td></tr>`)
            }
            if (recompenseProduits > 0) {
              l.push(`<tr><td style="padding:9px 14px;color:${C.muted};border-bottom:1px solid ${C.pale};">Récompense fidélité</td><td style="padding:9px 14px;color:#10B981;font-weight:800;text-align:right;border-bottom:1px solid ${C.pale};">−${euros(recompenseProduits)}</td></tr>`)
            }
            // ⚠️ ON NE DIT « PAYÉ EN LIGNE » QUE S'IL Y A EU UN PAIEMENT. Sinon
            // on dit ce qui est vrai : les avantages ont tout couvert.
            l.push(enLigne > 0
              ? `<tr><td style="padding:10px 14px;color:${C.muted};">Payé en ligne</td><td style="padding:10px 14px;color:#10B981;font-weight:900;text-align:right;">✓ ${euros(enLigne)}</td></tr>`
              : `<tr><td style="padding:10px 14px;color:${C.muted};">À payer</td><td style="padding:10px 14px;color:#10B981;font-weight:900;text-align:right;">Rien, c’est déjà couvert</td></tr>`)
            return l.join('')
          })()}
        </table>
      </div>
      <p style="margin:0 0 14px;font-size:12px;color:${C.deep};line-height:1.55;">
        Tes produits sont mis de côté. Tu les récupères en même temps que ${mots.laSienne}, rien à repayer sur place.
      </p>` : ''}
      <p style="margin:0 0 14px;font-size:13px;line-height:1.6;color:${C.ink};">
        ${deplace
          // 🔴 « MÊME TABLE, MÊME PRIX » SUR UNE TABLE (email d'Alex, 10/09 tard) :
          // une table n'a pas de prix, et la salle peut l'avoir changée pour une
          // autre à la nouvelle heure. Ce qui ne bouge JAMAIS pour elle, c'est le
          // nombre de personnes. Ailleurs, « même prix » seulement s'il y en a un.
          ? `Rien d'autre ne change : ${table
              ? 'même nombre de personnes, même référence'
              : `même ${mots.prestation}${prix_estime != null ? ', même prix' : ''}, même référence`} 🟣`
          : `Tout est prêt 🟣 Tu peux ajouter ${mots.laReservationDe} à ton calendrier dès maintenant.`}
      </p>
      <div style="background:#fff;border-radius:12px;padding:14px 16px;border:1.5px dashed ${C.main}44;margin-bottom:6px;">
        ...
        <p style="margin:0;font-size:12px;color:${C.deep};line-height:1.55;">Un fichier <strong>.ics</strong> est joint à cet email. ${deplace ? `Ouvre-le pour <strong>mettre à jour</strong> ${mots.laReservationDe} déjà ${mots.participePresent} dans ton calendrier (Apple, Google, Outlook) : ${mots.pronomSujet} se déplace tout seul, tu n'auras pas deux ${mots.nomPluriel}.` : `Ouvre-le pour ajouter ${mots.laReservationDe} à ton calendrier (Apple, Google, Outlook).`}${rappel_24h ? ' Rappel automatique 24h avant.' : ''}</p>
      </div>
      ${infos_pratiques ? `
      ...
      <div style="background:#FFFBEB;border-left:4px solid #F59E0B;border-radius:10px;padding:12px 14px;margin-top:14px;">
        ...
        <p style="margin:0 0 8px;font-size:12px;color:#7C2D12;line-height:1.5;">
          Tu peux annuler ${mots.laSienne} gratuitement jusqu'à <strong>${delai_annulation_heures}h</strong> avant ton créneau. ${
            // ⚠️ CETTE PHRASE NE PARLAIT QUE DE L'ACOMPTE (Alex, 30/08). Sur ce
            // rendez-vous-là, 50 € venaient d'un bon cadeau et d'une récompense :
            // le client lisait « remboursement de l'acompte » et pouvait croire
            // que le reste était perdu s'il annulait. Les deux reviennent, il
            // faut le dire ICI, au moment où la question se pose.
            //
            // 🔴 ET ELLE PROMETTAIT UN REMBOURSEMENT QUI N'EXISTE PAS (Alex,
            // 16/09, sur sa table garantie). « Remboursement automatique de
            // l'acompte en 5 à 10 jours » s'affichait SANS CONDITION, donc sur
            // une table qui n'a jamais rien payé : ni acompte, ni débit, rien à
            // rendre. Un client qui attend un virement qui ne viendra pas
            // rappelle son restaurant, puis conteste. Chaque bout ne s'écrit
            // désormais que s'il est VRAI.
            [
              aAcompte ? 'Ton acompte revient sur ton moyen de paiement en 5 à 10 jours.' : null,
              (Number(fidelite_remise) > 0 || Number(bon_cadeau_montant) > 0)
                ? `${Number(nb_bons) > 1 ? 'Tes' : 'Ton'} ${libelleBon(commercant_categorie, { pluriel: Number(nb_bons) > 1 })} comme ta récompense retournent chez toi, utilisables tout de suite.`
                : null,
              // ⚠️ LE MONTANT GARANTI SE REDIT ICI, au moment exact où la
              // question se pose, et le délai n'est PAS répété : il est dans la
              // phrase juste au-dessus.
              garantie > 0
                ? `Rien n'est débité si tu viens : ${echapperHtml(commercant_nom)} ne peut facturer ${euros(garantie)} que si personne ne se présente, ou si tu annules plus tard.`
                : null,
            ].filter(Boolean).join(' ')
          }
        </p>
      ...
      <p style="margin:14px 0 0;font-size:11px;color:${C.muted};line-height:1.55;">
        ⏰ Annulation possible jusqu'à ${delai_annulation_heures}h avant ${mots.laReservationDe} depuis ton espace Yopper.
      </p>`}
    `,
    ctaUrl: mesRdvUrl,
    ctaLabel: mots.ctaVoir,
  })
}

// 5) Email AU YOPPER : RDV annulé (iCal CANCEL joint)
// ─── « CONFIRME TA TABLE » : LE LIEN D'EMPREINTE (lot 4, 14/09) ─────────────
//
// Envoyé au client d'une réservation prise AU TÉLÉPHONE, où il n'y avait pas de
// carte au bout du fil.
//
// 🔴 CE QUE CET EMAIL DOIT FAIRE COMPRENDRE EN UNE LIGNE : rien ne sera débité
// s'il vient. Un client qui croit qu'on lui prend de l'argent ne clique pas, et
// il a raison de ne pas cliquer. On n'écrit JAMAIS qu'une somme est bloquée ou
// retenue sur son compte : avec un enregistrement de carte, rien ne l'est.
//
// ⚠️ SA TABLE EST DÉJÀ RÉSERVÉE, et l'email le dit. Laisser croire qu'elle
// dépend de ce clic serait faux, et ferait passer Yoppaa pour un service qui
// prend des tables en otage.
export function emailDemandeEmpreinte({ prenom, commercantNom, quand, couverts, montant, lien, expireLe }) {
  const nom = echapperHtml(commercantNom || '')
  const quandHtml = echapperHtml(quand || '')
  const jusquA = expireLe instanceof Date
    ? expireLe.toLocaleString('fr-BE', { timeZone: 'Europe/Brussels', dateStyle: 'full', timeStyle: 'short' })
    : ''
  return layout({
    title: 'Confirme ta table',
    intro: `${prenom ? `${echapperHtml(prenom)}, ta` : 'Ta'} table chez <strong>${nom}</strong> est bien réservée pour le ${quandHtml}${couverts > 1 ? `, ${couverts} personnes` : ''}.`,
    body: `
      <p style="margin:0 0 14px;font-size:15px;line-height:1.55;color:${C.ink};">
        Pour une table de cette taille, ${nom} demande d&rsquo;enregistrer ta carte.
        <strong>Rien n&rsquo;est débité si tu viens.</strong>
      </p>
      <div style="background:${C.bg};border:1px solid ${C.pale};border-radius:12px;padding:14px 16px;margin:0 0 14px;">
        <p style="margin:0;font-size:14px;line-height:1.6;color:${C.ink};">
          En cas d&rsquo;absence, ou d&rsquo;annulation trop tardive pour que ta table soit reprise,
          ${nom} peut facturer <strong>${echapperHtml(montant || '')}</strong>.
        </p>
      </div>
      <p style="margin:0;font-size:13px;line-height:1.55;color:${C.muted};">
        Ta table reste réservée même sans ce geste : il sert à la garantir.
        ${jusquA ? `Ce lien fonctionne jusqu&rsquo;au ${echapperHtml(jusquA)}.` : ''}
      </p>`,
    ctaUrl: lien,
    ctaLabel: 'Enregistrer ma carte',
    audience: 'yopper',
    commercantNom: commercantNom || '',
  })
}
...
export function emailRdvAnnule({
  ...
  // ⚠️ CE QUI REVIENT VRAIMENT, ET IL Y EN A TROIS SORTES (29/08, vu par Alex
  // sur un vrai email reçu). L'email ne parlait QUE de l'acompte : sur un
  // rendez-vous payé par bon cadeau, l'acompte vaut zéro, donc le bloc entier
  // disparaissait et le client lisait une annulation SANS UN SEUL MONTANT.
  //   • `refund_montant` : ce que Stripe rend sur la carte (acompte + produits)
  //   • `bon_rendu`      : ce qui retourne sur le bon cadeau, invisible pour
  //                        Stripe, et que personne n'annonçait
  //   • `produits_gardes`: le client a choisi de garder sa marchandise
  //
  // 🔴 ET UNE QUATRIÈME, TROUVÉE PAR ALEX LE 30/08 SUR SON PROPRE PARCOURS :
  //   • `recompense_rendue` : la récompense de fidélité retourne bel et bien
  //     sur la carte, et RIEN NE LE DISAIT. Alex l'a vu parce qu'il est allé
  //     regarder sa carte ; un Yopper, lui, croit avoir perdu ses 10 €.
  //     Frère exact du bon cadeau, corrigé la veille et jamais porté à côté.
  refund_montant = null, bon_rendu = 0, recompense_rendue = 0,
  // 🔴 COMBIEN DE BONS ONT PAYÉ (01/09). Un rendez-vous peut en porter cinq :
  // « sur ton bon gourmand » devant trois bons est faux.
  nb_bons = 0,
  produits_gardes = false, produits_montant = 0,
  commercant_categorie = null,   // le nom du bon suit le métier (31/08)
  // 🔴 « TABLE : TABLE DE 6 PERSONNES » POUR UN GROUPE DE QUATRE (11/09). La
  // confirmation disait le groupe depuis la veille ; l'annulation de la même
  // table redisait le format. Mêmes paramètres que `emailRdvConfirme`.
  table = false,
  couverts = null,
  // 🔴 CE QUE LE CLIENT PEUT ENCORE PAYER (15/09). Une table garantie annulée
  // trop tard pour être reprise reste facturable par le restaurant. L'email
  // qui confirme l'annulation doit le dire : le client qui découvrirait le
  // débit sans l'avoir lu nulle part contesterait, et il aurait raison.
  tardive_facturable = 0,
  delai_heures = null,
}) {
  const ficheUrl = lienFicheRdv(commercant_slug) || LIEN_ACCUEIL
  ...
  const mots = motsReservation({ categorie: commercant_categorie })
  const A = mots.participeAnnule
  const raisonLabel = lieuChange
    ? `Le lieu a changé chez <strong>${echapperHtml(commercant_nom)}</strong>`
    : raison_annulation === 'commercant'
      ? `${A.charAt(0).toUpperCase()}${A.slice(1)} par <strong>${echapperHtml(commercant_nom)}</strong>`
      : raison_annulation === 'auto'
        ? `${A.charAt(0).toUpperCase()}${A.slice(1)} automatiquement (paiement non finalisé)`
        : `${A.charAt(0).toUpperCase()}${A.slice(1)} à ta demande`
  return layout({
    title: lieuChange ? mots.emailLieuChange : mots.emailAnnule,
    intro: lieuChange
      ? `<strong>${echapperHtml(yopper_prenom)}</strong>, ${mots.laSienne} du <strong>${formatDateFr(date_rdv)} à ${heure_debut?.slice(0,5) || '?'}</strong> chez <strong>${echapperHtml(commercant_nom)}</strong> ne peut pas être ${mots.participeMaintenu} : l’endroit a changé. Reprends ta place en un clic, tu retrouveras les horaires à la nouvelle adresse.`
      : `<strong>${echapperHtml(yopper_prenom)}</strong>, ${mots.laSienne} du <strong>${formatDateFr(date_rdv)} à ${heure_debut?.slice(0,5) || '?'}</strong> chez <strong>${echapperHtml(commercant_nom)}</strong> est ${A}.`,
    body: `
      <div style="background:#FEF2F2;border-left:4px solid #DC2626;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
        ...
        <p style="margin:6px 0 0;font-size:12px;color:#991B1B;">${mots.prestationLigne} : ${echapperHtml(objetReservation({ prestation_nom, table, couverts })) || '—'}</p>
      </div>
      ${(() => {
        // Le montant qui revient sur la carte : celui que la route a calculé
        // quand elle le connaît, l'acompte sinon (annulation par le commerçant,
        // où rien n'est encore remboursé automatiquement).
        const surCarte = Number(refund_montant != null ? refund_montant : (acompte_paye ? acompte_montant : 0)) || 0
        const surBon = Number(bon_rendu) || 0
        const surCarteFid = Number(recompense_rendue) || 0
        const desProduits = Number(produits_montant) || 0
        if (!surCarte && !surBon && !surCarteFid && !produits_gardes) return ''
        const lignes = []
        if (surCarte > 0) {
          lignes.push(refund_en_cours
            ? `<strong>${euros(surCarte)}</strong> reviennent sur ton moyen de paiement sous 5 à 10 jours ouvrés.`
            : `Montant payé en ligne : <strong>${euros(surCarte)}</strong>. Contacte le commerçant si tu as une question.`)
        }
        // ⚠️ LA PART BON SE DIT À PART. Stripe ne la rembourse pas et ne peut
        // pas la rembourser : elle retourne sur le bon, où elle est utilisable
        // tout de suite. Sans cette ligne, le client croit l'avoir perdue.
        if (surBon > 0) {
          const plusieurs = Number(nb_bons) > 1
          lignes.push(`<strong>${euros(surBon)}</strong> sont recrédités sur ${plusieurs ? 'tes' : 'ton'} ${libelleBon(commercant_categorie, { pluriel: plusieurs })}, utilisables dès maintenant.`)
        }
        // 🔴 ET LA RÉCOMPENSE, POUR EXACTEMENT LA MÊME RAISON. Elle retourne sur
        // la carte de fidélité, où Stripe ne peut évidemment rien voir. Sans
        // cette ligne, le client lit « 5,00 € reviennent » et croit avoir perdu
        // les 10 € qu'il avait mis. Il faut ouvrir sa carte pour découvrir
        // qu'ils y sont, et personne ne va vérifier ce qu'on ne lui annonce pas.
        if (surCarteFid > 0) {
          lignes.push(`Ta récompense fidélité de <strong>${euros(surCarteFid)}</strong> retourne sur ta carte, utilisable à ton prochain passage.`)
        }
        if (produits_gardes) {
          lignes.push(desProduits > 0
            ? `Tes produits (<strong>${euros(desProduits)}</strong>) restent à toi : ils t'attendent en boutique.`
            : 'Tes produits restent à toi : ils t\'attendent en boutique.')
        } else if (desProduits > 0) {
          // 🔴 LES PRODUITS SE DISENT AUSSI QUAND ILS SONT RENDUS (Alex, 30/08
          // au soir). Le montant du remboursement les contient, mais le client
          // lit « 26,90 € reviennent » sans savoir que son shampoing en fait
          // partie : il attend un sachet qui ne sera pas préparé, et personne
          // ne lui dit qu'il peut le recommander. Un montant n'est pas une
          // information tant qu'on ne dit pas de quoi il est fait.
          lignes.push(`Tes produits (<strong>${euros(desProduits)}</strong>) sont remboursés eux aussi : ils ne seront pas mis de côté. Tu peux les recommander quand tu veux.`)
        }
        // ⚠️ LA RÉCOMPENSE COMPTE ICI AUSSI. Un rendez-vous entièrement payé par
        // la fidélité n'a rien à rembourser sur la carte bancaire : le bloc
        // serait passé en ORANGE « Remboursement à voir » alors que tout est
        // déjà revenu. La couleur dit l'état, elle ne doit pas mentir non plus.
        const vert = refund_en_cours || surBon > 0 || surCarteFid > 0
        return `
      <div style="background:${vert ? '#ECFDF5' : '#FFF7ED'};border-left:4px solid ${vert ? '#10B981' : '#EA580C'};border-radius:10px;padding:14px 16px;margin-bottom:14px;">
        <p style="margin:0 0 6px;font-size:11px;font-weight:800;color:${vert ? '#065F46' : '#9A3412'};text-transform:uppercase;letter-spacing:0.7px;">${vert ? 'Ce qui te revient' : 'Remboursement à voir'}</p>
        ${lignes.map(l => `<p style="margin:0 0 4px;font-size:13px;color:${vert ? '#065F46' : '#7C2D12'};line-height:1.55;font-weight:600;">${l}</p>`).join('')}
      </div>`
      })()}
      ${Number(tardive_facturable) > 0 ? `
      <div style="background:#FFF7ED;border-left:4px solid #EA580C;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
        <p style="margin:0 0 6px;font-size:11px;font-weight:800;color:#9A3412;text-transform:uppercase;letter-spacing:0.7px;">Annulation tardive</p>
        <p style="margin:0;font-size:13px;color:#7C2D12;line-height:1.55;font-weight:600;">Tu as annulé moins de ${Number(delai_heures) || 0} h avant : ${echapperHtml(commercant_nom)} peut facturer <strong>${euros(Number(tardive_facturable))}</strong>, le montant de la garantie que tu avais acceptée.</p>
      </div>` : ''}
      <p style="margin:0 0 4px;font-size:13px;color:${C.ink};line-height:1.6;">
        ${mots.laReservationDe.charAt(0).toUpperCase()}${mots.laReservationDe.slice(1)} a été automatiquement ${mots.participeRetire} de ton calendrier (un fichier .ics d'annulation est joint à cet email).
      </p>
    `,
    ctaUrl: ficheUrl,
    ctaLabel: lieuChange ? 'Reprendre ma place' : mots.ctaReprendre,
  })
}
...
// 🔴 ET LUI AUSSI NE PARLAIT QUE DE L'ACOMPTE (30/08 au soir, en cherchant les
// frères de la fenêtre d'annulation). Sur un rendez-vous où l'acompte vaut zéro
// parce qu'un bon cadeau de 40 € a payé la prestation, ce bloc disparaissait :
// le Yopper apprenait qu'il n'était pas venu, et rien de plus. Ses 40 € ne
// reviennent pourtant pas, et c'est justement ce qu'il faut lui dire pour qu'il
// puisse en parler au commerçant.
export function emailRdvNoShow({
  yopper_prenom, commercant_nom,
  prestation_nom, date_rdv, heure_debut,
  // 🔴 ET DEPUIS LE 30/08 AU SOIR, LE BON NE RESTE PLUS EN ENTIER CHEZ LE
  // COMMERÇANT (décision d'Alex) : la garantie ne porte que sur l'acompte dû,
  // ce qui dépasse revient au Yopper. L'email doit donc dire DEUX choses, et
  // c'est la nouvelle qui compte qui vient en dernier.
  // ⚠️ `bon_garde` EST CE QUI RESTE ACQUIS, PAS CE QUI AVAIT ÉTÉ POSÉ. Depuis
  // que le surplus revient au client, annoncer `bon_cadeau_montant` ferait
  // croire au Yopper qu'il perd 40 € alors qu'il n'en perd que 25.
  acompte_paye, acompte_montant, bon_garde = 0,
  bon_restitue = 0, recompense_rendue = 0,
  // 🔴 COMBIEN DE BONS ONT PAYÉ (01/09) : ces deux phrases parlent d'argent qui
  // reste et d'argent qui revient, et « ton bon » devant trois bons est faux
  // des deux côtés.
  nb_bons = 0,
  commercant_categorie = null,   // le nom du bon suit le métier (31/08)
  ...
  const plusieursBons = Number(nb_bons) > 1
  const motBons = libelleBon(commercant_categorie, { pluriel: plusieursBons })
  const mots = motsReservation({ categorie: commercant_categorie })
  return layout({
    audience: 'yopper',
    commercantNom: commercant_nom,
    title: mots.emailNoShow,
    intro: `<strong>${echapperHtml(yopper_prenom)}</strong>, ${mots.laReservationDe} du <strong>${formatDateFr(date_rdv)} à ${heure_debut?.slice(0,5) || '?'}</strong> chez <strong>${echapperHtml(commercant_nom)}</strong> a été ${mots.participeMarque} <strong style="color:#6B7280;">non ${mots.participeHonore}</strong>.`,
    body: `
      <div style="background:#F9FAFB;border-left:4px solid #6B7280;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:0.7px;">${mots.prestationLigne}</p>
        <p style="margin:0;font-size:14px;color:${C.ink};font-weight:600;">${echapperHtml(objetReservation({ prestation_nom, table, couverts })) || '—'}</p>
      </div>
      ...
      ${(() => {
        const surCarte = acompte_paye && Number(acompte_montant) > 0 ? Number(acompte_montant) : 0
        const surBon = Number(bon_garde) || 0
        if (!surCarte && !surBon) return ''
        const quoi = []
        if (surCarte > 0) quoi.push(`ton acompte de <strong>${euros(surCarte)}</strong>`)
        if (surBon > 0) quoi.push(`les <strong>${euros(surBon)}</strong> de ${plusieursBons ? 'tes' : 'ton'} ${motBons}`)
        return `
      <div style="background:#FFF7ED;border-left:4px solid #EA580C;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:#9A3412;text-transform:uppercase;letter-spacing:0.7px;">Ce qui reste chez le commerçant</p>
        <p style="margin:0;font-size:13px;color:#7C2D12;line-height:1.55;font-weight:600;">
          ${quoi.join(' et ')} ${quoi.length > 1 ? 'restent acquis' : 'reste acquis'} au commerçant : le créneau a été bloqué pour toi. Pour un geste commercial, contacte-le directement.
        </p>
      </div>`
      })()}
      ${(() => {
        // 🔴 ET CE QUI REVIENT QUAND MÊME. La garantie ne porte que sur
        // l'acompte dû : un bon cadeau qui dépassait cette somme n'était pas
        // une garantie, c'était une pénalité. Sans cette ligne, le Yopper croit
        // avoir tout perdu et ne va pas vérifier son bon.
        const surBon = Number(bon_restitue) || 0
        const surCarte = Number(recompense_rendue) || 0
        if (!surBon && !surCarte) return ''
        const lignes = []
        if (surBon > 0) {
          lignes.push(`<strong>${euros(surBon)}</strong> sont recrédités sur ${plusieursBons ? 'tes' : 'ton'} ${motBons}, utilisables dès maintenant : seule la garantie du commerçant reste acquise.`)
        }
        if (surCarte > 0) {
          lignes.push(`Ta récompense fidélité de <strong>${euros(surCarte)}</strong> retourne sur ta carte, utilisable à ton prochain passage.`)
        }
        return `
      <div style="background:#ECFDF5;border-left:4px solid #10B981;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
        <p style="margin:0 0 6px;font-size:11px;font-weight:800;color:#065F46;text-transform:uppercase;letter-spacing:0.7px;">Ce qui te revient quand même</p>
        ${lignes.map(l => `<p style="margin:0 0 4px;font-size:13px;color:#065F46;line-height:1.55;font-weight:600;">${l}</p>`).join('')}
      </div>`
      })()}
...
export function emailRdvReminder({
  ...
  // ⚠️ CETTE COLONNE MANQUAIT ICI, et le rappel de la veille est justement
  // l'email qu'un client relit avant de venir dîner.
  commercant_categorie = null,
  // 🔴 ET IL LUI REDISAIT LE FORMAT (11/09) : « Table de 6 personnes » sous
  // l'heure, pour un groupe de quatre que la salle avait assis plus grand.
  table = false,
  couverts = null,
}) {
  const ficheUrl = lienFicheRdv(commercant_slug) || LIEN_ACCUEIL
  ...
  const mots = motsReservation({ categorie: commercant_categorie })
  return layout({
    title: mots.emailRappelDemain,
    intro: `<strong>${echapperHtml(yopper_prenom)}</strong>, petit rappel : tu as ${mots.uneSienne} chez <strong>${echapperHtml(commercant_nom)}</strong> demain ${formatDateFr(date_rdv).split(' ').slice(0,3).join(' ')} à <strong>${heure_debut?.slice(0,5) || '?'}</strong> 🟣`,
    body: `
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;">
        ...
        <p style="margin:6px 0 0;font-size:13px;color:${C.deep};font-weight:700;">${echapperHtml(intituleReservation({ prestation_nom, table, couverts })) || '—'}</p>
...
export function emailFideliteRecompenseDebloquee({
  ...
  return layout({
    ...
    body: `
      <div style="background-color:#059669;background-image:linear-gradient(135deg,#10B981 0%,#059669 100%);border-radius:14px;padding:24px;margin-bottom:14px;text-align:center;color:#fff;">
...
export function emailNouveauRdvCommercant({
  ...
  prestation_nom, date_rdv, heure_debut, heure_fin, duree_minutes,
  prix_estime, acompte_paye, acompte_montant, fidelite_remise = 0, bon_cadeau_montant = 0, notes_client,
  ...
  produits = null,
  commercant_categorie = null,   // le nom du bon suit le métier (31/08)
  // 🔴 LE RESTAURATEUR NE SAVAIT PAS COMBIEN ILS VENAIENT (10/09 tard) : l'email
  // disait « Table de 6 personnes », le format assis, et jamais le groupe. Il
  // lit maintenant les deux, le groupe d'abord.
  table = false,
  couverts = null,
}) {
  const dashboardUrl = 'https://www.yoppaa.app/dashboard'
  const mots = motsReservation({ categorie: commercant_categorie })
  const objet = objetReservation({ prestation_nom, table, couverts })
  const quoi = objet && objet !== prestation_nom && prestation_nom ? `${objet} · ${prestation_nom}` : (prestation_nom || objet)
  return layout({
    audience: 'commercant',
    title: mots.alerteNouvelleTitre,
    intro: `<strong>${echapperHtml(nom_commercant)}</strong>, un client vient de réserver ${mots.uneSienne} ! 🟣`,
    body: `
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;">
        <p style="margin:0;font-size:16px;font-weight:900;color:${C.ink};letter-spacing:-0.3px;">${formatDateFr(date_rdv)}</p>
        ...
        <p style="margin:8px 0 0;font-size:13px;color:${C.main};font-weight:800;">${echapperHtml(quoi) || '—'}</p>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:13px;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
...
export function blocPaiementYopper(commande, commercant_categorie = null) {
  ...
  if (rienDeSu) return ''
  const etat = etatPaiementClient(commande, { categorie: commercant_categorie })
  if (!etat) return ''
  ...
  return articles.map(a => `
    <tr>
      <td style="padding:8px 14px;color:${C.ink};font-size:13px;border-bottom:1px solid ${C.pale};">
        ...
        ${a.option_libelle ? `<br/><span style="color:${C.muted};font-size:11px;">${echapperHtml(a.option_libelle)}</span>` : ''}
      </td>
  ...
  return layout({
    ...
    body: `
      ...
      <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ...
        ${bonEUR > 0 ? `
        <tr><td style="padding:10px 14px;color:#10B981;font-size:13px;font-weight:700;">🎁 ${libelleBon(commercant_categorie, { majuscule: true })}</td><td style="padding:10px 14px;color:#10B981;font-size:13px;font-weight:800;text-align:right;">−${euros(bonEUR)}</td></tr>` : ''}
        ...
        <tr><td colspan="2" style="padding:10px 14px 4px;color:${C.muted};font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:0.6px;border-top:1px solid ${C.pale};">${
          // ⚠️ « DONT TVA » TOUT SEUL SE LIT COMME « TVA SUR LA LIGNE DU DESSUS »
          // (Alex, 28/08, capture entourée). Sur une commande couverte par un
          // bon cadeau, la ligne du dessus dit « Plus rien à payer 0,00 € » et
          // la TVA de 1,39 € juste en dessous semble sortie de nulle part.
          //
          // Elle est pourtant JUSTE : le bon cadeau est un MOYEN DE PAIEMENT,
          // la vente vaut toujours 8,00 € et la TVA est due dessus. C'est donc
          // la phrase qu'on corrige, pas le chiffre — le faire disparaître
          // ferait s'évaporer une taxe réellement due.
          //
          // On rattache donc la TVA À SON ASSIETTE, en toutes lettres.
          // ⚠️ ET L'ASSIETTE SE LIT DANS LA VENTILATION, JAMAIS DANS `total` :
          // avec une RÉCOMPENSE, la base a été réduite (règle du 28/08), et
          // annoncer « le total » serait faux d'exactement le montant de la
          // remise. Avec un BON CADEAU elle vaut le total, et c'est voulu.
          // En la calculant depuis les lignes affichées juste en dessous, la
          // phrase ne peut pas diverger de ce qu'elle chapeaute.
          `TVA comprise dans les ${euros(ventilation_tva.reduce((s, v) => s + Number(v.ttc || 0), 0))}`
        }</td></tr>
      ...
      ${blocPaiementYopper(paiement, commercant_categorie)}
...
export function emailRappelRetrait({
  ...
  return layout({
    ...
    body: `
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;text-align:center;">
...
export function emailCommandeAnnuleeYopper({
  ...
  // 🔴 COMBIEN DE BONS ONT PAYÉ. Une commande peut en porter cinq depuis le
  // 01/09, et « ton bon gourmand » au singulier devant trois bons est faux.
  nb_bons = 0,
  ...
  if (bonOk) bouts.push(`<strong>${euros(bonMnt)}</strong> avec ${plusieurs ? 'tes' : 'ton'} ${motBon}`)

  // ═══════════════════════════════════════════════════════════════════════════
  // 🔴 CE QUI REVIENT, ET IL Y EN A DEUX SORTES (Alex, 01/09)
  // ═══════════════════════════════════════════════════════════════════════════
  //
  // Sur sa commande #CC2, cet email annonçait « ta récompense de 5,00 € t'est
  // rendue » et **ne disait pas un mot des 145 €** revenus sur ses trois bons.
  // L'argent était bel et bien recrédité : c'est l'email qui se taisait.
  //
  // ⚠️ ET C'ÉTAIT LE FRÈRE NON TRAITÉ. L'email d'annulation d'un RENDEZ-VOUS
  // dit cette phrase depuis le 29/08 (« X € sont recrédités sur ton bon »).
  // Celui de la commande ne l'a jamais reçue : deux gabarits pour une même
  // idée finissent toujours par en dire deux choses différentes.
  //
  // 🔴 LE BON PASSE EN PREMIER, et c'est délibéré : c'est le plus gros montant,
  // et le seul que Stripe ne rembourse pas. Un Yopper qui lit « remboursement
  // lancé » sans voir ses 145 € croit les avoir perdus et appelle le commerce.
  const retours = []
  if (bonOk) {
    retours.push(`<strong>${euros(bonMnt)}</strong> ${plusieurs ? 'sont recrédités sur tes' : 'sont recrédités sur ton'} ${motBon}, utilisables dès maintenant.`)
  }
  // ⚠️ UNE RÉCOMPENSE SE REND EN RÉCOMPENSE, PAS EN ARGENT : elle retourne sur
  // la carte, où Stripe ne peut évidemment rien voir.
  if (recompenseOk) {
    retours.push(`Ta récompense de <strong>${euros(remise)}</strong> t’est rendue : tu la retrouves dans ta carte de fidélité.`)
  }
  // 🔴 LE BLOC REMBOURSEMENT NE S'AFFICHE QUE SI LA CARTE A VRAIMENT PAYÉ
  // (Alex, 01/09). Sur sa commande #RE4, entièrement couverte par ses bons, il
  // promettait « le montant revient sur ton moyen de paiement dans 5 à 10
  // jours » alors que Stripe n'avait rien encaissé.
  const part = partCartePayee({ total, fidelite_remise, bon_cadeau_montant, paye_en_ligne })
  const surCarte = part === null ? paye_en_ligne : part > 0
  return layout({
    audience: 'yopper',
    commercantNom: commercant_nom,
    ...
    body: `
      ...
      ${retours.length ? `<div style="background:${C.bg};border-left:4px solid ${C.main};border-radius:10px;padding:12px 14px;margin-bottom:14px;">
        <p style="margin:0 0 6px;font-size:11px;font-weight:800;color:${C.main};text-transform:uppercase;letter-spacing:0.7px;">Ce qui te revient</p>
        ${retours.map(l => `<p style="margin:0 0 4px;font-size:13px;color:${C.deep};line-height:1.55;font-weight:600;">${l}</p>`).join('')}
      </div>` : ''}
      ${surCarte ? `
...
export function emailCommandeAnnuleeCommercant({
  ...
  fidelite_remise = null, bon_cadeau_montant = null, nb_bons = 0,
  refund_manuel = false, paye_en_ligne = true,
  commercant_categorie = null,   // le nom du bon suit le métier (31/08)
}) {
  const dashboardUrl = 'https://www.yoppaa.app/dashboard'
  ...
  const avantages = ligneAvantagesEmail({ fidelite_remise, bon_cadeau_montant, nb_bons, commercant_categorie })
  // 🔴 ET LE FRÈRE EST PIRE QUE L'ORIGINAL. Sur une commande couverte par des
  // bons, ce bloc disait au commerçant « rembourse manuellement depuis ton
  // Stripe Dashboard » — d'une transaction que Stripe n'a jamais vue. Il
  // chercherait un paiement inexistant, puis appellerait.
  const partPro = partCartePayee({ total, fidelite_remise, bon_cadeau_montant, paye_en_ligne })
  const surCartePro = partPro === null ? paye_en_ligne : partPro > 0
  return layout({
    audience: 'commercant',
    title: 'Une commande a été annulée',
    intro: `<strong>${echapperHtml(nom_commercant)}</strong>, ${echapperHtml(yopper_prenom) || 'un client'} vient d'annuler sa commande.`,
    body: `
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;">
...
export function emailCommandePrete({
  ...
  const ficheUrl = lienFiche(commercant_slug) || LIEN_ACCUEIL
  const mapsUrl = commercant_adresse ? `https://www.google.com/maps/search/${encodeURIComponent(commercant_adresse)}` : null
  ...
  if (est_expedition) {
    return layout({
      ...
      body: `
        <div style="background-color:#059669;background-image:linear-gradient(135deg,#10B981 0%,#059669 100%);border-radius:14px;padding:20px;margin-bottom:14px;text-align:center;color:#fff;">
...
// ⚠️ PAS DE `commercant_categorie` ICI, ET C'EST VOLONTAIRE : cet email ne
// nomme jamais le bon (il annonce une tournée en cours). Un paramètre reçu et
// jamais lu donnerait l'air câblé sans l'être, ce qui est pire que rien.
export function emailCommandeEnLivraison({
  ...
  return layout({
    ...
    body: `
      <div style="background-color:${C.deep};background-image:linear-gradient(135deg,${C.main} 0%,${C.deep} 100%);border-radius:14px;padding:20px;margin-bottom:14px;text-align:center;color:#fff;">
...
// ⚠️ NI ICI, même raison : un colis parti ne parle pas du bon.
...
export function emailNouvelleCommandeCommercant({
  ...
  return layout({
    ...
    intro: `<strong>${echapperHtml(nom_commercant)}</strong>, un client vient de passer commande ! 🛒`,
    body: `
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;">
...
// 🔴 ET LES TABLES DU JOUR DANS LE MÊME EMAIL (arbitrage d'Alex, 09/09 au soir).
//
// Le récapitulatif du matin était aiguillé par la CATÉGORIE : les commandes pour
// un alimentaire, les rendez-vous pour une vitrine. Un restaurant est
// alimentaire ET prend des réservations : il recevait donc « Aucune commande
// aujourd'hui » un matin où trente couverts l'attendaient le soir.
//
// ⚠️ UN SEUL EMAIL, DEUX SECTIONS, et c'est la décision d'Alex : « il ouvre ça
// entre deux services, pas deux fois ».
//
// ⚠️ ET ON COMPTE LES COUVERTS, PAS LES LIGNES. Huit réservations ne disent
// rien à un restaurateur ; vingt-six couverts lui disent s'il sort une personne
// en plus en cuisine. C'est toute la différence avec un carnet de rendez-vous.
export function emailRecapCommandesJour({
  ...
  const blocBons = bons.length === 0 ? '' : `
      <div style="background:${C.pale};border-radius:12px;padding:12px 14px;margin-top:14px;">
        <p style="margin:0 0 2px;font-size:11px;font-weight:800;color:${C.main};text-transform:uppercase;letter-spacing:0.6px;">
          🎁 ${bons.length > 1
            ? `${bons.length} ${libelleBon(commercant_categorie, { pluriel: true })} vendus`
            : `${libelleBon(commercant_categorie, { majuscule: true })} vendu`}
        </p>
  ...
  // ─── La salle, quand ce commerce en a une ────────────────────────────────
  //
  // ⚠️ `null` ET `[]` NE DISENT PAS LA MÊME CHOSE, et les confondre ferait
  // afficher « Aucune table » à une boulangerie qui n'en a jamais eu. `null` =
  // ce commerce ne prend pas de réservations, la section n'existe pas ; `[]` =
  // il en prend et il n'y en a aucune aujourd'hui, ce qui est une information.
  const prendDesTables = Array.isArray(rdvs)
  const tables = prendDesTables ? rdvs : []
  const couverts = tables.reduce((s, r) => s + (Number(r?.couverts) > 0 ? Number(r.couverts) : 1), 0)
  const motsSalle = motsReservation({ categorie: commercant_categorie })
  const lignesTables = tables.map(r => `
    <tr>
      ...
      <td style="padding:10px 14px;color:${C.ink};font-weight:700;font-size:13px;border-bottom:1px solid ${C.pale};">
        ...
        <br/><span style="color:${C.muted};font-weight:500;font-size:11px;">${echapperHtml(r.prestation_nom) || '—'}</span>
      </td>
      <td style="padding:10px 14px;color:${C.main};font-weight:900;font-size:13px;text-align:right;border-bottom:1px solid ${C.pale};white-space:nowrap;">${Number(r?.couverts) > 0 ? `${r.couverts} couv.` : ''}</td>
    </tr>
  `).join('')
  const blocTables = !prendDesTables ? '' : `
      <p style="margin:18px 0 8px;font-size:11px;font-weight:800;color:${C.main};text-transform:uppercase;letter-spacing:0.6px;">
        🍽️ ${tables.length === 0 ? motsSalle.recapLesTiennes : `${couverts} couvert${couverts > 1 ? 's' : ''} en ${tables.length} ${tables.length > 1 ? motsSalle.recapPluriel : motsSalle.recapSingulier}`}
      </p>
      ${tables.length === 0
        ? `<p style="margin:0;font-size:13px;color:${C.muted};line-height:1.6;">${motsSalle.recapAucun}</p>`
        : `<table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;">${lignesTables}</table>`}`

  // Le titre annonce ce qui attend vraiment : deux natures, deux comptes.
  const titreCommandes = `${total} commande${total !== 1 ? 's' : ''}`
  const titreTables = prendDesTables && tables.length > 0
    ? ` et ${couverts} couvert${couverts > 1 ? 's' : ''}`
    : ''
  return layout({
    audience: 'commercant',
    title: `${titreCommandes}${titreTables} ${dateLabel.split(' ').slice(1,3).join(' ')}`,
    intro: `Bonjour <strong>${echapperHtml(nom_commercant)}</strong>, voici ${prendDesTables ? 'ta journée' : 'tes commandes'} pour <strong>${dateLabel}</strong> 🟣`,
    body: (total === 0
      ? `<p style="margin:0;font-size:14px;color:${C.muted};line-height:1.6;text-align:center;padding:${prendDesTables ? '10px' : '24px'} 0;">📭 Aucune commande aujourd'hui.${prendDesTables ? '' : ' Bonne journée !'}</p>`
      : `
      <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ${rows}
      </table>
...
export function emailKitBienvenue({ nom_commercant, slug, avant_lancement = false }) {
  ...
  return layout({
    ...
    intro: avant_lancement
      ? `<strong>${echapperHtml(nom_commercant)}</strong>, c'est validé : ta page Yoppaa est prête. Elle s'ouvre au public le ${ouverture}, et d'ici là ton kit sert à préparer le terrain.`
      : `<strong>${echapperHtml(nom_commercant)}</strong>, ta page est en ligne. Voici ton kit de démarrage : tout est prêt, il n'y a plus qu'à le partager.`,
    body: `
      ${avant_lancement ? `
      ...
      <div style="background-color:${C.pale};background-image:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:16px;">
...
function blocBonCadeau({ code, montant, commercant_nom, expires_at, commercant_categorie = null }) {
  const nom = libelleBon(commercant_categorie)
  return `
    <div style="background-color:#160636;background-image:linear-gradient(135deg,#160636 0%,#2D0F6B 100%);border-radius:16px;padding:22px 20px;margin-bottom:14px;text-align:center;">
      <p style="margin:0 0 2px;font-size:11px;font-weight:800;color:${C.light};text-transform:uppercase;letter-spacing:1px;">${libelleBon(commercant_categorie, { majuscule: true })} · ${echapperHtml(commercant_nom)}</p>
    ...
    <p style="margin:0 0 14px;font-size:12px;color:${C.muted};line-height:1.6;">
      Ce bon s'utilise <strong>en une ou plusieurs fois</strong>, directement chez <strong>${echapperHtml(commercant_nom)}</strong> (montre ton code au comptoir) ou lors d'une commande en ligne sur Yoppaa (champ « J'ai un ${nom} » au moment de payer). Le solde restant reste sur le bon.
    </p>`
}
...
export function emailBonCadeauBeneficiaire({
  ...
  return layout({
    ...
    intro: `<strong>${echapperHtml(beneficiaire_prenom) || 'Hello'}</strong>, ${acheteur_prenom ? `<strong>${echapperHtml(acheteur_prenom)}</strong> t'offre` : 'on t\'offre'} un ${nom} chez <strong>${echapperHtml(commercant_nom)}</strong> !`,
    body: `
      ${message ? `
      <div style="background:${C.pale};border-radius:12px;padding:12px 14px;margin-bottom:14px;">
        <p style="margin:0 0 2px;font-size:10px;font-weight:800;color:${C.main};text-transform:uppercase;letter-spacing:0.6px;">${acheteur_prenom ? `Le mot ${elisionDe(echapperHtml(acheteur_prenom))}` : 'Petit mot'}</p>
        <p style="margin:0;font-size:13px;color:${C.deep};line-height:1.55;font-style:italic;">« ${echapperHtml(message)} »</p>
      </div>` : ''}
      ${blocBonCadeau({ code, montant, commercant_nom, expires_at, commercant_categorie })}
      <p style="margin:0;font-size:12px;color:${C.muted};line-height:1.55;">
...
export function emailBonCadeauAcheteur({
  ...
  return layout({
    audience: 'yopper',
    commercantNom: commercant_nom,
    title: pour_moi ? `Ton ${nom} est prêt` : 'Ton cadeau est envoyé 🟣',
    intro: pour_moi
      ? `<strong>${echapperHtml(acheteur_prenom) || 'Merci'}</strong>, ton ${nom} chez <strong>${echapperHtml(commercant_nom)}</strong> est activé !`
      : `<strong>${echapperHtml(acheteur_prenom) || 'Merci'}</strong>, ton ${nom} chez <strong>${echapperHtml(commercant_nom)}</strong> vient d'être envoyé à <strong>${echapperHtml(beneficiaire_prenom || beneficiaire_email)}</strong> 🟣`,
    body: pour_moi
      ? blocBonCadeau({ code, montant, commercant_nom, expires_at, commercant_categorie })
      : `
      <div style="background:#fff;border-radius:12px;padding:16px;border:1px solid ${C.pale};margin-bottom:14px;text-align:center;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:${C.muted};text-transform:uppercase;letter-spacing:0.7px;">${libelleBon(commercant_categorie, { majuscule: true })} offert</p>
        <p style="margin:0;font-size:24px;font-weight:900;color:${C.ink};letter-spacing:-1px;">${euros(Number(montant))}</p>
        <p style="margin:6px 0 0;font-size:13px;color:${C.muted};">envoyé à <strong style="color:${C.ink};">${echapperHtml(beneficiaire_prenom) || ''} ${beneficiaire_email ? `(${echapperHtml(beneficiaire_email)})` : ''}</strong></p>
      </div>
    ...
    ctaLabel: pour_moi && token ? `Voir mon ${nom}` : 'Explorer les commerces',
  })
}

// 17) Email AU COMMERÇANT : un bon cadeau vient d'être vendu
export function emailBonCadeauVenduCommercant({ nom_commercant, montant, acheteur_email, pour_moi, commercant_categorie = null }) {
  const nom = libelleBon(commercant_categorie)
  return layout({
    audience: 'commercant',
    title: `${libelleBon(commercant_categorie, { majuscule: true })} vendu 🟣`,
    intro: `Bonne nouvelle <strong>${echapperHtml(nom_commercant)}</strong>, tu viens de vendre un ${nom} de <strong>${euros(Number(montant))}</strong> !`,
    body: `
      ...
      <p style="margin:0;font-size:12px;color:${C.muted};line-height:1.6;">
        Quand le client viendra l'utiliser, encode simplement son code dans ton dashboard → onglet ${libelleBon(commercant_categorie, { pluriel: true, majuscule: true })} (ou il l'appliquera lui-même en commandant en ligne).
      </p>
    `,
    ctaUrl: 'https://www.yoppaa.app/dashboard',
    ctaLabel: 'Ouvrir mon dashboard',
...
export function emailRecapRdvJour({
  ...
  // ⚠️ MANQUAIT ICI AUSSI : le récapitulatif du matin est le premier écran que
  // le restaurateur ouvre en arrivant, et il y lisait « tes RDV ».
  commercant_categorie = null,
}) {
  const dashboardUrl = 'https://www.yoppaa.app/dashboard'
  const mots = motsReservation({ categorie: commercant_categorie })
  ...
  const rdvsRows = (rdvs || []).map(r => `
    <tr>
      ...
      <td style="padding:10px 14px;color:${C.ink};font-weight:700;font-size:13px;border-bottom:1px solid ${C.pale};">
        ${echapperHtml(r.yopper_prenom) || ''} ${echapperHtml(r.yopper_nom) || ''}
        <br/><span style="color:${C.muted};font-weight:500;font-size:11px;">${echapperHtml(r.prestation_nom) || '—'} · ${r.duree_minutes || '?'} min</span>
      </td>
  ...
  return layout({
    audience: 'commercant',
    title: `${total} ${total > 1 ? mots.recapPluriel : mots.recapSingulier} ${dateLabel.split(' ').slice(1,3).join(' ')}`,
    intro: `Bonjour <strong>${echapperHtml(nom_commercant)}</strong>, voici ${mots.recapLesTiennes} pour <strong>${dateLabel}</strong> 🟣`,
    body: total === 0
      ? `<p style="margin:0;font-size:14px;color:${C.muted};line-height:1.6;text-align:center;padding:24px 0;">📭 ${mots.recapAucun} Bonne journée !</p>`
      : `
      <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ${rdvsRows}
      </table>
