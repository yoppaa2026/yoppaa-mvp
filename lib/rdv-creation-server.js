...
import { enModeInventaire, formatLibrePour, formatPourAffichage } from './inventaire-salle'
...
export async function creerReservationRdv(db, {
  ...
  if (estParCouverts(prestation)) {
    ...
    dureeRetenue = dureeSelonCouverts(prestation, couvertsRetenus)
    ...
    if (enModeInventaire(formatsTable)) {
      const reference = formatPourAffichage(formatsTable, couvertsRetenus)
      if (reference) dureeRetenue = dureeSelonCouverts(reference, couvertsRetenus)
    }
  }
