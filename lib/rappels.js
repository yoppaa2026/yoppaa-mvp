...
export async function programmerRappelCommande(commandeId, supabase) {
  try {
    ...
    const res = await envoyerPushParExternalId(clientId, {
      ...
      contents: `N’oublie pas : retire ta commande #${referenceCommande(cmd) || ''} ${chezLeCommerce(cmd.commercant?.nom || 'le commerçant')} (créneau ${creneauTxt}).`,
...
export async function programmerRappelRdv(rdvId, supabase) {
  try {
    ...
    // 🔴 UNE TABLE SE DIT EN PERSONNES (11/09) : « (6 personnes) », et plus le
    // format où la salle les a assis. Même règle que les emails.
    const objet = objetReservation({
      prestation_nom: rdv.prestation?.nom,
      table: rdv.prestation?.par_couverts === true,
      couverts: rdv.couverts,
    })
    const presta = objet ? ` (${objet})` : ''
    const motsR = motsReservation(rdv.commercant)
    const res = await envoyerPushParExternalId(clientId, {
      headings: `⏰ ${motsR.laSienne.charAt(0).toUpperCase()}${motsR.laSienne.slice(1)} dans 1h`,
      contents: `Rappel : ${motsR.nom} ${chezLeCommerce(rdv.commercant?.nom || 'ton commerçant')} à ${rdv.heure_debut.slice(0,5)}${presta}. À tout à l’heure !`,
...
/**
 * Replanifie le rappel d'un RDV que le commerçant vient de DÉPLACER.
 *
 * 🔴 UN RENDEZ-VOUS DÉPLACÉ GARDAIT SON RAPPEL (trouvé le 11/09). Le push est
 * programmé chez OneSignal à la réservation, avec l'heure dans son texte, et
 * le déplacement ne le touchait pas : le client recevait « dans 1h, à 19:00 »
 * à 18:00 pour une table passée à 20:30, ou la veille d'un rendez-vous
 * reporté d'une semaine.
 *
 * ⚠️ SEULEMENT S'IL Y EN AVAIT UN : une réservation posée au téléphone n'en a
 * jamais eu, et un déplacement n'a pas à lui en inventer un.
 * ⚠️ ON REPROGRAMME MÊME SI L'ANNULATION ÉCHOUE : un rappel déjà parti ne
 * s'annule plus (OneSignal répond 400), et le nouveau reste le bon.
 */
export async function replanifierRappelRdv(rdvId, supabase) {
  try {
    const { data: rdv, error } = await supabase
      .from('rdv_reservations')
      .select('id, rappel_push_id')
      .eq('id', rdvId)
      .single()
    if (error || !rdv) return { ok: false, error: 'rdv introuvable' }
    if (!rdv.rappel_push_id) return { ok: true, skipped: 'aucun_rappel' }

    const ancien = await annulerPush(rdv.rappel_push_id)
    if (!ancien?.ok) console.warn('[rappels] ancien rappel non annulé', { rdvId, error: ancien?.error })

    // ⚠️ LA CASE SE LIBÈRE, sinon `programmerRappelRdv` croit le rappel posé et
    // ne fait rien. Seulement si c'est ENCORE le même : un autre geste a pu le
    // remplacer entre-temps, et on n'efface pas le sien.
    const { error: errMaj } = await supabase
      .from('rdv_reservations')
      .update({ rappel_push_id: null })
      .eq('id', rdv.id)
      .eq('rappel_push_id', rdv.rappel_push_id)
    if (errMaj) return { ok: false, error: errMaj.message }

    return await programmerRappelRdv(rdvId, supabase)
  } catch (e) {
    console.warn('[rappels] replanifierRappelRdv KO', e?.message)
    return { ok: false, error: e?.message || String(e) }
  }
}
