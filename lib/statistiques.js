...
import { resteApresRemboursement, indexerRetoursBons, bonReste } from './remboursements'
...
// ─── CE QU'UN NO-SHOW A LAISSÉ AU COMMERÇANT ────────────────────────────────
//
// 🔴 IL N'APPARAISSAIT NULLE PART (03/09, arbitrage d'Alex). `no_show` est
// absent de `STATUTS_RDV_ENCAISSES` avec ce motif, écrit noir sur blanc : « le
// client n'est pas venu, la prestation n'a pas eu lieu ». C'était vrai. Depuis
// le 31/08, le no-show fait GARDER UNE GARANTIE : de l'argent réel, que le
// journal comptable écrit désormais et que le tableau de bord ignorait. Les
// deux écrans divergeaient sur la même question.
//
// ⚠️ ET IL NE VAUT PAS SON PRIX, il vaut CE QUI EST RESTÉ. Le compter comme une
// prestation rendue gonflerait le chiffre d'affaires d'une séance qui n'a pas
// eu lieu ; l'ignorer le prive d'un encaissement bien réel. C'est exactement la
// règle du journal : on compte ce qui est resté.
//
// ⚠️ LA PART SUR BON NE SE DEVINE PAS : `bon_cadeau_montant` dit ce que le bon a
// payé, jamais ce qu'il a fini par payer. Seuls les mouvements de re-crédit le
// savent, et c'est pour ça qu'ils remontent jusqu'ici.
export function valeurNoShow(rdv = {}, retoursDuRdv = []) {
  return arrondi(acompteRdv(rdv) + bonReste(rdv.bon_cadeau_montant, retoursDuRdv))
}

export function rdvNoShow(rdv = {}) {
  return String(rdv.statut || '') === 'no_show'
}
...
// fait vivre.
//
// `retoursBons` : les mouvements de re-crédit (`source = 'annulation'`). Ils
// servent à savoir ce qu'un no-show a laissé sur un bon, que nulle colonne ne
// dit. Sans eux, la garantie gardée sur un bon reste invisible.
export function chiffreAffaires(commandes = [], rdvs = [], abonnements = [], retoursBons = []) {
  ...
  // ⚠️ LE NO-SHOW N'EST PAS UN RENDEZ-VOUS HONORÉ, mais il a laissé de l'argent.
  // Il compte donc pour CE QUI EST RESTÉ, et jamais pour le prix de la séance.
  const noShows = rdvs.filter(rdvNoShow)
  const garanties = noShows.reduce((s, r) => s + valeurNoShow(r, retours.parRdv.get(r.id)), 0)
  const prestations = arrondi(honores.reduce((somme, r) => somme + valeurRdv(r), 0) + garanties)
  const acomptes = [...honores, ...noShows].reduce((somme, r) => somme + acompteRdvEnLigne(r), 0)
  ...
  return {
    ...
    // ⚠️ ET LE NOMBRE DE RENDEZ-VOUS NE COMPTE QUE LES HONORÉS : un no-show a
    // laissé de l'argent, il n'a pas rendu de séance. Le compter ici ferait
    // croire à un carnet plus rempli qu'il ne l'est.
    nb_rdv: honores.length,
    // Ce que les clients qui ne sont pas venus ont laissé. Zéro la plupart du
    // temps, et c'est une information : la garantie a bien été rendue.
    garanties_no_show: arrondi(garanties),
