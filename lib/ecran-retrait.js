...
// Le vocabulaire du métier : un restaurant réserve une TABLE, un salon prend un
// RENDEZ-VOUS, et le participe s'accorde avec ce qu'on vient de réserver.
...
// ⚠️ ET L'ACCORD ÉTAIT ÉCRIT SANS ÊTRE TENU (Alex, 09/09) : « Ton RDV est
// Yoppé » s'affichait après une réservation de table, alors que la phrase juste
// au-dessus dit que TABLE et RÉSERVATION sont féminins. La règle vivait en
// commentaire, elle vit maintenant dans le code.
//
// ⚠️ `commercant` EST FACULTATIF, et son absence rend le vocabulaire du
// rendez-vous : c'est ce que lisent les bancs qui appellent sans contexte, et
// c'est le bon défaut. Une vitrine ne change pas d'un mot.
//
// ⚠️ ET SEUL LE CAS DU RENDEZ-VOUS BOUGE. Une commande à emporter ou un panier
// anti-gaspi restent « Ta commande est Yoppée » chez le restaurateur comme chez
// tout le monde : ce n'est pas une table, c'est une commande.
export function textesConfirmation(contexte, { commercantNom = 'ton commerçant', avecProduits = false, commercant = null } = {}) {
  ...
  switch (contexte) {
    ...
    case RETRAIT_RDV:
      return {
        titre: mots.yoppe,
        sousTitre: avecProduits ? mots.yoppeSousTitreProduits : mots.yoppeSousTitre,
        etapes: avecProduits
          ? [
            mots.yoppeEtapeUn,
