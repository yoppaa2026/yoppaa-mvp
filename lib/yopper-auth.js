...
//
// 🔴 LE REPLI SUR LE COOKIE SIGNÉ EST RETIRÉ (15/09). Il était resté « le temps
// que le tunnel impose la connexion », marqué `prouve: false`, et deux routes
// s'en contentaient encore : `commande/ignore-avis` et `signaux` (le détail est
// dans chacune). `POST /api/yopper/session` signe ce qu'on lui déclare : un
// cookie signé prouve que NOUS l'avons émis, jamais que celui qui le présente
// est la bonne personne. Il ne sert plus qu'à préremplir, et aucune route ne
// s'y fie : `npm run verif:acces-api` le tient.

import { createClient } from '@supabase/supabase-js'
import { ficheUtilisablePar } from './fiche-client'
...
// Identité du Yopper courant, PROUVÉE par le jeton Supabase.
//   { client_id, email, prenom, nom, telephone, prouve: true } → authentifié
//   null                                                        → personne
export async function identiteYopper(request) {
  ...
  if (user?.email) {
    ...
    if (!fiche) {
      ...
      // 🔴 UNE FICHE RELIÉE À UN AUTRE COMPTE NE SE REND PAS (15/09). Le code
      // refusait déjà de la rattacher, et la rendait quand même, avec le nom et
      // le téléphone de l'autre. Cas réel : un compte qui change d'email laisse
      // sa fiche à l'ancienne adresse, et l'email est unique dans `clients`.
      fiche = ficheUtilisablePar(parEmail, user.id) ? parEmail : null
  ...
  // 🔴 SANS JETON VÉRIFIÉ, PERSONNE (15/09). Le repli sur le cookie déclaré est
  // retiré : `POST /api/yopper/session` le signe pour quiconque le demande.
  return null // sans jeton vérifié, personne
}

// Ce que les routes importent. Le filtre sur `prouve` ne coûte rien, et il
// tiendrait encore si un repli revenait un jour.
