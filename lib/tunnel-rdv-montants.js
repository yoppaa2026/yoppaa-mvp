...
export function ventilerTunnelRdv({
  ...
  // ⚠️ LA RÉCOMPENSE BAISSE LE PRIX, LE BON PAIE LE PRIX. Toute la règle de
  // l'acompte tient dans cette distinction, posée par Alex le 30/08 devant une
  // prestation à 60 € couverte à 50 € : « il reste 10 €, et on demande encore
  // 5 € d'acompte, le client se dit qu'il a déjà payé bien plus que ça ».
  //
  //   • LA RÉCOMPENSE est un cadeau du commerçant : elle réduit ce qui est dû,
  //     donc l'assiette de l'acompte. C'est la règle F22 du 24/08, inchangée :
  //     30 € à 25 % avec 5 € de récompense font 6,25 € et non 7,50 €.
  //
  //   • LE BON CADEAU est de l'argent DÉJÀ VERSÉ chez ce commerçant. Or un
  //     acompte n'est rien d'autre qu'une GARANTIE contre le lapin, et le bon
  //     en est déjà une, plus grosse : il ne revient qu'à l'annulation, jamais
  //     sur un no-show. Le déduire de l'assiette puis réclamer un pourcentage
  //     du reste, c'est demander DEUX FOIS la même garantie.
  //
  // Donc : l'assiette est la prestation APRÈS récompense, et le bon se retranche
  // ensuite de l'acompte DÛ, euro pour euro, jusqu'à zéro.
  //
  // ⚠️ ET L'ACOMPTE NE DÉPASSE JAMAIS CE QUI RESTE À PAYER. Le calcul ne peut
  // pas y arriver tant que le pourcentage vaut 100 au plus, mais un réglage à
  // 120 % ferait avancer au Yopper plus que le prix de sa prestation.
  const acompteDu = (acompteEnLigne && prestaApresRecompense !== null && pct > 0)
    ? arrondi(Math.round(prestaApresRecompense * pct) / 100)
    : 0
  const acompte = prestaNette === null
    ? 0
    : arrondi(Math.min(Math.max(0, acompteDu - bonSurPresta), prestaNette))
  ...
  return {
    ...
    prestaNette,
    produitsAPayer,
    // Ce que le pourcentage réclamait AVANT que le bon s'en déduise. Rendu pour
    // que l'écran puisse dire « ton bon couvre déjà l'acompte » au lieu
    // d'afficher un zéro sans explication.
    acompteDu,
    acompte,
