 ...
 */
// L'ORDRE DE SERVICE DES BONS, ÉCRIT UNE FOIS POUR LES DEUX RÉPARTITIONS.
//
// ⚠️ IL VIT ICI PARCE QU'IL EST DEVENU PARTAGÉ (01/09). `repartirBonsRdv` a
// besoin exactement du même ordre que `repartirBons` : le recopier aurait fait
// diverger la boutique et le rendez-vous à la première correction, et c'est le
// motif qui a produit le plus de défauts sur ce projet.
//
// ⚠️ LE TABLEAU REÇU N'EST JAMAIS RÉORDONNÉ : `filter` rend déjà une copie, et
// c'est ELLE que `sort` réarrange. Le trier sur place changerait la liste
...
//
// 🔴 ET C'EST ICI, ET NULLE PART AILLEURS, QU'UN SOLDE NUL EST ÉCARTÉ. J'avais
// aussi un `if (pris <= 0) continue` dans la boucle : deux gardes qui se
// couvraient l'une l'autre, donc AUCUNE des deux ne pouvait rougir seule.
function ordonnerBons(bons) {
...
export function repartirBons(bons, totalDu) {
  ...
  const ordonnes = ordonnerBons(bons)
...
/**
 * Répartit plusieurs bons sur les DEUX cibles d'un rendez-vous.
 *
 * 🔴 UN RENDEZ-VOUS N'A PAS UNE CIBLE, IL EN A DEUX, et c'est ce qui distingue
 * ce tunnel de la boutique. La part qui paie la prestation s'écrit sur
 * `rdv_reservations`, celle qui paie les produits sur `commandes` : deux
 * mouvements, deux cibles, comme l'impose `bons_cadeaux_mouvements_une_cible`.
 *
 * ⚠️ ET CE PARTAGE N'EST PAS COSMÉTIQUE : à l'annulation, le Yopper peut GARDER
 * ses produits. La part prestation lui revient toujours, la part produits non.
 * Un seul montant global rendrait ce choix impossible à honorer.
 *
 * ⚠️ LES DEUX MONTANTS VIENNENT DE `ventilerTunnelRdv`, PAS DE L'ÉCRAN : cette
 * fonction ne décide RIEN sur le combien, seulement sur le QUI paie quoi.
 *
 * ⚠️ LE MINIMUM STRIPE N'INTERVIENT PAS ICI, et c'est voulu. `surPresta` et
 * `surProduits` sont déjà plafonnés par la ventilation au solde total offert :
 * les seaux se vident donc exactement, `restePresta` et `resteProduits` valent
 * zéro, et la correction de `repartirBons` n'aurait rien à corriger. Le
 * minimum, lui, est gardé en amont par les routes, qui refusent un paiement
 * Stripe sous 0,50 €.
 *
 * ⚠️ LES DEUX RESTES SONT RENDUS QUAND MÊME. S'ils ne sont pas nuls, la
 * ventilation et la répartition ne sont pas d'accord : c'est une incohérence
 * interne, à dire dans les journaux, jamais à avaler en silence.
 *
 * @param {Array<{id?, code?, solde, expires_at?}>} bons Les bons retenus.
 * @param {{surPresta: number, surProduits: number}} parts Ce que chacune couvre.
 */
export function repartirBonsRdv(bons, { surPresta = 0, surProduits = 0 } = {}) {
  const cents = (n) => {
    const c = Math.round(Number(n) * 100)
    return Number.isFinite(c) ? Math.max(0, c) : 0
  }
  // 🔴 LE MÊME ORDRE QUE LA BOUTIQUE, par la même fonction : le plus proche de
  // l'expiration paie en premier, et il paie la PRESTATION en premier. C'est
  // l'ordre qui sert le mieux le Yopper : ce qui est posé sur la prestation
  // allège l'acompte, donc ce qu'il sort le jour du rendez-vous.
  const ordonnes = ordonnerBons(bons)
  const presta = []
  const produits = []
  let restePresta = cents(surPresta)
  let resteProduits = cents(surProduits)

  for (const b of ordonnes) {
    if (restePresta <= 0 && resteProduits <= 0) break
    let dispo = Math.round(Number(b.solde) * 100)
    const partPresta = Math.min(dispo, restePresta)
    if (partPresta > 0) {
      presta.push({ id: b.id ?? null, code: b.code ?? null, montant: partPresta })
      restePresta -= partPresta
      dispo -= partPresta
    }
    const partProduits = Math.min(dispo, resteProduits)
    if (partProduits > 0) {
      produits.push({ id: b.id ?? null, code: b.code ?? null, montant: partProduits })
      resteProduits -= partProduits
    }
  }

  const enEuros = (l) => l.map(x => ({ ...x, montant: x.montant / 100 }))
  const somme = (l) => l.reduce((s, x) => s + x.montant, 0) / 100
  return {
    presta: enEuros(presta),
    produits: enEuros(produits),
    totalPresta: somme(presta),
    totalProduits: somme(produits),
    total: somme(presta) + somme(produits),
    restePresta: Math.max(0, restePresta) / 100,
    resteProduits: Math.max(0, resteProduits) / 100,
  }
}

/**
 * Répartit une restitution PARTIELLE sur les bons qui ont payé.
 *
 * 🔴 LE NO-SHOW NE REND PAS LE BON, IL REND CE QUI DÉPASSE LA GARANTIE. Le
 * commerçant garde l'équivalent de l'acompte : sur trois bons ayant financé
 * 180 €, il peut n'y avoir que 145 € à rendre, et il faut décider À QUI.
 *
 * 🔴 ON REND EN COMMENÇANT PAR LA FIN, et c'est le miroir exact du débit. Les
 * bons sont dépensés du plus proche de l'expiration au plus lointain : rendre
 * dans l'ordre inverse repose l'argent sur celui qui expire LE PLUS TARD, donc
 * celui que le Yopper a le plus de temps pour dépenser.
 *
 * ⚠️ ET ÇA NE COÛTE AUCUNE LECTURE : `bons_utilises` ne porte pas les dates
 * d'expiration, mais il porte l'ORDRE, qui les encode déjà.
 *
 * @param {Array<{id, montant}>} bonsUtilises Ce que chaque bon a payé.
 * @param {number} montantRendu Ce qui revient au Yopper, au total.
 */
export function repartirRestitution(bonsUtilises, montantRendu) {
  const liste = (Array.isArray(bonsUtilises) ? bonsUtilises : [])
    .filter(l => l?.id && Number(l.montant) > 0)
  let reste = Math.round(Number(montantRendu) * 100)
  if (!Number.isFinite(reste) || reste <= 0 || liste.length === 0) return []

  const lignes = []
  for (let i = liste.length - 1; i >= 0 && reste > 0; i--) {
    const paye = Math.round(Number(liste[i].montant) * 100)
    const rend = Math.min(paye, reste)
    if (rend > 0) {
      lignes.push({ id: liste[i].id, montant: rend / 100 })
      reste -= rend
    }
  }
  // ⚠️ REMIS DANS L'ORDRE DU DÉBIT pour l'affichage et les journaux : on rend
  // en partant de la fin, on ne RACONTE pas à l'envers.
