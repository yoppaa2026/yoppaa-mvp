...
import { capacitePrestation, premierePlaceLibre, estParCouverts, couvertsValides, occupationDe } from './cours-collectifs'
...
import { prestationAutoriseeSurCreneaux, praticienAutorisePourPrestation, jourSemaineDate, timeToMinutes } from './rdv-slots'
...
export async function creerReservationRdv(db, {
  ...
  if (champs?.source !== 'commercant') {
    ...
    const autorise = prestationAutoriseeSurCreneaux({
      ...
      finMin: timeToMinutes(heure) + Number(prestation.duree_minutes || 0),
  ...
  if (estParCouverts(prestation)) {
    ...
    const finMin = debutMin + Number(prestation.duree_minutes || 0)
