...
// ─── UN CRÉNEAU DÉJÀ COMMENCÉ ──────────────────────────────────────────────
//
// 🔴 TROIS PORTES SUR QUATRE ACCEPTAIENT UN CRÉNEAU PASSÉ (trouvé le 10/09 tard,
// en cherchant les frères du déplacement dans le passé). Seule
// `/api/rdv/reserver` le refusait ; l'acompte en ligne, le rendez-vous avec
// produits et la séance sur abonnement prenaient une date d'hier telle quelle.
// La fiche ne propose que l'avenir, mais un écran ne décide de rien.
//
// ⚠️ À L'HEURE MURALE DE BRUXELLES, jamais à celle du serveur : Vercel tourne en
// temps universel. Et une date ou une heure illisible compte comme passée :
// dans le doute, on refuse plutôt que d'écrire n'importe quoi.
export function creneauDejaCommence(dateStr, heure, maintenant = Date.now()) {
  const instant = brusselsInstant(dateStr, heure)
  const reference = maintenant instanceof Date ? maintenant.getTime() : Number(maintenant)
  return isNaN(instant.getTime()) || !Number.isFinite(reference) || instant.getTime() <= reference
}

// L'alarme « la veille » du fichier calendrier sonnera-t-elle encore ?
//
// 🔴 « RAPPEL AUTOMATIQUE 24H AVANT » SUR UNE TABLE DE CE SOIR (email d'Alex,
// 10/09 tard). L'alarme sonne vingt-quatre heures avant le rendez-vous : à
// moins de vingt-quatre heures, elle tombe dans le passé et aucun calendrier ne
// la fait sonner. L'email ne doit plus la promettre, et le fichier ne plus la
// porter. Une date illisible ne promet rien.
export function rappelVeillePossible(dateStr, heure, maintenant = Date.now()) {
  const instant = brusselsInstant(dateStr, heure)
  const reference = maintenant instanceof Date ? maintenant.getTime() : Number(maintenant)
  return !isNaN(instant.getTime()) && Number.isFinite(reference) && instant.getTime() - 24 * 3600 * 1000 > reference
}
...
// AVANCER OU RECULER D'UN NOMBRE DE JOURS, EN RESTANT UNE DATE CIVILE.
//
// ⚠️ « 2026-09-19 » plus deux jours vaut « 2026-09-21 », et ce calcul ne doit
// JAMAIS passer par un instant : ajouter 48 heures à une date tombe une heure à
// côté au changement d'heure, et relire un instant avec `toISOString()` rend le
// jour de Greenwich, pas celui de Bruxelles. On compte donc en jours entiers,
// ancrés à minuit UTC, et on relit les composantes UTC sans jamais formater
// l'instant. Écrit ici pour qu'il n'existe qu'une seule façon de le faire.
export function jourCivilPlus(dateStr, jours) {
  const [a, m, j] = String(dateStr || '').split('-').map(Number)
  if (!Number.isFinite(a) || !Number.isFinite(m) || !Number.isFinite(j)) return ''
  const d = new Date(Date.UTC(a, m - 1, j + Number(jours || 0)))
...
// L'heure murale belge d'un instant, au format « HH:MM ».
//
// ⚠️ SERT À L'EXPORT COMPTABLE (19/08, demande d'Alex). Une ligne de journal
// qui porte l'heure permet de retrouver la transaction dans le relevé du
// terminal, où tout est horodaté. Encore faut-il que ce soit l'heure que le
// commerçant a vue sur sa pendule, pas celle de Greenwich : un encaissement de
// 00h28 affiché « 22:28 » serait introuvable, et daté de la veille par-dessus.
//
// Rend '' plutôt qu'une heure inventée quand l'instant est absent ou illisible.
export function heureBruxelles(instant) {
  const p = partiesBruxelles(instant)
  if (!p) return ''
