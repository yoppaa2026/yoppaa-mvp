...
import { referenceCommande, referenceRdv } from './numero-commande'
...
export function construireLignes({ commandes = [], rdvs = [], abonnements = [], tauxDefaut = null, articlesParId = {} }) {
  ...
  for (const c of commandes) {
    ...
    lignes.push({
      ...
      reference: referenceCommande(c) || String(c.id).slice(0, 8),
      total,
      parTaux,
  ...
  for (const r of rdvs) {
    ...
    if (MODES_QUI_ENCAISSENT.includes(String(r.encaisse_mode || '')) && encaisse > 0) {
      ...
      lignes.push({
        ...
        canal: 'Rendez-vous',
        regime: 'emporter',
        reference: referenceRdv(r) || String(r.id).slice(0, 8),
        total: encaisse,
