...
export function contenuVisuel({
  type = TYPE_ACTU, enseigne = '', titre = '',
  prix = null, prixBarre = null, suffixe = null, description = null,
  tempsRestant = null, quantite = null, lien = null,
} = {}) {
  const nom = String(enseigne || '').trim()
  ...
  // ⚠️ LES PASTILLES SONT RÉSERVÉES À L'INVENDU. Un deal n'a pas d'heure de fin
  // et une actualité n'a pas de stock : leur en coller ferait de tous les trois
  // le même objet, ce qu'Alex a fait corriger dans l'application le 04/09.
  const pastilles = []
  if (type === TYPE_INVENDU) {
    if (tempsRestant) pastilles.push({ icone: 'horloge', texte: String(tempsRestant) })
    if (quantite) pastilles.push({ icone: 'sac', texte: String(quantite) })
  }
  ...
  return {
    ...
    enseigne: nom,
    titre: quoi,
    description: type === TYPE_ACTU && description ? String(description).trim() : null,
    prix: p,
