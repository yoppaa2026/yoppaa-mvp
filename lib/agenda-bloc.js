...
// `jour`, `fin` et `aujourdhui` sont des dates 'AAAA-MM-JJ'. Aucune horloge
// n'est lue ici : le jour du jour est injecté, sans quoi ce libellé serait
// intestable et le banc pourrirait au fil du calendrier, déjà arrivé le 05/08.
//
// ⚠️ `fin` EST ARRIVÉE LE 16/08, ET ELLE VIENT D'UN SECOND DÉFAUT. L'intitulé
// posé le matin même a révélé pire que ce qu'il corrigeait : dans l'onglet
// Rendez-vous, la date des compteurs NE POUVAIT PAS CHANGER. Le sélecteur de
// jours n'y est pas affiché, l'agenda a sa propre navigation, et les deux ne se
// parlaient pas : les compteurs étaient bloqués sur aujourd'hui à vie.
//
// La règle est maintenant que LES COMPTEURS DÉCRIVENT CE QUE L'AGENDA MONTRE.
// En vue semaine, l'agenda montre sept jours : un intitulé au singulier y
// recréerait exactement le malentendu qu'on vient de nommer.
export function libellePeriodeStats({ jour, fin = null, aujourdhui, historique = false } = {}) {
  if (historique) return 'Historique'

  // Une fenêtre de plusieurs jours se nomme comme telle. On garde le mois une
  // seule fois quand il ne change pas : « du 17 au 23 août » se lit mieux que
  // « du 17 août au 23 août », et il faut bien les deux quand le mois tourne.
  if (fin && fin !== jour) {
    const a = new Date(`${jour}T12:00:00`)
    const b = new Date(`${fin}T12:00:00`)
    if (Number.isNaN(a.getTime()) || Number.isNaN(b.getTime())) return ''
    const memeMois = a.getMonth() === b.getMonth() && a.getFullYear() === b.getFullYear()
    return memeMois
      ? `Semaine du ${a.getDate()} au ${b.getDate()} ${MOIS_LONGS_FR[b.getMonth()]}`
      : `Semaine du ${a.getDate()} ${MOIS_LONGS_FR[a.getMonth()]} au ${b.getDate()} ${MOIS_LONGS_FR[b.getMonth()]}`
  }
