...
import { etatPaiementClient } from './rdv-paiement'
...
export function ligneAvantagesEmail({ fidelite_remise, bon_cadeau_montant } = {}) {
  ...
  const bouts = []
  if (recompense) bouts.push(`<strong>${recompense.toFixed(2)} €</strong> de récompense fidélité`)
  if (bon) bouts.push(`<strong>${bon.toFixed(2)} €</strong> en bon cadeau`)
...
export function emailFideliteProgression({
  ...
  return layout({
    ...
    intro: `<strong>${echapperHtml(yopper_prenom)}</strong>, ton RDV chez <strong>${echapperHtml(commercant_nom)}</strong> a été marqué comme honoré. Tu as gagné un point fidélité 🟣`,
    body: `
      ...
      <p style="margin:0;font-size:13px;color:${C.ink};line-height:1.6;text-align:center;">
        Merci d'être fidèle à <strong>${echapperHtml(commercant_nom)}</strong> 🟣
      </p>
    `,
    ctaUrl: ficheUrl,
...
export function emailFideliteRecompenseDebloquee({
  ...
  return layout({
    ...
    intro: `<strong>${echapperHtml(yopper_prenom)}</strong>, tu viens de débloquer ta récompense fidélité chez <strong>${echapperHtml(commercant_nom)}</strong> 🟣`,
    body: `
      <div style="background:linear-gradient(135deg,#10B981 0%,#059669 100%);border-radius:14px;padding:24px;margin-bottom:14px;text-align:center;color:#fff;">
        ...
        <p style="margin:8px 0 0;font-size:13px;font-weight:700;opacity:0.9;">à ton prochain RDV chez ${echapperHtml(commercant_nom)}</p>
      </div>
      ${code_promo ? `
  ...
  return layout({
    ...
    body: `
      ...
      <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ...
        ${remiseFidEUR > 0 ? `
        <tr><td style="padding:10px 14px;color:${C.main};font-size:13px;font-weight:700;">🟣 Récompense fidélité</td><td style="padding:10px 14px;color:${C.main};font-size:13px;font-weight:800;text-align:right;">−${remiseFidEUR.toFixed(2)} €</td></tr>` : ''}
        ${bonEUR > 0 ? `
        <tr><td style="padding:10px 14px;color:#10B981;font-size:13px;font-weight:700;">🎁 Bon cadeau</td><td style="padding:10px 14px;color:#10B981;font-size:13px;font-weight:800;text-align:right;">−${bonEUR.toFixed(2)} €</td></tr>` : ''}
        ${(remiseFidEUR > 0 || bonEUR > 0) ? `
        <tr><td style="padding:10px 14px;color:${C.ink};font-size:13px;font-weight:900;border-top:1px solid ${C.pale};">${netApresRemises > 0 ? 'Total après remise' : 'Plus rien à payer'}</td><td style="padding:10px 14px;color:${C.ink};font-size:13px;font-weight:900;text-align:right;border-top:1px solid ${C.pale};">${netApresRemises.toFixed(2)} €</td></tr>` : ''}
...
export function emailCommandeAnnuleeYopper({
  ...
  const bouts = []
  if (recompenseOk) bouts.push(`<strong>${remise.toFixed(2)} €</strong> avec ta récompense`)
  if (bonOk) bouts.push(`<strong>${bonMnt.toFixed(2)} €</strong> avec ton bon cadeau`)
  ...
  const rendu = recompenseOk
    ? `Ta récompense de <strong>${remise.toFixed(2)} €</strong> t’est rendue : tu la retrouves dans ta carte de fidélité.`
    : ''
  return layout({
    audience: 'yopper',
    commercantNom: commercant_nom,
...
export function emailCommandeAnnuleeCommercant({
  ...
  return layout({
    ...
    body: `
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        <tr><td style="padding:10px 14px;color:${C.muted};border-bottom:1px solid ${C.pale};">Client</td><td style="padding:10px 14px;color:${C.ink};font-weight:700;text-align:right;border-bottom:1px solid ${C.pale};">${echapperHtml(yopper_prenom) || ''} ${echapperHtml(yopper_nom) || ''}</td></tr>
        <tr><td style="padding:10px 14px;color:${C.muted};${avantages ? `border-bottom:1px solid ${C.pale};` : ''}">Total annulé</td><td style="padding:10px 14px;color:${C.ink};font-weight:700;text-align:right;${avantages ? `border-bottom:1px solid ${C.pale};` : ''}">${total != null ? Number(total).toFixed(2) + ' €' : '—'}</td></tr>
  ...
  return layout({
    ...
    body: `
      ...
      <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ...
        <tr><td style="padding:10px 14px;color:#10B981;font-size:13px;font-weight:700;">🎁 Bon cadeau</td><td style="padding:10px 14px;color:#10B981;font-size:13px;font-weight:800;text-align:right;">−${bonEUR.toFixed(2)} €</td></tr>` : ''}
        ${(remiseFidEUR > 0 || bonEUR > 0) ? `
        <tr><td style="padding:10px 14px;color:${C.ink};font-size:13px;font-weight:900;border-top:1px solid ${C.pale};">Total après remise</td><td style="padding:10px 14px;color:${C.ink};font-size:13px;font-weight:900;text-align:right;border-top:1px solid ${C.pale};">${netCommercant.toFixed(2)} €</td></tr>` : ''}
      </table>
      ${notes_client ? `
      <div style="background:#FFFBEB;border-left:4px solid #F59E0B;border-radius:10px;padding:12px 14px;margin-bottom:14px;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:#78350F;text-transform:uppercase;letter-spacing:0.6px;">Note du client</p>
