...
// Ce que porte chaque mode. Décision Alex du 10/08 : un préfixe là où la
// distinction change le geste du commerçant, rien ailleurs.
...
export function prefixePourCommande({ mode_retrait, creneau_id } = {}) {
  if (mode_retrait === 'livraison') return 'L'
  if (mode_retrait === 'expedition') return 'E'
  if (creneau_id) return 'C'
  return ''
}
