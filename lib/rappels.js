...
export async function programmerRappelCommande(commandeId, supabase) {
  try {
    ...
    const res = await envoyerPushParExternalId(clientId, {
      ...
      contents: `Ta commande #${cmd.numero_commande || ''} est à retirer chez ${cmd.commercant?.nom || 'le commerçant'} vers ${cmd.creneau.heure_debut.slice(0,5)}.`,
...
export async function programmerRappelRdv(rdvId, supabase) {
  try {
    ...
    const res = await envoyerPushParExternalId(clientId, {
      headings: '⏰ Ton rendez-vous approche',
      contents: `Rappel : tu as rendez-vous chez ${rdv.commercant?.nom || 'ton commerçant'} à ${rdv.heure_debut.slice(0,5)}${presta}.`,
      url: rdv.commercant?.slug ? `/commander/rdv/${rdv.commercant.slug}` : '/commander',
