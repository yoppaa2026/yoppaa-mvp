...
import { effacerIdentiteLocale } from '@/lib/identite-locale'
...
export function marquerDeconnexionVoulue() {
  ...
  effacer(CLE_COPIE)
  // 🔴 ET CE QUE LE NAVIGATEUR GARDAIT DE LA PERSONNE PART AVEC ELLE (15/09).
  oublierIdentiteDuNavigateur()
}

// 🔴 L'IDENTITÉ GARDÉE PAR LE NAVIGATEUR PART AVEC LA SESSION (15/09, trouvé par
// Alex). Le nom, l'email et le GSM du Yopper vivent dans le localStorage et dans
// le cookie `yoppaa_yopper`, pour préremplir les formulaires. Seules les pages
// Yopper les effaçaient : une déconnexion faite depuis le tableau de bord,
// l'admin ou « Me connecter avec un autre compte » les laissait en place, et le
// profil Yopper affichait encore les coordonnées de la personne partie. Sur un
// ordinateur partagé, c'est la suivante qui les lit.
//
// ⚠️ ICI, PARCE QUE TOUTES LES SORTIES PASSENT DÉJÀ PAR LA MARQUE : c'est le seul
// endroit qu'une sortie de plus ne pourra pas oublier. Le cookie s'efface en
// tâche de fond ; s'il survivait à une panne réseau, l'appli Yopper ne le relit
// de toute façon plus après une déconnexion voulue.
function oublierIdentiteDuNavigateur() {
  effacerIdentiteLocale()
  if (typeof window === 'undefined') return
  try {
    fetch('/api/yopper/session', { method: 'DELETE', keepalive: true }).catch(() => {})
  } catch { /* fetch indisponible : rien à effacer côté serveur */ }
}
