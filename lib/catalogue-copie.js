...
export function aVerifierApresCopie({
  ...
  if (variantes > 0) {
    ...
    // pas, et le commerçant cherche pourquoi.
    points.push({
      quoi: variantes === 1
  ...
  if (options > 0) {
    // ⚠️ CELUI-LÀ EST UNE BONNE NOUVELLE, et il a sa place : sans lui, le
    // commerçant les recrée à la main sans avoir vérifié qu'elles y sont déjà.
    points.push({
      quoi: options === 1
...
/** Les mêmes points, en texte pour la fenêtre d'information. */
export function texteAVerifier(points = []) {
  return (Array.isArray(points) ? points : [])
    .map(p => `• ${p.quoi}. ${p.faire}`)
    .join('\n')
}
