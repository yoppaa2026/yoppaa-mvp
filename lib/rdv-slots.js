...
import { enModeInventaire, conflitSalle } from './inventaire-salle'
...
export function conflitReservation({
...
export function genererSlots({ dateChoisie, dureeMinutes, creneaux, reservations, horairesDetail, capacite = 1, prestationId = null, liaisonsCreneaux = null, parCouverts = false, couvertsDemandes = 1, salle = null }) {
  ...
  for (const cr of creneauxRetenus) {
    ...
    for (let t = debut; t + dureeMinutes <= fin; t += pas) {
      ...
      const motif = !c.conflit ? null
        : c.raison === 'complet' || c.raison === 'trop_grand' ? 'complet'
