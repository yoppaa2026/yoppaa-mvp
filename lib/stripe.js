...
export const PAYMENT_KIND = {
  RDV_ACOMPTE:     'rdv_acompte',      // acompte d'un rendez-vous, paiement partiel
  RDV_COMMANDE:    'rdv_commande',     // tunnel unique : acompte RDV + produits retirés le jour du RDV
  // ⚠️ CELUI-CI NE PREND RIEN, et c'est toute sa raison d'être. L'empreinte
  // d'une grande table enregistre la carte (Checkout `mode: 'setup'`), rien
  // n'est débité si le client vient, et le no-show se facture plus tard hors
  // session. Il n'a donc ni montant ni PaymentIntent au moment de la session :
  // le webhook le reconnaît à `session.setup_intent`, pas à son paiement.
  RDV_EMPREINTE:   'rdv_empreinte',    // empreinte bancaire sur une table (lot 4, 14/09)
  COMMANDE_TOTAL:  'commande_total',   // paiement complet d'une commande de produits
  BON_CADEAU:      'bon_cadeau',       // achat d'un bon cadeau digital (montant libre)
  // Achat d'un abonnement par un client, en UNE FOIS (décision Alex 15/08).
  // ⚠️ Ce n'est PAS l'abonnement du commerçant à son palier Yoppaa, qui vit
  // dans /api/stripe/billing et n'a rien à voir : ici l'argent va au
  // commerçant, là il vient chez nous. Deux mots identiques, deux flux opposés.
