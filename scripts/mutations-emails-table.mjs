...
const MUTATIONS = [
  ...
  { nom: '🔴 les deux annulations divergent (client)', banc: 'verif:table', fichier: 'app/api/rdv/cancel/route.js',
    de: '          nb_bons:           nbBonsRendus,',
    vers: '          nb_bons_client:    nbBonsRendus,' },
  ...
  // ─── L'ÉCRAN D'ANNULATION ET L'ESPACE DU CLIENT (11/09) ─────────────────
  { nom: '🔴 le restaurant relit « Ton RDV est annulé. »', banc: 'verif:table',
    de: "  ecranAnnule: 'Ta réservation est annulée.',",
    vers: "  ecranAnnule: 'Ton RDV est annulé.'," },
  { nom: '⚠️ le salon dérive sur « déjà annulé »', banc: 'verif:table',
    de: "  ecranDejaAnnule: 'Ce RDV est déjà annulé.',",
    vers: "  ecranDejaAnnule: 'Ce rendez-vous est déjà annulé.'," },
  { nom: '🔴 le lien du restaurant disparaît du module', banc: 'verif:table',
    de: "  annulerLien: 'Annuler cette réservation',",
    vers: '' },
  { nom: '🔴 la route réécrit « déjà annulé » en dur', banc: 'verif:table', fichier: 'app/api/rdv/cancel/route.js',
    de: 'message: mots.ecranDejaAnnule,',
    vers: "message: 'Ce RDV est déjà annulé.'," },
  ...
  { nom: '⚠️ l’écran redit « ton bon » devant trois bons', banc: 'verif:table', fichier: 'app/api/rdv/cancel/route.js',
    de: '    const plusieursBons = nbBonsRendus > 1',
    vers: '    const plusieursBons = false' },
  { nom: '🔴 l’espace du client ne charge plus le nombre', banc: 'verif:table', fichier: 'app/api/rdv/mes-rdvs/route.js',
    de: '      abonnement_id, couverts,',
    vers: '      abonnement_id,' },
  { nom: '🔴 l’espace du client ne sait plus ce qu’est une table', banc: 'verif:table', fichier: 'app/api/rdv/mes-rdvs/route.js',
    de: 'prestation:rdv_prestations(nom, duree_minutes, par_couverts)',
    vers: 'prestation:rdv_prestations(nom, duree_minutes)' },
  { nom: '🔴 la carte redit le format', banc: 'verif:table', fichier: 'app/api/rdv/mes-rdvs/route.js',
    de: '      table: r.prestation?.par_couverts === true,',
    vers: '      table: false,' },
  { nom: '🔴 la fenêtre d’annulation perd la récompense', banc: 'verif:table', fichier: 'app/api/rdv/mes-rdvs/route.js',
    de: '      fidelite_remise, bon_cadeau_montant,',
    vers: '      bon_cadeau_montant,' },
  { nom: '🔴 une table sans acompte lit « Ton acompte te revient »', banc: 'verif:table', fichier: 'lib/rdv-paiement.js',
    de: "  if (r.acompte > 0) bouts.push('ton acompte')",
    vers: "  bouts.push('ton acompte')" },
  { nom: '⚠️ la séance d’abonnement n’est plus annoncée', banc: 'verif:table', fichier: 'lib/rdv-paiement.js',
    de: "  if (rdv?.abonnement_id) phrases.push('Ta séance revient sur ton abonnement.')",
    vers: '' },
  { nom: '⚠️ « te reviennent » pour un seul retour', banc: 'verif:table', fichier: 'lib/rdv-paiement.js',
    de: "${bouts.length > 1 ? 'te reviennent' : 'te revient'}",
    vers: 'te reviennent' },
  { nom: '🔴 la fenêtre annonce trois retours à tout le monde', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: '      message: annonceRetoursClient(rdv, { categorie: rdv.commercant?.categorie }),',
    vers: "      message: 'Ton acompte, ton bon et ta récompense fidélité te reviennent automatiquement.'," },
  { nom: '🔴 la fenêtre redemande « Annuler ce RDV ? »', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: '      title: mots.annulerTitre,',
    vers: "      title: 'Annuler ce RDV ?'," },
  { nom: '🔴 le bouton de la fenêtre redit « Annuler le RDV »', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: '      confirmLabel: mots.annulerBouton,',
    vers: "      confirmLabel: 'Annuler le RDV'," },
  { nom: '⚠️ le bouton qui renonce redevient « Annuler »', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: "      cancelLabel: 'Garder ma place',",
    vers: '' },
  { nom: '🔴 le toast redit « Ton RDV est annulé »', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: '${data.message || mots.ecranAnnule}',
    vers: "${data.message || 'Ton RDV est annulé'}" },
  { nom: '🔴 la carte du client repropose « Annuler ce RDV »', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: '{motsReservation(r.commercant).annulerLien}',
    vers: 'Annuler ce RDV' },
  { nom: '🔴 le montant promis sur les produits redevient le brut', banc: 'verif:table', fichier: 'app/commander/page.js',
    de: 'euros(p.rembourse != null ? Number(p.rembourse) : Number(p.total) + Number(p.acompte))',
    vers: 'euros(Number(p.total) + Number(p.acompte))' },
]
