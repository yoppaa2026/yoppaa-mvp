...
export async function construireVisuel(annonce, formatCle = FORMAT_CARRE) {
  ...
  const hPied = hPoints + hAdresse + F.ecart * 1.2

  const blocs = [hBadge, hEnseigne, hTitre, hDesc, hPrix, hPast].filter(v => v > 0)
  const hTotal = blocs.reduce((s, v) => s + v, 0) + F.ecart * (blocs.length - 1) + F.ecart + hPied
  let y = Math.max(F.marge, (F.hauteur - hTotal) / 2)

  // ── Le badge du type ─────────────────────────────────────────────────────
  ctx.font = police(900, F.badge)
  const largeurMarque = h.marqueSurBadge ? F.badge * 1.5 : 0
  const lBadge = ctx.measureText(h.badge).width + F.badge * 1.9 + largeurMarque
  ctx.fillStyle = h.badgeFond
  coinsArrondis(ctx, gauche, y, lBadge, hBadge, hBadge / 2)
  ctx.fill()
  if (h.marqueSurBadge) {
    tracerMarque(ctx, gauche + F.badge * 0.85, y + (hBadge - F.badge) / 2, F.badge, h.badgeMarque)
  }
  ctx.fillStyle = h.badgeEncre
  ctx.textBaseline = 'middle'
  ctx.fillText(h.badge, gauche + F.badge * 0.95 + largeurMarque, y + hBadge / 2)
  y += hBadge + F.ecart
  ...
  tracerMarque(ctx, gauche, y, F.enseigne, h.marque)
  ctx.font = police(900, F.enseigne)
  ctx.fillStyle = h.accent
  ctx.fillText(c.enseigne.toUpperCase(), gauche + F.enseigne * 1.45, y + F.enseigne * 0.55)
  y += hEnseigne + F.ecart
  ...
  const yPied = F.hauteur - F.marge - hPoints - hAdresse
  ...
  pointsDuVisuel(F.point, h.pointsClairs).forEach(p => {
    ...
    ctx.beginPath()
    ctx.arc(gauche + p.x + p.diametre / 2, yPied + p.decalage + p.diametre / 2, p.diametre / 2, 0, Math.PI * 2)
    ctx.fill()
  })
  ...
  if (c.adresse) {
    ...
    ctx.fillStyle = h.accent
    ctx.fillText(c.adresse, gauche, yPied + hPoints + ta * 0.85)
  }
