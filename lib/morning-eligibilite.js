...
// 🔴 LE FORFAIT EFFECTIF (15/09) : un commerçant en essai publiait un deal que
// son tableau de bord acceptait, et le Good Morning l'écartait sans un mot.
// ⚠️ Les selects appelants doivent charger `essai_plan` et `created_at`.
export function commercantEligibleDeal(c, codesPostaux, maintenant = new Date()) {
  if (!c) return false
  if (!fichePubliee(c)) return false
  const plan = planEffectif(c, maintenant)
  if (!canDo(plan, 'deals') || !canDo(plan, 'morning')) return false
...
export function commercantEligibleActu(c, codesPostaux, maintenant = new Date()) {
  if (!c) return false
  if (!fichePubliee(c)) return false
  if (!canDo(planEffectif(c, maintenant), 'actu_gmy')) return false
  const cp = extraireCodePostal(c.adresse)
  return !!cp && codesPostaux.has(cp)
}
