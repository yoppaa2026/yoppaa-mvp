 ...
 *
 * ⚠️ ON COMPTE LES DEUX NIVEAUX, et c'est voulu : « 2 groupes » ne dit pas si
 * le client aura deux choix ou vingt. « 2 groupes · 9 options » le dit.
 *
 * @param groupes [{ valeurs: [...] }] les groupes de CET article
...
export function apercuOptions(groupes = []) {
  ...
  const partGroupes = liste.length === 1 ? '1 groupe' : `${liste.length} groupes`
  // ⚠️ UN GROUPE VIDE EXISTE : créé puis jamais rempli. Annoncer « 1 groupe ·
  // 0 option » serait exact mais illisible ; on dit ce qui manque.
  if (options === 0) return `${partGroupes}, aucune option`
  const partOptions = options === 1 ? '1 option' : `${options} options`
  return `${partGroupes} · ${partOptions}`
}
