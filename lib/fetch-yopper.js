...
// Le jeton du moment, ou null. Tente UN renouvellement avant d'abandonner.
...
/**
 * Appel qui transporte la preuve d'identité SI elle existe, et part quand même
 * sinon.
 *
 * ⚠️ CE N'EST PAS UN `fetchYopper` PERMISSIF, C'EST UN AUTRE BESOIN. Les routes
 * de `fetchYopper` sont personnelles : sans jeton, elles n'ont rien à dire et
 * partir en anonyme fabriquait l'écran vide du 11/08. Le TUNNEL DE COMMANDE,
 * lui, doit continuer de fonctionner pour un INVITÉ : la commande est son
 * cœur de métier, et refuser l'appel faute de session bloquerait la vente.
 *
 * Le jeton n'y sert qu'à une chose : permettre au serveur de reconnaître un
 * Yopper connecté pour lui proposer sa récompense de fidélité. Sans jeton, le
 * serveur ne verra qu'un invité, la commande passera, et la récompense ne sera
 * simplement pas offerte. C'est exactement l'arbitrage d'Alex.
 */
export async function fetchAvecPreuveSiConnecte(url, options = {}) {
  const token = await jetonCourant()
