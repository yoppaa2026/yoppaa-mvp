...
import { partRemboursee, indexerRetoursBons } from './remboursements'
import { USAGE_UNIQUE, regimeDuBon } from './bons-tva'
...
// ⚠️ LE REGROUPEMENT DES RETOURS DE BONS VIT DANS `lib/remboursements.js`
// DEPUIS LE 03/09, parce que le TABLEAU DE BORD en a besoin du mot pour mot :
// lui aussi doit savoir ce qu'un no-show a laissé au commerçant.
export { indexerRetoursBons }
...
// ─── LE RÉGIME DE CHAQUE BON, ET CE QUI A DÉJÀ ÉTÉ TAXÉ À SA VENTE ──────────
//
// ⚠️ UN BON À USAGE UNIQUE A PORTÉ SA TVA LE JOUR DE SA VENTE. Le compter une
// seconde fois à l'utilisation la déclarerait deux fois. Et un même règlement
// peut mélanger les deux régimes : `bons_utilises` porte le détail, bon par
// bon, avec le montant que chacun a payé.
export function regimesParBon(bons = []) {
  const m = new Map()
  for (const b of (bons || [])) if (b?.id) m.set(String(b.id), regimeDuBon(b))
  return m
}

// La part d'un règlement par bon qui a DÉJÀ été taxée à la vente du bon.
export function partDejaTaxee(objet, regimeParBon, plafond) {
  const liste = Array.isArray(objet?.bons_utilises) ? objet.bons_utilises : []
  const brut = liste.reduce((s, b) => (
    s + (regimeParBon?.get(String(b?.id)) === USAGE_UNIQUE ? (Number(b?.montant) || 0) : 0)
  ), 0)
  // ⚠️ JAMAIS PLUS QUE CE QUE LA LIGNE PORTE : `bons_utilises` est un
  // historique, la colonne de montant est ce qui a réellement payé cette vente.
  return arrondi(Math.min(arrondi(brut), Math.max(0, arrondi(plafond))))
}

/**
 * Le régime du ou des bons qui ont payé une ligne, en deux lettres.
 *
 * ⚠️ 🔴 IL MANQUAIT AU COMPTABLE, ET C'EST SON COMPTABLE QUI L'A DEMANDÉ (03/09).
 * L'en-tête du fichier explique bien pourquoi certaines lignes sont à zéro, mais
 * il l'explique GLOBALEMENT : devant une ligne précise, le lecteur doit encore
 * deviner de quel régime elle relève. Un commerce peut avoir des bons des deux
 * régimes en circulation, puisque le régime est figé à la vente de chaque bon.
 *
 * ⚠️ CETTE COLONNE NE VIT QUE DANS LE DÉTAIL. Le journal agrège par JOUR : un
 * régime n'y voudrait rien dire, comme l'heure et le nom du client.
 *
 * ⚠️ ET ELLE NE PORTE JAMAIS LE CODE DU BON, contrairement à ce qui a été
 * suggéré. Ce fichier quitte l'application : un code encore chargé y serait
 * dépensable par quiconque l'ouvre. Deux lettres suffisent à comprendre.
 */
export function libelleRegimeBon(dejaTaxe, aImposer) {
  const uu = arrondi(dejaTaxe) > 0
  const um = arrondi(aImposer) > 0
  if (uu && um) return 'UU+UM'
  if (uu) return 'UU'
  if (um) return 'UM'
  return ''
}
...
export function construireLignes({
  ...
  retoursBons = [], bons = [], periode = null,
}) {
  const lignes = []
  const retours = indexerRetoursBons(retoursBons)
  const regimeParBon = regimesParBon(bons)
  ...
  for (const c of commandes) {
    ...
    // ─── 🔴 LE FRAIS RETENU D'UNE VENTE ANNULÉE NE FIGURAIT NULLE PART ──────
    //
    // Trouvé le 03/09 dans l'export d'Alex, par un chiffre qui ne pouvait pas
    // exister : un acompte portait 0,07 € de frais. Cette valeur ne naît que
    // d'une ventilation, donc le paiement avait coûté 0,35 et une commande
    // sœur en portait 0,28. Cette commande était `annulee_client_refund`, donc
    // écartée par son statut, et Stripe ne restitue jamais ses frais : le
    // journal annonçait 0,07 pour un paiement qui en avait coûté 0,35.
    //
    // ⚠️ C'EST L'ARBITRAGE D'ALEX DU 31/08, APPLIQUÉ À MOITIÉ. « Une vente
    // ramenée à zéro reste visible et porte le frais retenu » n'avait été
    // appliqué qu'aux remboursements qui GARDENT leur statut. Une vente
    // annulée emportait son coût avec elle.
    //
    // ⚠️ ON N'ÉCRIT QUE LE FRAIS, PAS LA VENTE, et c'est délibéré. Ressusciter
    // la vente exigerait de lui poser sa contrepassation, or dans le tunnel
    // unique le webhook écrit le MÊME `stripe_refund_amount` sur le
    // rendez-vous et sur la commande : deux remboursements successifs et c'est
    // le dernier qui reste. Une contrepassation SOUS-évaluée créerait du
    // chiffre d'affaires qui n'existe pas, ce qui est bien pire que le trou
    // qu'on comble. Le comptable, lui, n'a pas besoin d'une vente annulée : il
    // a besoin du coût, pour son compte de charges.
    //
    // ⚠️ 🔴 J'AI EXIGÉ LE REMBOURSEMENT ENREGISTRÉ, ET CETTE LIGNE N'EST JAMAIS
    // SORTIE (constaté en base le 03/09 : SEPT commandes exclues portent un
    // frais Stripe, AUCUNE ne porte `stripe_refund_amount`). La colonne n'est
    // écrite que par le webhook `charge.refunded`, qui ne trouve pas toujours la
    // commande dans le tunnel partagé. Une garde qui ne se déclenche jamais est
    // pire qu'une garde absente : elle fait croire que le trou est comblé.
    //
    // ⚠️ LE FRAIS EST LA PREUVE, PAS LA COLONNE DE REMBOURSEMENT. Stripe ne
    // prélève un frais que sur un paiement RÉUSSI : `stripe_frais > 0` dit donc
    // à lui seul que l'argent est passé. Un paiement échoué ou en attente n'en a
    // aucun, et s'exclut tout seul sans qu'on ait à le nommer.
    //
    // ⚠️ CE QU'ON NE SAIT TOUJOURS PAS, ET QU'ON N'INVENTE PAS : si le principal
    // est reparti chez le client ou si le commerçant l'a gardé. Le premier cas
    // est un remboursement, le second serait du chiffre d'affaires que ce
    // journal ne compte pas. Ce trou-là reste ouvert et NOMMÉ, en tête de
    // fichier comme ici. Écrire le frais ne l'aggrave pas : il le réduit.
    if (!estComptabilisable(c.statut)) {
      const fraisPerdu = arrondi(c.stripe_frais)
      if (fraisPerdu > 0) {
        poser({
          date: jourComptable(c.created_at, c.date_commande),
          heure: heureComptable(c.created_at),
          ...
          // La vente n'existe plus : aucun chiffre d'affaires, donc aucune
          // ventilation par taux. Un frais Stripe ne porte pas de TVA
          // déductible ici : c'est une charge, pas un achat de bien.
          total: 0,
          parTaux: {},
          enLigne: 0,
          comptoir: 0,
          ...
          bonCadeau: 0,
          venteBon: 0,
          fraisStripe: fraisPerdu,
          netStripe: arrondi(-fraisPerdu),
          statut: c.statut,
          fraisRetenu: true,
        })
      }
    ...
    // ─── ET LA PART D'UN BON À USAGE UNIQUE EST DÉJÀ TAXÉE ─────────────────
    //
    // ⚠️ ELLE SORT DU CHIFFRE D'AFFAIRES DE CETTE LIGNE, comme la récompense,
    // mais pour une raison différente : la récompense n'a jamais été payée, ce
    // bon-ci l'a été le jour de sa vente, et cette vente-là porte sa propre
    // ligne avec sa TVA. Le recompter ici déclarerait la même TVA deux fois.
    //
    // ⚠️ ET L'INVARIANT TIENT TOUJOURS : ce qui sort du chiffre d'affaires sort
    // aussi de la colonne « payé par bon cadeau », donc les deux côtés baissent
    // du même montant.
    const parBonDejaTaxe = partDejaTaxee(c, regimeParBon, parBon)
    const parBonAImposer = arrondi(parBon - parBonDejaTaxe)
    const caLigne = arrondi(totalNet - parBonDejaTaxe)

    // ⚠️ L'ÉCART SE MESURE SUR CE QUI EST RÉELLEMENT IMPOSABLE. La remise et la
    // part déjà taxée se retrouvent ainsi rattachées au taux dominant,
    // exactement comme les autres écarts de reconstitution.
    const ecart = arrondi(caLigne - ttcArticles - fraisLivraison)
    ...
    // ⚠️ PLAFONNÉ À CE QUI A ÉTÉ COMPTÉ, pas à ce que le bon portait : on ne
    // contrepasse pas un chiffre d'affaires qu'une part déjà taxée n'a jamais
    // fait entrer dans cette ligne.
    remboursements.push(...retoursPlafonnes(retours.parCommande.get(c.id), parBonAImposer))
    ...
    poser({
      ...
      bonCadeau: parBonAImposer,
      bonDejaTaxe: parBonDejaTaxe,
      regimeBon: libelleRegimeBon(parBonDejaTaxe, parBonAImposer),
      venteBon: 0,
  ...
  for (const r of rdvs) {
    ...
    // ⚠️ MÊME RÈGLE QUE SUR LA COMMANDE : un bon à usage unique a déjà porté sa
    // TVA à sa vente. La ligne reste écrite même quand tout est déjà taxé, à
    // zéro, parce qu'un rendez-vous entièrement réglé par un bon qui disparaît
    // du journal est exactement le défaut corrigé le 29/08.
    const parBonRdvDejaTaxe = partDejaTaxee(r, regimeParBon, parBonRdv)
    const parBonRdvAImposer = arrondi(parBonRdv - parBonRdvDejaTaxe)
    if (parBonRdv > 0) {
      ...
      poser({
        ...
        regime: 'emporter',
        ...refLigne(referenceQualifiee(referenceRdv(r), r.numero_semaine) || String(r.id).slice(0, 8)),
        total: parBonRdvAImposer,
        parTaux: parBonRdvAImposer > 0 ? { [cleB]: parBonRdvAImposer } : {},
        enLigne: 0,
        comptoir: 0,
        resteAEncaisser: 0,
        bonCadeau: parBonRdvAImposer,
        bonDejaTaxe: parBonRdvDejaTaxe,
        regimeBon: libelleRegimeBon(parBonRdvDejaTaxe, parBonRdvAImposer),
        venteBon: 0,
        ...
        id: r.id,
        remboursements: retoursPlafonnes(retours.parRdv.get(r.id), parBonRdvAImposer),
      })
    }
  ...
  //
  // Trouvée en interrogeant la base, jamais par un banc ni par la lecture d'un
  // export : AUCUN contrôle sur un fichier ne peut révéler une ligne ABSENTE.
  // Quinze bons vendus, encaissés sur le compte Stripe du commerçant, sans une
  // seule écriture. La colonne « encaissé en ligne », présentée au commerçant
  // comme la clé du rapprochement, ne pouvait donc pas se rapprocher.
  //
  // ⚠️ ET CE QUI L'A CACHÉE ÉTAIT DANS CE FICHIER : quatre commentaires
  // affirmaient que « cette vente-là a sa propre ligne ». C'était faux, et
  // relu quatre fois sans être vérifié. Une affirmation en commentaire se
  // vérifie exactement comme du code.
  //
  // ⚠️ ON NE PEUT PAS SIMPLEMENT L'AJOUTER AU CHIFFRE D'AFFAIRES : sous le
  // régime des usages multiples, l'utilisation du bon y est DÉJÀ comptée, et la
  // même prestation compterait deux fois. La vente est alors un ENCAISSEMENT
  // SANS CHIFFRE D'AFFAIRES, une dette envers le client jusqu'au jour où il
  // consomme, et l'invariant s'étend d'un cran :
  //     CA TTC + vente de bons = en ligne + comptoir + payé par bon + reste
  //
  // Sous le régime de l'usage unique, au contraire, c'est CETTE vente qui porte
  // le chiffre d'affaires et la TVA, et l'utilisation ne porte plus rien.
  // L'invariant d'origine tient alors sans le moindre ajout.
  for (const b of bons) {
    // Un bon non payé n'est pas une vente : rien n'est entré.
    if (!b || String(b.statut || '') === 'paiement_en_attente') continue
    const montant = arrondi(b.montant_initial)
    if (montant <= 0) continue
    // ⚠️ SANS DATE DE PAIEMENT, PAS DE LIGNE. C'est le cas des bons vendus
    // avant que la colonne existe : le cron nocturne va la chercher dans la
    // session Stripe, et ils apparaîtront d'eux-mêmes. On ne les rattache pas
    // à un jour inventé en attendant.
    const date = jourComptable(b.paye_le)
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) continue

    const usageUnique = regimeDuBon(b) === USAGE_UNIQUE
    const cle = cleTaux(b.tva_taux != null ? b.tva_taux : tauxDefaut)
    poser({
      date,
      heure: heureComptable(b.paye_le),
      client: nomClientLigne(b.acheteur_prenom),
      type: 'Vente bon cadeau',
      canal: 'Bon cadeau',
      // Une prestation ou un bien à emporter : jamais de consommation en salle.
      regime: 'emporter',
      // ⚠️ 🔴 LE CODE DU BON NE SORT PAS D'ICI, ET C'EST UNE RÈGLE DE SÉCURITÉ.
      // Ce fichier quitte l'application : y écrire des codes encore chargés
      // permettrait à quiconque l'ouvre de les dépenser. L'identifiant suffit
      // au rapprochement, et il est unique, donc jamais ambigu — d'où l'absence
      // de `refLigne`, comme pour les abonnements.
      reference: `BON${String(b.id || '').slice(0, 8)}`,
      total: usageUnique ? montant : 0,
      parTaux: usageUnique ? { [cle]: montant } : {},
      // Le paiement d'un bon est toujours en ligne : il n'existe pas de vente
      // de bon au comptoir dans Yoppaa.
      enLigne: montant,
      comptoir: 0,
      resteAEncaisser: 0,
      modeEncaissement: null,
      // Ce n'est pas un règlement PAR bon, c'est la vente D'un bon.
      bonCadeau: 0,
      venteBon: usageUnique ? 0 : montant,
      regimeBon: usageUnique ? 'UU' : 'UM',
      fraisStripe: montantStripe(montant, b.stripe_frais),
      netStripe: montantStripe(montant, b.stripe_net),
      statut: b.statut,
    })
  }
  ...
  for (const l of lignes) {
    if (!jours.has(l.date)) {
      jours.set(l.date, {
        ...
        enLigne: 0, comptoir: 0, resteAEncaisser: 0, bonCadeau: 0, venteBon: 0, fraisStripe: 0, netStripe: 0,
    ...
    // Repli a zero comme pour « reste a encaisser » : une ligne d'un type qui
    // oublierait le champ ne doit pas empoisonner le total du jour.
    j.venteBon = arrondi(j.venteBon + (Number(l.venteBon) || 0))
...
// ⚠️ UNE LIGNE À ZÉRO QUI PORTE UN FRAIS S'EXPLIQUE, SINON ELLE INTRIGUE. Le
// comptable doit savoir que la vente a bien existé, qu'elle a été remboursée en
// entier, et que seul le coût d'encaissement subsiste parce que Stripe ne le
// restitue pas. Sans cette phrase, il chercherait une vente absente.
export function compterFraisRetenus(lignes = []) {
  return (lignes || []).filter(l => l && l.fraisRetenu).length
}

// ⚠️ UNE COLONNE QUI S'AJOUTE AU CHIFFRE D'AFFAIRES DOIT SE DIRE. Sans cette
// phrase, un comptable qui vérifie « CA = en ligne + comptoir + bon + reste »
// trouve un écart et cherche l'erreur : elle n'est pas là, c'est l'égalité qui
// a gagné un terme le jour où la vente des bons est entrée au journal.
export function compterVentesBons(lignes = []) {
  return (lignes || []).filter(l => l && (Number(l.venteBon) || 0) > 0).length
}

// ⚠️ 🔴 DIX LIGNES ENTIÈREMENT À ZÉRO, ET RIEN NE DISAIT POURQUOI (03/09, sur
// l'export réel d'Alex). Un rendez-vous réglé par un bon à usage unique ne porte
// plus ni chiffre d'affaires ni TVA, puisque la vente du bon les a portés. C'est
// juste, et c'est illisible : le comptable voit des écritures vides et cherche
// l'erreur. Un montant sans explication n'est pas une information.
//
// On les compte et on le DIT, exactement comme pour les références ambiguës et
// les montants négatifs.
export function compterDejaTaxes(lignes = []) {
  return (lignes || []).filter(l => l && (Number(l.bonDejaTaxe) || 0) > 0).length
}

function entete({ commercant, du, au, assujetti, avertissementTaux, tauxManquants, refsIncompletes = 0, remboursements = 0, fraisRetenus = 0, ventesBons = 0, dejaTaxes = 0 }) {
  const l = [
  ...
  if (dejaTaxes > 0) {
    l.push(ligneCsv([`${dejaTaxes} ligne${dejaTaxes > 1 ? 's' : ''} sont a zero et c est normal : elle${dejaTaxes > 1 ? 's ont' : ' a'} ete reglee${dejaTaxes > 1 ? 's' : ''} par un bon cadeau a USAGE UNIQUE, dont la TVA a deja ete portee a la VENTE du bon. Leur utilisation n est plus une operation taxable, et les recompter ici declarerait la meme TVA deux fois.`]))
  }
  if (ventesBons > 0) {
    l.push(ligneCsv(['La colonne "Vente de bons" porte des bons cadeaux vendus dont la TVA n est due qu a leur UTILISATION (bons a usages multiples) : c est un encaissement, pas un chiffre d affaires. L egalite du fichier devient donc : CA TTC + Vente de bons = Encaisse en ligne + Encaisse au comptoir + Paye par bon cadeau + Reste a encaisser.']))
    l.push(ligneCsv(['Les bons dont le taux est connu des l emission (bons a usage unique) figurent au contraire en chiffre d affaires a la date de leur VENTE, et leur utilisation ne porte alors ni chiffre d affaires ni TVA.']))
  }
  if (fraisRetenus > 0) {
    l.push(ligneCsv([`${fraisRetenus} ligne${fraisRetenus > 1 ? 's' : ''} "Frais retenu" : la vente a ete annulee apres avoir ete payee, et Stripe ne restitue pas ses frais. Seul ce cout subsiste, sans chiffre d affaires ni TVA. Yoppaa ne sait pas si le principal a ete rendu au client ou conserve par le commercant : verifiez ces lignes avec lui.`]))
  }
  l.push('')
  return l
}
...
export function csvJournal({ lignes, commercant, du, au, assujetti = true, avertissementTaux = false }) {
  ...
  const out = entete({ commercant, du, au, assujetti, avertissementTaux, tauxManquants,
    ...
    fraisRetenus: compterFraisRetenus(lignes),
    ventesBons: compterVentesBons(lignes),
    dejaTaxes: compterDejaTaxes(lignes) })
  ...
  out.push(ligneCsv([
    ...
    'Paye par bon cadeau', 'Vente de bons', 'Reste a encaisser', 'Frais Stripe', 'Net Stripe',
  ]))

  const totaux = { nb: 0, total: 0, enLigne: 0, comptoir: 0, reste: 0, terminal: 0, especes: 0, virement: 0, bonCadeau: 0, venteBon: 0, frais: 0, net: 0, base: {}, tva: {} }
  for (const j of jours) {
    ...
    cells.push(nombre(j.enLigne), nombre(j.comptoir), nombre(j.terminal), nombre(j.especes), nombre(j.virement),
      nombre(j.bonCadeau), nombre(j.venteBon), nombre(j.resteAEncaisser),
    ...
    totaux.venteBon = arrondi(totaux.venteBon + (Number(j.venteBon) || 0))
  ...
  fin.push(nombre(totaux.enLigne), nombre(totaux.comptoir), nombre(totaux.terminal), nombre(totaux.especes),
    nombre(totaux.virement), nombre(totaux.bonCadeau), nombre(totaux.venteBon), nombre(totaux.reste),
...
export function csvDetail({ lignes, commercant, du, au, assujetti = true, avertissementTaux = false }) {
  ...
  out.push(ligneCsv([
    'Date', 'Heure', 'Client', 'Type', 'Canal', 'Regime', 'Reference', 'Statut', 'Regime du bon', 'Moyen au comptoir', 'Montant TTC', ...colonnesTaux(taux),
    'Encaisse en ligne', 'Encaisse au comptoir', 'Paye par bon cadeau', 'Vente de bons', 'Reste a encaisser', 'Frais Stripe', 'Net Stripe',
  ]))
  ...
  for (const l of lignes) {
    ...
    const cells = [l.date, l.heure || '', l.client || '', l.type, l.canal, l.regime === 'sur_place' ? 'Sur place' : 'A emporter', l.reference, l.statut, l.regimeBon || '', moyen, nombre(l.total)]
    for (const t of taux) {
    ...
    cells.push(nombre(l.enLigne), nombre(l.comptoir), nombre(l.bonCadeau), nombre(Number(l.venteBon) || 0), nombre(Number(l.resteAEncaisser) || 0), l.fraisStripe == null ? '' : nombre(l.fraisStripe), l.netStripe == null ? '' : nombre(l.netStripe))
    out.push(ligneCsv(cells))
  }
