...
// ⚠️ `planEffectif` ET NON `commercant.plan` : un commerçant en essai de
// Communiquer a les deals, et on ne doit plus les lui réclamer.
import { canDo, isVitrine, isDetail, isAlimentaire, planEffectif } from './plans'
...
export const LIBELLE_ENVIE = {
  ...
  // ⚠️ IDÉE D'ALEX, 04/09, ET ELLE EST MEILLEURE QUE LA MIENNE. Je voulais
  // remplir l'écran de fin de journée avec les offres des forfaits inférieurs,
  // au risque de le rendre peu fiable. Lui a proposé de remplir la BOÎTE DU
  // COMMERÇANT avec la demande : au lieu d'un écran à moitié sûr, une pression
  // qui vient de ses propres habitants.
  //
  // ⚠️ LE LIBELLÉ PARLE DE SON COMMERCE, pas de la fonction Yoppaa : « tes
  // invendus », pas « l'anti-gaspi ». C'est la règle 2 de ce module.
  invendus: {
    court: 'Proposer ses invendus',
    phrase: (n) => `${n} ${n > 1 ? 'habitants passeraient' : 'habitant passerait'} en fin de journée pour tes invendus`,
  },
}
...
// ─── ENVOYER UN SIGNAL, ET LIRE CE QUI REVIENT ──────────────────────────────
//
// 🔴 LE FORMULAIRE DE SUGGESTION MENTAIT (08/09). Il écrivait
// `await fetch(...).catch(() => {})` puis affichait « Merci pour ta
// suggestion ! » : un 429, un 403 ou un 500 donnaient exactement le même
// écran, et les quatre champs remplis par l'habitant partaient à la poubelle
// sans que personne, ni lui ni nous, ne le sache. Un `await` dont on ne lit
// pas le résultat est un espoir, pas une action.
//
// ⚠️ ON NE RECOPIE PAS LE MESSAGE DU SERVEUR À L'ÉCRAN. Sur une erreur
// interne, la route rend le message brut de la base : il nomme des tables, des
// colonnes et des contraintes à un visiteur qui n'a même pas de compte. Seuls
// les refus que NOUS écrivons sont montrables, et ils sont listés ici.
export const REFUS_SIGNAL_MONTRABLES = [
  'type invalide',
  'cible manquante',
...
export function messageEchecSignal(statut, erreurServeur) {
  if (statut === 429) return 'Trop d’envois d’un coup. Attends un instant et réessaie.'
  if (statut === 401 || statut === 403) return 'Envoi refusé. Reconnecte-toi et réessaie.'
  if (statut === 400) {
    const brut = String(erreurServeur || '').trim()
    if (REFUS_SIGNAL_MONTRABLES.includes(brut)) {
      return brut.charAt(0).toUpperCase() + brut.slice(1) + '.'
    }
    return 'Il manque une information pour envoyer ça.'
  }
  return `L’envoi n’est pas passé (erreur ${statut || 'réseau'}). Réessaie dans un moment.`
}

export const MESSAGE_SIGNAL_RESEAU = 'Pas de réseau. Rien n’est parti, réessaie.'

// ─── SIGNALER UN AVIS (20/09) ───────────────────────────────────────────────
// 🔴 CE QUI MANQUAIT. Les huit motifs de la modale de signalement visent tous
// la FICHE d'un commerce : fermé, horaires, adresse, téléphone, articles, site
// web, doublon, autre. Aucun ne parle du contenu écrit par un habitant. Or les
// avis sont publics sur les fiches, et une application qui publie du contenu
// d'utilisateur doit permettre de le signaler. Apple l'a demandé le 20/09.
//
// ⚠️ DEUX LISTES, DEUX CIBLES, ET ELLES NE SE MÉLANGENT PAS. « Horaires
// incorrects » n'a aucun sens sur un avis, et « propos haineux » n'en a aucun
// sur une fiche. Montrer les huit motifs de fiche sur un avis ferait choisir au
// hasard, et la file de modération recevrait des signalements inexploitables.
//
// ⚠️ ET LE SERVEUR VÉRIFIE L'APPARTENANCE. Le motif d'un signalement de fiche
// est du texte libre borné à soixante caractères : sur un avis, ça laisserait
// entrer n'importe quel mot comme motif. Ici la liste est fermée.
export const MOTIFS_AVIS = {
  haineux:    'Propos haineux, insultants ou menaçants',
  faux:       'Faux avis, ou visite qui n’a jamais eu lieu',
  personnel:  'Données personnelles (nom, téléphone, adresse)',
  illegal:    'Contenu illégal',
  hors_sujet: 'Hors sujet, publicité ou spam',
  autre:      'Autre',
}

export const TYPES_MOTIF_AVIS = Object.keys(MOTIFS_AVIS)

// ⚠️ `hasOwnProperty.call` ET NON `motif in MOTIFS_AVIS` : `'constructor'` et
// `'toString'` répondent vrai à `in`, et entreraient en base comme motifs.
export function motifAvisConnu(motif) {
  return typeof motif === 'string'
    && Object.prototype.hasOwnProperty.call(MOTIFS_AVIS, motif)
}

export function libelleMotifAvis(motif) {
  return motifAvisConnu(motif) ? MOTIFS_AVIS[motif] : MOTIFS_AVIS.autre
}

// ─── LES MOTIFS D'UNE FICHE, ET POURQUOI ILS DESCENDENT ICI ─────────────────
// 🔴 LE CODE ET LA BASE SE CONTREDISAIENT. La route acceptait un motif de fiche
// en TEXTE LIBRE borné à soixante caractères, alors que `signalements_type_check`
// n'autorise que ces huit valeurs. Un appel direct à l'API avec un motif inventé
// rendait donc 500, exactement comme le signalement d'avis du 20/09. Ce n'était
// pas exploité, puisque la modale ne propose que ces huit ; mais « pas exploité »
// n'est pas « fermé », et c'est une garde d'écran qui tenait toute seule.
//
// ⚠️ LA LISTE VIT ICI ET PLUS DANS L'ÉCRAN, pour la même raison que celle des
// avis : le serveur doit pouvoir la vérifier, et un banc doit pouvoir l'exécuter.
// Une liste qui ne vit que dans un composant ne peut faire ni l'un ni l'autre.
//
// ⚠️ ET ELLE DOIT RESTER L'IMAGE EXACTE DE LA CONTRAINTE SQL. Y ajouter un motif
// sans toucher à `signalements_type_check` rendrait un 500 au premier clic.
export const MOTIFS_FICHE = {
  ferme:     'Fermé / disparu',
...
export const TYPES_MOTIF_FICHE = Object.keys(MOTIFS_FICHE)

export function motifFicheConnu(motif) {
  return typeof motif === 'string'
    && Object.prototype.hasOwnProperty.call(MOTIFS_FICHE, motif)
}

// Rend `{ ok: true }` ou `{ ok: false, statut, message }`. Le message est
// toujours écrit par nous, jamais recopié d'une erreur interne.
export async function envoyerSignal(corps, { fetchImpl } = {}) {
  const f = fetchImpl || (typeof fetch === 'function' ? fetch : null)
  if (!f) return { ok: false, statut: null, message: MESSAGE_SIGNAL_RESEAU }
  let r
  try {
    r = await f('/api/signaux', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(corps),
    })
  } catch {
    return { ok: false, statut: null, message: MESSAGE_SIGNAL_RESEAU }
  }
  const j = await r.json().catch(() => null)
  // ⚠️ LES DEUX CONDITIONS. Une route peut rendre 200 avec `{ ok: false }` :
  // ne lire que `res.ok` laisserait passer un refus applicatif pour un succès.
  if (r.ok && j?.ok) return { ok: true }
  return { ok: false, statut: r.status, message: messageEchecSignal(r.status, j?.error) }
}
...
export function enviesProposables(commercant, { peutCommander = false, proposeDesInvendus = false } = {}) {
  ...
  // 🔴 « LE SIGNAL S'AFFICHE ALORS QU'ON LE FAIT DÉJÀ » (Alex, 27/08, sur La
  // Boutique Témoin, qui EXPÉDIE).
  //
  // ⚠️ DEUX MÉCANIQUES, UN SEUL BESOIN. `livraison_actif` est l'interrupteur de
  // la TOURNÉE alimentaire ; une boutique de détail, elle, ne fait pas de
  // tournée, elle EXPÉDIE (`boutique_mode_vente`). Je lisais donc un drapeau
  // qui n'est JAMAIS allumé chez un détaillant, et le bouton restait à vie.
  //
  // ⚠️ ET C'EST LE MOT DU CLIENT QUI TRANCHE, pas le nôtre : un habitant veut
  // être servi chez lui, il se moque de savoir si c'est une camionnette ou
  // bpost. Un seul bouton « Livraison », qui disparaît dès que le commerçant
  // fait l'UN OU L'AUTRE (décision d'Alex, 27/08).
  const sertADomicile = vitrine || isDetail(commercant)
    // Détail et vitrine : le mode de vente de la boutique. `retrait` est la
    // valeur par DÉFAUT de la colonne, donc la seule qui signifie « rien ».
    ? String(commercant.boutique_mode_vente || 'retrait') !== 'retrait'
    // Alimentaire : la tournée.
    : commercant.livraison_actif === true
  ...
  // Livraison : seulement si commander est DÉJÀ possible. Réclamer la suite
  // d'une chose qui n'existe pas encore n'aiderait personne.
  if (peutCommander && !sertADomicile) sorties.push('livraison')
  ...
  // ─── LES INVENDUS ────────────────────────────────────────────────────────
  //
  // ⚠️ CHEZ LES ALIMENTAIRES SEULEMENT (Alex, 04/09). Ce sont eux qui jettent :
  // demander ses invendus à un coiffeur ou à une boutique de vêtements n'aurait
  // aucun sens, et un bouton qui n'a pas de sens décrédibilise les cinq autres.
  //
  // ⚠️ ET ON DEMANDE CE QUI MANQUE, PAS CE QUE LE FORFAIT NE CONTIENT PAS.
  // C'est la règle de ce module : un commerçant qui a déjà la fonction et ne
  // s'en sert pas est justement celui qu'un mot de client décide, puisqu'il n'a
  // rien de plus à payer. `proposeDesInvendus` est donc renseigné par
  // l'appelant, comme `peutCommander`.
  if (isAlimentaire(commercant) && !proposeDesInvendus) sorties.push('invendus')
  // Bonnes affaires, deals et actus : réservés à Communiquer et au-dessus.
  if (!canDo(planEffectif(commercant), 'deals')) sorties.push('deals')

  // ⚠️ « VOIR LES PRIX » A DISPARU, ET IL ÉTAIT MORT DEPUIS TOUJOURS.
  // Sa condition était `!canDo(plan, 'prix_affiches')` — or `prix_affiches`
  // vaut `true` dans LES TROIS forfaits (`plans.js`). La condition n'a donc
  // JAMAIS été vraie chez personne : ce bouton n'a jamais été vu.
  // ⚠️ Le type `prix` reste dans `TYPES_ENVIE` : d'anciens signaux existent
  // peut-être en base, et le tableau de bord doit continuer à les nommer.
  // On retire le bouton, pas le vocabulaire.
