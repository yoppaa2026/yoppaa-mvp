...
export async function morningADuContenu(supabase, commune) {
  ...
  const [{ data: deals }, { data: actus }, { data: officielles }] = await Promise.all([
    ...
    supabase.from('actualites')
      .select('id, service:services_publics ( codes_postaux, national )')
  ...
  if ((officielles || []).some(a => servicePublicEligible(a.service, codesPostaux))) return true
  return false
}
