...
// Helper Pills statut — calcule l'état des 5 pills selon plan + données runtime
// Retourne un array [{ key, label, actif }] pour rendu uniformisé.
export function getPillsStatut(commercant, { dealActif = false, actuActive = false } = {}) {
  if (isService(commercant)) {
    return [
      ...
      { key: 'actu',      label: 'ACTU',      actif: actuActive },
  ...
  return [
    { key: 'enligne',   label: 'EN LIGNE',  actif: true },
    { key: 'deal',      label: 'DEAL',      actif: canDo(plan, 'deals') && dealActif },
    { key: 'actu',      label: 'ACTU',      actif: canDo(plan, 'actus') && actuActive },
