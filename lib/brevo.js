...
// ─── SMS transactionnel (B.6 fidélité, étape 6) ─────────────────────────────
// Même clé API v3 que les contacts. Côté Brevo il faut UNIQUEMENT :
//   • des crédits SMS achetés (non inclus dans les plans email)
//   • un expéditeur alphanumérique déclaré (3 à 11 caractères, ex. Yoppaa)
// L'expéditeur est configurable via BREVO_SMS_SENDER (défaut : Yoppaa).
//
// `type: 'transactional'` : ce sont des SMS de service (carte créée,
// récompense débloquée), pas du marketing. Ils ne demandent donc pas
// d'opt-in publicitaire, mais restent conditionnés au réglage du
// commerçant (fidelite_sms_actif) et à son solde de crédits.
export async function envoyerSms({ to, contenu }) {
  const apiKey = getApiKey()
  if (!to || !contenu) throw new Error('destinataire et contenu requis')
  ...
  const res = await fetch(`${BREVO_API_URL}/transactionalSMS/sms`, {
    ...
    body: JSON.stringify({
      sender: (process.env.BREVO_SMS_SENDER || 'Yoppaa').slice(0, 11),
      recipient: to,                 // format international : +32470123456
      content: contenu,
      type: 'transactional',
    }),
  })
  ...
  const errBody = await res.text().catch(() => '')
  throw new Error(`Brevo SMS ${res.status} : ${errBody}`)
}
