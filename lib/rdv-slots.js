...
export function genererSlots({ dateChoisie, dureeMinutes, creneaux, reservations, horairesDetail, capacite = 1, prestationId = null }) {
  ...
  for (const cr of creneauxJour) {
    ...
    for (let t = debut; t + dureeMinutes <= fin; t += pas) {
      ...
      // ⚠️ Un cours collectif est, par définition, plusieurs réservations qui
      // se chevauchent. On sépare donc celles qui SONT ce cours de celles qui
      // occupent le praticien à côté : les premières se comptent, les
      // secondes bloquent, comme avant.
      const memeSeance = places > 1
        ? plagesReservees.filter(p => p.start === t && p.end === slotEnd
            && (prestationId === null || p.prestation_id === prestationId))
        : []
      const autres = places > 1
        ? plagesReservees.filter(p => !memeSeance.includes(p))
        : plagesReservees

      const overlapAutre = autres.some(p => t < p.end && slotEnd > p.start)
      const complet = places > 1 && memeSeance.length >= places
      const pris = overlapAutre || complet

      const motif = !pris ? null
        : complet ? 'complet'
      ...
      slotsMap.set(heure, {
        ...
        placesTotal: places > 1 ? places : null,
        placesPrises: places > 1 ? memeSeance.length : null,
