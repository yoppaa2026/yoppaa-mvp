...
// `payeA` est l'INSTANT du paiement, `achatLe` le jour civil belge qui ouvre la
// période. ⚠️ LES DEUX SONT NÉCESSAIRES, et ce n'est pas une redondance : la
// fenêtre de validité se raisonne en jours, l'écriture comptable en instants.
// N'écrire que le jour rangeait un minuit universel dans `paye_le`, et l'export
// affichait alors « 02:00 » pour tout le monde (Alex, 19/08).
export function contratDepuisFormule(formule, { achatLe, payeA = null, commercantId = null, client = {} } = {}) {
...
export function etatAbonnement(abonnement, reservations = [], { aujourdhui } = {}) {
  ...
  return {
    ...
    // 🔴 « TERMINÉ » N'EST PAS « PAS VALABLE » (règle d'Alex, 31/08).
    //
    // `valable` rend faux pour QUATRE raisons différentes, et deux d'entre
    // elles ne sont pas des fins : un abonnement dont la date de début est
    // ENCORE À VENIR, et un contrat sans dates. Ranger le premier dans
    // l'historique archiverait un abonnement acheté ce matin pour le mois
    // prochain, c'est-à-dire l'exact contraire de ce qu'il est.
    //
    // Un abonnement est terminé quand son solde connu est tombé à zéro, quand
    // sa date de fin est PASSÉE, ou quand le contrat a été résilié. Pas avant.
    //
    // ⚠️ ET UN SOLDE INCONNU N'EST PAS UN SOLDE ÉPUISÉ, neuvième fois que ce
    // piège se présente : `epuise` exige déjà `solde === 0`, jamais `null`.
    termine: Boolean(
      solde === 0
...
export function messageRetourAbonnement(retour, { nomCommerce = '' } = {}) {
  if (retour === 'ok') {
    return {
      ...
      message: nomCommerce
        ? `C'est bon, ton abonnement ${chezLeCommerce(nomCommerce)} est actif. Tu reçois l'email de confirmation d'ici quelques instants, avec le détail de ce que tu as pris.`
