...
// ⚠️ LA RÈGLE, PAS UNE COPIE : ce qu'une récompense vaut se décide à un seul
// endroit, et la mécanique le décide seule.
import { recompenseDue, libelleRecompense } from '@/lib/fidelite'
...
export async function creerRecompensesDebloquees(supabase, { carte, commercant, nombre }) {
  ...
  // 🔴 LA VALEUR VIENT DE LA MÉCANIQUE, PLUS DE DEUX RÉGLAGES LIBRES (16/09).
  // Cette route lisait `fidelite_recompense_type` et `..._valeur` directement :
  // une cagnotte de 10 € pouvait donc créer une récompense de 5 €, et un café
  // rendre 25 % de son chiffre sans que personne ne le sache. `recompenseDue`
  // est désormais la seule définition, partagée avec les deux écrans.
  //
  // ⚠️ ET CE QUI EST ÉCRIT ICI EST FIGÉ POUR TOUJOURS : c'est tout l'objet de
  // `fidelite_recompenses`. Le commerçant peut changer ses réglages demain,
  // ce que le client a déjà mérité ne bouge pas.
  const { type, valeur } = recompenseDue(commercant)
  const libelle = String(commercant.fidelite_recompense_libelle || '').trim() || libelleRecompense(commercant)
