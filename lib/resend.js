...
export function emailCommandeConfirmee({
  ...
  return layout({
    ...
    body: `
      <div style="background:linear-gradient(135deg,${C.bg} 0%,${C.pale} 100%);border-radius:14px;padding:18px 20px;border:1px solid ${C.main}22;margin-bottom:14px;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:${C.main};text-transform:uppercase;letter-spacing:0.7px;">${estLivraison ? 'Livraison prévue' : 'Retrait prévu'}</p>
      ...
      <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ...
        ${estLivraison && Number(frais_livraison) > 0 ? `<tr><td style="padding:10px 14px;color:${C.deep};font-size:13px;font-weight:600;border-top:1px solid ${C.pale};">Frais de livraison</td><td style="padding:10px 14px;color:${C.deep};font-size:13px;font-weight:700;text-align:right;border-top:1px solid ${C.pale};">${Number(frais_livraison).toFixed(2)} €</td></tr>` : ''}
      ...
      <div style="background:#fff;border-radius:12px;padding:14px 16px;border:1px solid ${C.pale};margin-bottom:14px;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:${C.muted};text-transform:uppercase;letter-spacing:0.6px;">📍 ${estLivraison ? 'Adresse de livraison' : 'Adresse de retrait'}</p>
        <p style="margin:0;font-size:13px;color:${C.ink};font-weight:700;">${(estLivraison ? adresse_livraison : commercant_adresse) || '—'}</p>
      </div>
      <p style="margin:0 0 14px;font-size:12px;color:${C.muted};line-height:1.55;">
        🔔 Tu recevras un nouvel email dès que ta commande sera <strong>${estLivraison ? 'prête pour la livraison' : 'prête à retirer'}</strong>.
      </p>
      ${cancelUrl ? `
      <div style="background:#FFFBEB;border-left:4px solid #F59E0B;border-radius:10px;padding:12px 14px;margin-top:14px;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:800;color:#78350F;text-transform:uppercase;letter-spacing:0.6px;">Tu veux annuler ?</p>
      ...
      <div style="background:#fff;border-radius:12px;padding:14px 16px;border:1px solid ${C.pale};margin-top:14px;">
        ...
        <a href="https://www.yoppaa.app/commander/auth/definir-mdp" style="display:inline-block;padding:8px 14px;background:${C.main};color:#fff;border-radius:100px;font-weight:800;font-size:12px;text-decoration:none;">Créer mon mot de passe</a>
      </div>` : ''}
    `,
