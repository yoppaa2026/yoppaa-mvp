...
export function libelleCibleDeal({
  ...
  if (requis) {
    return { label: `Ce que ça vise : ${liste} *`, optionGenerale: `— Choisis ${liste} —` }
  }
  return {
    label: `Deal général, ou sur ${liste}`,
    // ⚠️ L'OPTION NOMME CE QU'ON NE LIE PAS, et elle suit la même liste :
    // « pas lié à un produit » devant un menu qui propose des prestations
    // laisserait croire que celles-ci, elles, seraient liées.
    optionGenerale: `— Deal général (pas lié à ${liste}) —`,
  }
}
