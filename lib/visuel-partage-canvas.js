...
export async function construireVisuel(annonce, formatCle = FORMAT_CARRE) {
  ...
  const hEntete = Math.max(hBadge, hPoints) + F.ecart
  const hTotal = hEntete + blocs.reduce((s, v) => s + v, 0) + F.ecart * (blocs.length - 1) + F.ecart + hPied
  ...
  const yEntete = y + (Math.max(hBadge, hPoints) - hBadge) / 2
  const yPoints = y + (Math.max(hBadge, hPoints) - hPoints) / 2
