...
// Reutilise la palette + le layout commun (defini ici en self-contained).
  ...
  <div style="max-width:560px;margin:0 auto;padding:24px 16px;">
    <div style="background:linear-gradient(135deg,${C.panel} 0%,#2D0F6B 60%,${C.ink} 100%);border-radius:18px 18px 0 0;padding:32px 28px 26px;">
      <div style="display:flex;gap:5px;margin-bottom:12px;">
        ...
        <span style="display:inline-block;width:11px;height:11px;border-radius:50%;background:${C.mid};"></span>
      </div>
      <p style="margin:0;color:#fff;font-weight:900;font-size:30px;letter-spacing:-1.4px;line-height:1;">yoppaa</p>
      <p style="margin:8px 0 0;color:${C.light};font-size:12px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;">${title}</p>
    </div>
    ...
    <p style="margin:20px 0 0;text-align:center;font-size:11px;color:${C.muted};">
      yoppaa.app · Mettet, Belgique · bonjour@yoppaa.app
      <br/>Tu reçois cet email parce que tu t'es inscrit(e) sur yoppaa.app.
    </p>
  </div>
</body>
</html>`
  ...
  if (mode_landing === 'teasing') {
    return layout({
      ...
      body: `
        ...
        <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${C.ink};">
          Je construis quelque chose à Mettet depuis plusieurs mois. Un projet belge, ancré, qui prendra sa pleine forme le <strong style="color:${C.main};">7 juillet 2026</strong>.
        </p>
        <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${C.ink};">
          Ce jour-là, tu recevras un email avec le grand dévoilement. Tu seras parmi les <strong>premiers à savoir</strong>.
        </p>
...
// Email envoye le 7 juillet 9h aux pre-inscrits Teasing : le grand reveal.
export function emailRevealLaunch() {
  return layout({
    title: 'Le voile se lève',
    intro: `Le moment est venu. Yoppaa se révèle. 🟣`,
    body: `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${C.ink};">
        Tu fais partie des <strong>premiers curieux</strong>. Tu as laissé ton email pendant la phase mystère.
      </p>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${C.ink};">
        Aujourd'hui, je peux enfin te dévoiler ce que je construis : <strong style="color:${C.main};">Yoppaa, l'app belge des commerces de quartier</strong>.
      </p>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${C.ink};">
