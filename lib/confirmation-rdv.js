...
// ⚠️ L'HEURE MURALE BELGE, jamais l'horloge du serveur : Vercel tourne en UTC,
// et une absence déclarée « à l'heure » y arriverait deux heures trop tôt.
import { brusselsInstant } from './timezone'

// 🔴 ON NE DÉCLARE PAS UNE ABSENCE AVANT L'HEURE (Alex, 16/09 : « la capture
// permet un no-show pour un rendez-vous dans 30 min »).
//
// Le client n'a pas encore eu l'occasion de venir. Le marquer absent l'accuse à
// tort, lui envoie un email qui le lui dit, libère un créneau qu'il occupe
// encore, et surtout ouvre le bouton qui FACTURE sa table. Le débit, lui, était
// déjà tenu par sa propre fenêtre ; mais un restaurateur qui voit « Facturer »
// apparaître croit légitimement pouvoir le faire.
//
// ⚠️ À L'HEURE PILE, C'EST POSSIBLE : un client qui n'est pas là quand son
// service commence est en retard, et c'est au restaurateur d'en juger.
export function noShowPossible(rdv, maintenant = new Date()) {
  const debut = brusselsInstant(rdv?.date_rdv, rdv?.heure_debut)
  if (!debut || isNaN(debut.getTime())) return false
...
export function questionRdv(action, rdv, categorie = null) {
  ...
  // ⚠️ UNE TABLE N'EST PAS UN RENDEZ-VOUS, et certaines issues n'ont aucun sens
  // sur elle. `par_couverts` le dit, et c'est la prestation qui décide, jamais
  // la catégorie : une friterie qui prend des commandes n'a pas de tables.
  const estTable = rdv?.prestation?.par_couverts === true
  ...
  if (action === 'annule_commercant') {
    ...
    return {
      ...
      actions: [
        ...
        // 🔴 PAS SUR UNE TABLE (Alex, 16/09 : « le message de changement
        // d'adresse est aussi un problème »). Cette issue existe pour un salon
        // qui déménage et qui invite ses clients à reprendre place ailleurs.
        // Un restaurant qui annule une table ne change pas d'adresse : proposer
        // ce bouton au milieu d'un service, c'est offrir un geste absurde à
        // quelqu'un de pressé, et le mot « adresse » inquiète le client qui le
        // reçoit pour rien.
        ...(estTable ? [] : [{ valeur: 'lieu', ton: 'danger', label: 'Annuler, en lui disant que je change d’adresse' }]),
...
//
// 🔴 ET ELLE PROMETTAIT UN EMAIL QUI NE PART PLUS (trouvé le 10/09). « Chacune
// reçoit son email de fin de séance » : depuis le 27/08, clôturer n'envoie plus
// rien — l'email de l'ancienne fidélité a été retiré (voir `changerStatutRdv`,
// tableau de bord). Ce qui se passe vraiment : le montant entre au chiffre
// d'affaires, et le passage est compté sur la carte de fidélité quand il y en a
// une. Une garde exigeait le mot « email » : elle figeait la phrase fausse.
//
// ⚠️ `table` : UNE SALLE NE CLÔTURE PAS DES PERSONNES. Le bloc d'un service
// porte des tables ; « marquer cette personne comme venue » devant une table de
// six est faux deux fois. Absent, le texte d'un cours ne change pas d'un mot.
export function questionSeanceHonoree(nb, { montant = 0, table = false } = {}) {
  const n = Number(nb)
  if (!Number.isFinite(n) || n < 1) return null
  const titre = table
    ? (n === 1 ? 'Marquer cette table comme venue ?' : `Marquer ces ${n} tables comme venues ?`)
    : (n === 1 ? 'Marquer cette personne comme venue ?' : `Marquer ces ${n} personnes comme venues ?`)
  const details = table
    ? 'Une table manquait ? Ferme cette fenêtre et note son absence sur sa ligne : les autres resteront à clôturer.'
    : 'Quelqu’un manquait ? Ferme cette fenêtre et note son absence sur sa ligne : les autres resteront à clôturer.'
  ...
  if (!Number.isFinite(aEncaisser) || aEncaisser <= 0) {
    return {
      titre,
      // ✅ UNE TABLE N'A PAS DE PRIX (Alex, 10/09 au soir) : il n'y a aucun
      // montant à faire entrer dans le chiffre d'affaires, l'addition se règle
      // à la caisse du restaurant. Seul le passage compte.
      message: table
        ? (n === 1
          ? 'Le passage est compté sur la carte de fidélité quand il y en a une. Ce geste ne se défait pas.'
          : 'Chaque passage est compté sur la carte de fidélité quand il y en a une. Ce geste ne se défait pas.')
        : n === 1
        ? 'Le montant entre dans ton chiffre d’affaires, et le passage est compté sur la carte de fidélité quand il y en a une. Ce geste ne se défait pas.'
        : 'Les montants entrent dans ton chiffre d’affaires, et chaque passage est compté sur la carte de fidélité quand il y en a une. Ce geste ne se défait pas.',
      details,
      actions: [
        { valeur: 'honore', ton: 'principal', label: table
          ? (n === 1 ? 'Oui, ils étaient là' : 'Oui, toutes les tables étaient là')
          : (n === 1 ? 'Oui, elle était là' : 'Oui, tout le monde était là') },
        { valeur: 'rien', ton: 'neutre', label: 'Ne rien faire' },
      ],
    }
  }
  ...
  return {
    titre,
    message: table
      ? `Il reste ${somme} à encaisser en tout : le moyen que tu choisis vaut pour tout le service, et se corrige ensuite ligne par ligne si une table a payé autrement.`
      : `Il reste ${somme} à encaisser en tout : le moyen que tu choisis vaut pour tout le cours, et se corrige ensuite ligne par ligne si quelqu’un a payé autrement.`,
    details,
    actions: [
...
export function confirmationSeanceHonoree({ faits = 0, echecs = 0, table = false } = {}) {
  if (faits < 1) return 'Rien n’a pu être enregistré. Vérifie ta connexion et recommence.'
  const qui = table ? ['table', 'tables'] : ['personne', 'personnes']
  const debut = faits === 1
    ? `1 ${qui[0]} est marquée comme venue.`
    : `${faits} ${qui[1]} sont marquées comme venues.`
  if (echecs > 0) {
    return `${debut} En revanche ${echecs === 1 ? '1 ligne n’a pas pu être enregistrée' : `${echecs} lignes n’ont pas pu être enregistrées`} : rouvre ${table ? 'le service' : 'le cours'} pour les reprendre.`
  }
  // ✅ UNE TABLE N'A PAS DE PRIX (Alex, 10/09 au soir) : pas de montant à
  // annoncer dans le chiffre d'affaires.
  return table ? debut : `${debut} Leur montant entre dans ton chiffre d’affaires du jour.`
}
