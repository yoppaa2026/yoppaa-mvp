...
//
// `ancres` : [{ cat, offsetTop }] dans l'ordre d'apparition.
// Rend la catégorie dont le titre est le dernier à avoir passé la ligne de
// lecture, ou la première tant qu'aucune ne l'a franchie.
...
export function categorieAtteinte({ scrollTop = 0, hauteurEntete = 0, ancres = [] } = {}) {
  ...
  let courante = ancres[0]?.cat ?? null
  for (const ancre of ancres) {
    // ⚠️ UNE ANCRE SANS POSITION N'EST PAS UNE ANCRE À ZÉRO. Un élément pas
    // encore monté rendrait `offsetTop` absent : le compter comme 0 le
    // placerait en tête de page et allumerait le mauvais onglet.
    if (!ancre || !Number.isFinite(ancre.offsetTop)) continue
    if (scrollTop >= ancre.offsetTop - hauteurEntete - MARGE_LECTURE) courante = ancre.cat
  }
  return courante
}
