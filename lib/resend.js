  ...
  return layout({
    ...
    body: `
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;background:#fff;border:1px solid ${C.pale};border-radius:12px;overflow:hidden;margin-bottom:14px;">
        ...
        ${lignesAvantages(C, { fidelite_remise, bon_cadeau_montant })}
        ...
        <p style="margin:0 0 8px;font-size:12px;color:#7C2D12;line-height:1.5;">
          Tu peux annuler ton RDV gratuitement jusqu'à <strong>${delai_annulation_heures}h</strong> avant ton créneau. ${
            ...
            (Number(fidelite_remise) > 0 || Number(bon_cadeau_montant) > 0)
              ? 'Ton acompte revient sur ton moyen de paiement en 5 à 10 jours, et ton bon cadeau comme ta récompense retournent chez toi, utilisables tout de suite.'
  ...
  return layout({
    ...
    body: `
      ...
      ${(() => {
        ...
        if (surBon > 0) {
          lignes.push(`<strong>${euros(surBon)}</strong> sont recrédités sur ton bon cadeau, utilisables dès maintenant.`)
        }
  ...
  return layout({
    ...
    body: `
      ...
      ${(() => {
        ...
        if (surBon > 0) quoi.push(`les <strong>${euros(surBon)}</strong> de ton bon cadeau`)
        return `
      <div style="background:#FFF7ED;border-left:4px solid #EA580C;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
      ...
      ${(() => {
        ...
        const lignes = []
        if (surBon > 0) {
          lignes.push(`<strong>${euros(surBon)}</strong> sont recrédités sur ton bon cadeau, utilisables dès maintenant : seule la garantie du commerçant reste acquise.`)
        }
        if (surCarte > 0) {
...
export function emailBonCadeauBeneficiaire({
  ...
  return layout({
    ...
    intro: `<strong>${echapperHtml(beneficiaire_prenom) || 'Hello'}</strong>, ${acheteur_prenom ? `<strong>${echapperHtml(acheteur_prenom)}</strong> t'offre` : 'on t\'offre'} un bon cadeau chez <strong>${echapperHtml(commercant_nom)}</strong> !`,
    body: `
      ${message ? `
...
export function emailBonCadeauAcheteur({
  ...
  return layout({
    ...
    intro: pour_moi
      ? `<strong>${echapperHtml(acheteur_prenom) || 'Merci'}</strong>, ton bon cadeau chez <strong>${echapperHtml(commercant_nom)}</strong> est activé !`
      : `<strong>${echapperHtml(acheteur_prenom) || 'Merci'}</strong>, ton bon cadeau chez <strong>${echapperHtml(commercant_nom)}</strong> vient d'être envoyé à <strong>${echapperHtml(beneficiaire_prenom || beneficiaire_email)}</strong> 🟣`,
    body: pour_moi
      ? blocBonCadeau({ code, montant, commercant_nom, expires_at })
      : `
      <div style="background:#fff;border-radius:12px;padding:16px;border:1px solid ${C.pale};margin-bottom:14px;text-align:center;">
...
export function emailBonCadeauVenduCommercant({ nom_commercant, montant, acheteur_email, pour_moi }) {
  return layout({
    ...
    intro: `Bonne nouvelle <strong>${echapperHtml(nom_commercant)}</strong>, tu viens de vendre un bon cadeau de <strong>${euros(Number(montant))}</strong> !`,
    body: `
