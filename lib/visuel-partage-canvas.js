...
import {
  ...
  pointsDuVisuel, largeurDesPoints, nomFichierVisuel, prixDuVisuel,
} from './visuel-partage'

const POLICE = '"DM Sans", system-ui, Arial, sans-serif'

/** Le pictogramme tracé à la main : aucun fichier à charger. */
...
// 🔴 `tracerSac` A ÉTÉ RETIRÉ LE 05/09, en branchant les vrais boutons de
// partage. Il dessinait la pastille de stock, et le stock ne s'écrit plus sur
// une image : « il en reste 3 » n'est vrai qu'à la seconde du dessin, alors que
// la publication, elle, reste. Une fonction sans appelant est du code mort, et
// la garde qui la surveille est une garde verte qui ne protège rien.
...
export async function construireVisuel(annonce, formatCle = FORMAT_CARRE) {
  ...
  // ── Le prix ──────────────────────────────────────────────────────────────
  if (c.prix !== null) {
    // ⚠️ LE FORMAT VIENT DU MODULE QUI DÉCIDE, il n'est plus écrit ici. La
    // légende du post l'écrit aussi : deux copies auraient fini par afficher
    // « 9 € » sur l'image et « 9,00 € » dans le texte, sur la même publication.
    ctx.font = police(900, F.prix)
    ctx.fillStyle = h.accent
    const txtPrix = prixDuVisuel(c.prix)
    ...
    if (c.prixBarre !== null) {
      ...
      ctx.fillStyle = h.douce
      const txtBarre = prixDuVisuel(c.prixBarre)
  ...
  //
  // ⚠️ ELLES S'ARRÊTENT AU BORD, ELLES NE LE FRANCHISSENT PAS. Leur texte vient
  // du code et non du commerçant, mais il vient de DEUX modules (l'heure de
  // `libelleFenetre`, le reste de `libelleReste`) qui peuvent s'allonger sans
  // que personne ne pense à cette carte. Une pastille coupée par le bord est le
  // débordement qu'Alex a vu sur les premières maquettes : mieux vaut n'en
  // afficher qu'une que d'en montrer une et demie.
  if (c.pastilles.length) {
    ...
    c.pastilles.forEach(p => {
      ...
      tracerHorloge(ctx, x + F.past * 0.8, yIco, tailleIco, h.pastEncre)
