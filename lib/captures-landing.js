...
export const CAPTURES_COMMERCANT = [
  // ⚠️ CELLE-CI A REMPLACÉ UNE MAQUETTE, ET C'EST LA PREMIÈRE FOIS (Alex,
  // 22/09 : « remplace ce visuel par la capture jointe »). Le tableau de bord
  // était dessiné en JSX depuis le début ; il est maintenant montré tel qu'il
  // est, avec ses vrais compteurs et une vraie commande.
  //
  // 🔴 ET ELLE PORTAIT UN NUMÉRO DE GSM ET UNE ADRESSE EMAIL EN CLAIR. Une
  // capture prise en production emporte les données du client qui s'y trouve.
  // Les deux zones sont pixellisées puis floutées par
  // `scripts/extraire-ecran-visuel.mjs`, qui garde la trace de ce qui a été
  // masqué. Le fichier SOURCE ne doit jamais entrer dans le dépôt : il vit
  // dans `public/captures/` le temps de l'extraction, et le `.gitignore` l'y
  // retient. Il a fallu l'y ajouter ce jour-là, il ne couvrait que `.jpeg`.
  //
  // ⚠️ CE QU'ELLE MONTRE ET QU'AUCUN DESSIN NE MONTRAIT : une commande LIÉE À
  // UN RENDEZ-VOUS, avec la phrase qui évite au commerçant de préparer un
  // retrait qui n'aura pas lieu. C'est le genre de détail qui ne s'invente pas
  // et qui fait la différence entre une maquette et une preuve.
  {
    cle: 'dashboard_commandes',
    fichier: 'dashboard-commandes.webp',
    largeur: 760,
    hauteur: 1534,
    alt: 'Le tableau de bord d’un commerce de services : les commandes du jour, le chiffre d’affaires, et une commande liée à un rendez-vous à remettre pendant la prestation.',
    titre: 'Ton comptoir tient dans ta poche',
    legende: 'Les commandes du jour, ce qu’il te reste à préparer, ton chiffre. Et quand un produit se remet pendant un rendez-vous, ton tableau de bord le sait : ton client ne repassera pas le chercher.',
  },
...
export const CAPTURES_YOPPER = [
  ...
  },
  {
    ...
    // ⚠️ LA DERNIÈRE PHRASE VIENT DE LA MAQUETTE RETIRÉE LE 22/09. Elle était
    // la SEULE ligne de toute la landing à annoncer le rappel, et le délai a été
    // revérifié dans le code avant de la déplacer : `RAPPEL_RDV_MIN = 60`.
    legende: 'Le salon est fermé, son agenda ne l’est pas. Tu choisis ta prestation, la personne si tu en as une, et l’heure qui t’arrange. Le rappel arrive une heure avant.',
  },
  {
    cle: 'yopper_invendus',
