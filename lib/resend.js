...
// « 0,00 € de récompense ».
export function ligneAvantagesEmail({ fidelite_remise, bon_cadeau_montant, commercant_categorie = null } = {}) {
  const montant = (v) => {
    const n = Number(v)
  ...
  if (bon) bouts.push(`<strong>${euros(bon)}</strong> en ${libelleBon(commercant_categorie)}`)
...
export function emailRdvConfirme({
  ...
  prix_estime, acompte_paye, acompte_montant, fidelite_remise = 0, bon_cadeau_montant = 0, delai_annulation_heures = 24,
        ...
        <p style="margin:0 0 8px;font-size:12px;color:#7C2D12;line-height:1.5;">
          Tu peux annuler ton RDV gratuitement jusqu'à <strong>${delai_annulation_heures}h</strong> avant ton créneau. ${
            ...
            (Number(fidelite_remise) > 0 || Number(bon_cadeau_montant) > 0)
              ? `Ton acompte revient sur ton moyen de paiement en 5 à 10 jours, et ton ${libelleBon(commercant_categorie)} comme ta récompense retournent chez toi, utilisables tout de suite.`
  ...
  return layout({
    ...
    body: `
      ...
      ${(() => {
        ...
        if (surBon > 0) {
          lignes.push(`<strong>${euros(surBon)}</strong> sont recrédités sur ton ${libelleBon(commercant_categorie)}, utilisables dès maintenant.`)
        }
  ...
  return layout({
    ...
    body: `
      ...
      ${(() => {
        ...
        if (surBon > 0) quoi.push(`les <strong>${euros(surBon)}</strong> de ton ${libelleBon(commercant_categorie)}`)
        return `
      <div style="background:#FFF7ED;border-left:4px solid #EA580C;border-radius:10px;padding:14px 16px;margin-bottom:14px;">
      ...
      ${(() => {
        ...
        const lignes = []
        if (surBon > 0) {
          lignes.push(`<strong>${euros(surBon)}</strong> sont recrédités sur ton ${libelleBon(commercant_categorie)}, utilisables dès maintenant : seule la garantie du commerçant reste acquise.`)
        }
        if (surCarte > 0) {
...
export function emailCommandeAnnuleeYopper({
  ...
  if (bonOk) bouts.push(`<strong>${euros(bonMnt)}</strong> avec ton ${libelleBon(commercant_categorie)}`)
...
export function emailCommandeAnnuleeCommercant({
  ...
  const avantages = ligneAvantagesEmail({ fidelite_remise, bon_cadeau_montant, commercant_categorie })
  return layout({
    audience: 'commercant',
