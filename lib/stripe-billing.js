...
// ⚠️ UNE SEULE FAÇON DE DIRE « C'EST LA MÊME ADRESSE », partagée avec l'écran
// qui change l'email. Deux comparaisons écrites séparément finissent toujours
// par diverger sur la casse ou les espaces, et ici la divergence voudrait dire
// « on pousse chez Stripe alors que rien n'a changé », ou l'inverse.
import { memeEmail } from './messages-auth'
...
// 🔴 STRIPE GARDE SA PROPRE COPIE DE L'EMAIL, ET ELLE NE SUIT PAS TOUTE SEULE.
//
// Le Customer est créé une fois avec `commercant.email`, et plus rien ne
// remonte. `/api/dashboard/facturation` poussait déjà le nom, l'adresse et les
// métadonnées : l'email avait été oublié. Tant que personne ne pouvait changer
// son email, ça ne se voyait pas. Depuis le 23/09 il le peut, et sans cette
// fonction sa facture porterait l'ancienne adresse, pour toujours, exactement
// comme l'adresse postale avant le 22/09.
//
// 🔴 ELLE NE PEUT JAMAIS FAIRE ÉCHOUER CE QUI L'APPELLE. Elle est branchée sur
// le chemin d'un paiement : une exception ici casserait un encaissement pour
// une question d'adresse email. Elle capture tout, journalise, et rend un état.
//
// ⚠️ ELLE N'APPELLE STRIPE QUE SI C'EST NÉCESSAIRE : avec un customer déjà en
// main, elle compare sans un seul appel réseau, et ne pousse que si ça diffère.
//
// Rend : 'sans objet' | 'à jour' | 'poussé' | 'en retard'.
export async function synchroniserEmailCustomer(commercant, customerConnu = null) {
  // ⚠️ PAS DE CUSTOMER, PAS D'ERREUR. Un commerçant sans abonnement n'a rien
  // chez Stripe : son Customer naîtra plus tard avec la bonne adresse.
  if (!commercant?.stripe_customer_id || !commercant?.email) return 'sans objet'
  try {
    // 🔴 ON NE RÉCLAME STRIPE QU'AU MOMENT DE S'EN SERVIR. Première version :
    // `requireStripe()` en tête de fonction. Elle LÈVE quand la clé manque,
    // donc même le cas « rien à faire, l'email est déjà bon » partait dans le
    // gestionnaire d'erreur et rendait « en retard ». Deux gardes du banc ont
    // rougi là-dessus : un chemin qui n'a besoin de rien ne doit rien exiger.
    let customer = customerConnu
    if (!customer || customer.id !== commercant.stripe_customer_id) {
      customer = await requireStripe().customers.retrieve(commercant.stripe_customer_id)
    }
    if (!customer || customer.deleted) return 'sans objet'
    if (memeEmail(customer.email, commercant.email)) return 'à jour'
    await requireStripe().customers.update(commercant.stripe_customer_id, { email: commercant.email })
    return 'poussé'
  } catch (e) {
    // 🔴 ON LE DIT, ON NE LÈVE PAS. Un silence ici recréerait le défaut qu'on
    // vient de fermer : la base à jour, Stripe en retard, et personne au
    // courant jusqu'à la facture.
    console.error('[stripe-billing] email non poussé chez Stripe', {
  ...
  if (commercant.stripe_customer_id) {
    try {
      ...
      if (customer && !customer.deleted) {
        // ⚠️ LE CUSTOMER EST DÉJÀ EN MAIN : la comparaison ne coûte aucun appel,
        // et on ne pousse que si l'email a bougé. C'est le passage obligé de
        // tous les chemins d'achat, donc le meilleur endroit pour rattraper un
        // changement d'email fait entre-temps.
        await synchroniserEmailCustomer(commercant, customer)
        return customer
      }
    } catch (e) {
