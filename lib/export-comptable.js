...
export function construireLignes({ commandes = [], rdvs = [], abonnements = [], tauxDefaut = null, articlesParId = {} }) {
  ...
  for (const r of rdvs) {
    ...
    if (MODES_QUI_ENCAISSENT.includes(String(r.encaisse_mode || '')) && encaisse > 0) {
      ...
      lignes.push({
        date: String(r.encaisse_le || r.date_rdv || '').slice(0, 10),
        type: 'Solde RDV',
        canal: 'Rendez-vous',
        regime: 'emporter',
  ...
  for (const a of abonnements) {
    ...
    const date = String(a.paye_le || '').slice(0, 10)
...
export function csvJournal({ lignes, commercant, du, au, assujetti = true, avertissementTaux = false }) {
  ...
  out.push(ligneCsv([
    ...
    'Encaisse en ligne', 'Encaisse au comptoir', 'Dont terminal', 'Dont especes',
    'Paye par bon cadeau', 'Frais Stripe', 'Net Stripe',
  ]))

  const totaux = { nb: 0, total: 0, enLigne: 0, comptoir: 0, terminal: 0, especes: 0, bonCadeau: 0, frais: 0, net: 0, base: {}, tva: {} }
  for (const j of jours) {
    ...
    cells.push(nombre(j.enLigne), nombre(j.comptoir), nombre(j.terminal), nombre(j.especes),
  ...
  fin.push(nombre(totaux.enLigne), nombre(totaux.comptoir), nombre(totaux.terminal), nombre(totaux.especes),
    nombre(totaux.bonCadeau), nombre(totaux.frais), nombre(totaux.net))
  out.push('')
