...
// ⚠️ LE MODULE BAS DES TABLES : il n'importe que `deplacement-rdv`, aucun cycle.
...
export function remiseSurPrestation(prestation, deals = [], jourISO) {
  // ✅ UNE TABLE N'A PAS DE PRIX (Alex, 10/09), donc rien à remiser : un deal
  // « toutes mes prestations » ne lui fabrique pas un tarif barré.
  if (!prestation || estParCouverts(prestation)) return null
...
export function prixEffectifPrestation(prestation, deals = [], jourISO) {
  // ✅ UNE TABLE N'A PAS DE PRIX (Alex, 10/09 au soir). C'est ICI que la règle
  // mord, parce que tout ce qui parle d'argent sur un rendez-vous passe par
  // cette fonction : la fiche, l'acompte, la récompense, le bon cadeau, et les
  // trois routes serveur. Un prix resté en base sur une table ne décide plus
  // de rien.
