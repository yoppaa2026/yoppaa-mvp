...
//
// ⚠️ 🔴 LE RELEVÉ DU COMPTOIR S'EFFACE AVEC LA REMISE, et j'avais tranché
// L'INVERSE le 22/08. Alex l'a vu à l'essai : la commande revenait dans la
// liste « à remettre » en restant marquée PAYÉE.
//
// Mon raisonnement d'alors — « si le commerçant a encaissé, l'argent est
// réellement entré » — supposait deux gestes séparés. Il n'y en a qu'UN :
// depuis le 17/08, la fenêtre d'encaissement s'ouvre AU MOMENT de la remise,
// justement parce que « honoré » et « payé » étaient le même clic. On ne peut
// donc pas avoir encaissé sans avoir marqué remis. Si la remise était un clic
// raté, la réponse sur le moyen de paiement l'était aussi : le client n'était
// même pas là.
//
// ⚠️ ET LE LAISSER EN PLACE COÛTAIT PLUS CHER QUE DE L'EFFACER :
//   • la fenêtre d'encaissement ne se rouvre QUE si `encaisse_mode` est vide.
//     Au vrai retrait, le commerçant ne se verrait donc jamais reposer la
//     question, et le moyen resterait celui saisi par erreur ;
//   • `encaisse_le` daterait la vente du jour du clic raté ;
//   • une vente qui n'a pas eu lieu entrerait au journal comptable.
//
// ⚠️ ON N'EFFACE QUE LE RELEVÉ DU COMPTOIR. `paye_en_ligne` n'est JAMAIS
// touché : l'argent est chez Stripe, il ne doit rien à ce clic. Une commande
// payée en ligne dont le commerçant a encaissé un reste au comptoir perd ce
// reste, et garde son paiement en ligne.
...
export function retourArriereAutorise(commande) {
  ...
  // ⚠️ `'rien'` COMPTE COMME UN RELEVÉ : c'est un impayé assumé, une réponse à
  // la question. Elle a été donnée sur le même clic raté, elle s'efface avec.
  const releveAuComptoir = !!commande.encaisse_mode
  return {
    versStatut: 'pret',
    ...
    effaceEncaissement: releveAuComptoir,
    ...
    // découvre pas après coup ; elle ne s'affiche pas non plus quand il n'y a
    // rien à effacer, sans quoi elle inquiéterait pour rien.
    aide: releveAuComptoir
      ? 'La commande revient dans ta liste, comme si elle n’avait pas été remise. Ce que tu as relevé au comptoir est effacé : la question du paiement se reposera à la vraie remise.'
      : 'La commande revient dans ta liste, comme si elle n’avait pas été remise.',
  }
}
...
/**
 * SUR QUOI LE TABLEAU DE BORD S'OUVRE.
 *
 * 🔴 TOUT LE MONDE OUVRAIT SUR « COMMANDES » (Alex, 08/09 : « normal pour les
 * alims et détails, peut-être un peu moins pour les services »). Un centre de
 * yoga tombait donc chaque matin sur un écran vide qui ne le concerne pas,
 * pendant que sa journée entière est dans son agenda.
 *
 * ⚠️ ET LA RÈGLE NE CHANGE PAS SOUS SES DOIGTS. On ouvre sur ce que le commerce
 * REÇOIT, pas sur « l'écran qui a quelque chose en attente » : un tableau de
 * bord qui n'ouvre pas toujours au même endroit ne s'apprend jamais.
 *
 * ⚠️ UN SALON QUI VEND AUSSI DES PRODUITS OUVRE SUR SON AGENDA. C'est sa
 * journée : les quelques produits qu'il vend ne la commandent pas.
 *
 * `commandesVisibles` dit si l'onglet des commandes existe pour ce commerce :
 * une vitrine sans le module de commande ne l'a pas, et l'ouvrir dessus
 * afficherait un onglet que la barre ne montre même pas.
 */
export function ongletDouverture(commercant, { commandesVisibles = true } = {}) {
  if (!commercant) return 'commandes'
  const agenda = commercant.rdv_actif === true
  if (agenda && (commercant.categorie === 'vitrine' || !commandesVisibles)) return 'rdv'
  // Ni commandes ni agenda : on l'emmène là où il y a quelque chose à faire,
  // plutôt que sur un onglet vide ou caché.
  if (!commandesVisibles) return 'config'
  return 'commandes'
}
