...
function logoEmailDark(wordmarkSize = 32) {
  const dotBase   = Math.round(wordmarkSize * 0.254)
  const dotMini   = Math.round(dotBase * 0.55)
  const dotGap    = Math.round(dotBase * 0.55)
  const dotOffset = Math.round(dotBase * 0.4)
  ...
  return `
    <p style="margin:0;font-family:${fontStack};font-weight:800;font-size:${wordmarkSize}px;letter-spacing:-0.05em;line-height:1;">
      <span style="color:#FFFFFF;">yo</span><span style="color:${C.light};">pp</span><span style="color:${C.mid};">aa</span>
    </p>
    <div style="margin-top:${wordmarkToDots}px;display:inline-flex;gap:${dotGap}px;align-items:flex-start;">
      <span style="display:inline-block;width:${dotBase}px;height:${dotBase}px;border-radius:50%;background:#FFFFFF;"></span>
      <span style="display:inline-block;width:${dotMini}px;height:${dotMini}px;border-radius:50%;background:${C.light};margin-top:${dotOffset}px;"></span>
      <span style="display:inline-block;width:${dotBase}px;height:${dotBase}px;border-radius:50%;background:${C.light};margin-top:${dotOffset}px;"></span>
      <span style="display:inline-block;width:${dotMini}px;height:${dotMini}px;border-radius:50%;background:${C.mid};margin-top:${dotOffset}px;"></span>
      <span style="display:inline-block;width:${dotBase}px;height:${dotBase}px;border-radius:50%;background:${C.mid};"></span>
    </div>`
}
    ...
    body: `
      ...
      <div style="background:#FFFBEB;border-left:4px solid #F59E0B;border-radius:10px;padding:12px 14px;margin-top:14px;">
        ...
        <p style="margin:0 0 8px;font-size:12px;color:#7C2D12;line-height:1.5;">
          Tu peux annuler ta commande gratuitement jusqu'à <strong>${delai_annulation_heures}h</strong> avant ton créneau de retrait. Remboursement automatique en 5 à 10 jours.
        </p>
...
export function emailCommandeAnnuleeYopper({
  ...
  refund_ok = true, refund_manuel = false, paye_en_ligne = true,
}) {
  return layout({
    audience: 'yopper',
    commercantNom: commercant_nom,
