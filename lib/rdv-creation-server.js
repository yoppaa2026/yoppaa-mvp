...
import { enModeInventaire, quantiteDe, occupationParFormat } from './inventaire-salle'
...
export async function creerReservationRdv(db, {
  ...
  if (estParCouverts(prestation)) {
    ...
    if (enModeInventaire(formatsTable)) {
      // ⚠️ ON VÉRIFIE LE FORMAT DEMANDÉ, PAS ENCORE LE MEILLEUR. En 2a le client
      // choisit toujours sa table dans la liste : lui en donner une autre que
      // celle qu'il a désignée serait un changement de parcours, pas un calcul
      // de disponibilité. `formatLibrePour` attend le lot 2b, quand il dira
      // seulement combien ils sont.
      const total = quantiteDe(prestation)
      const pris = occupationParFormat(autour || [], debutMin, finMin).get(String(prestation.id)) || 0
      if (total !== null && pris >= total) {
        return { ok: false, code: 'salle_complete', restants: 0 }
      }
    } else {
  ...
  const payload = {
    ...
    // vérifier du tout.
    duree_minutes: dureeSelonCouverts(prestation, couvertsRetenus),
    heure_fin: minutesToTime(timeToMinutes(heure) + dureeSelonCouverts(prestation, couvertsRetenus)),
  }
