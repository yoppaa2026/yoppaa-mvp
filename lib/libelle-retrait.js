...
export function libelleRetrait(commande, creneau = null, { court = false } = {}) {
  ...
  if (commande.rdv_reservation_id) {
    // ⚠️ LA COMMANDE PORTE DÉJÀ SON COMMERÇANT, inutile d'ajouter un paramètre :
    // `etatPaiementClient` lit la même catégorie au même endroit. Sans la
    // jointure, le vocabulaire du rendez-vous s'applique, donc rien ne change.
    const mots = motsReservation(commande.commercant)
    return jour
      ? `À emporter le ${jour}, ${mots.avecLaSienne}`
      : `À emporter ${mots.avecLaSienne}`
  }
