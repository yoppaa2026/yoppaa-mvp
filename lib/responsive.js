...
// ─── QUELLE CATÉGORIE ON EST EN TRAIN DE LIRE ───────────────────────────────
//
// La barre de catégories d'une fiche allume l'onglet de la section qu'on a
// sous les yeux. La décision vit ici, en fonction pure, parce que le composant
// ne doit rien faire d'autre que donner les mesures du navigateur.
//
// ⚠️ ELLE EST APPELÉE À CHAQUE ÉVÉNEMENT DE DÉFILEMENT. Elle ne doit donc rien
// allouer d'inutile et rien mesurer elle-même : les ancres arrivent déjà lues.
//
// `ancres` : [{ cat, offsetTop }], dans n'importe quel ordre.
// Rend la catégorie dont le titre est le dernier à avoir passé la ligne de
// lecture, ou la plus haute tant qu'aucune ne l'a franchie.
export const MARGE_LECTURE = 80

// 🔴 « TOUT EST DÉCALÉ » (Alex, 17/09). L'onglet « Frites » s'allumait au-dessus
// des desserts, et pas seulement sur une catégorie : toute la barre était
// décalée par rapport aux sections.
//
// ⚠️ LA CAUSE N'ÉTAIT PAS LE CALCUL, C'ÉTAIT L'ORDRE DU TABLEAU. Cette fonction
// prenait « la DERNIÈRE ancre franchie », c'est-à-dire la dernière **du
// tableau** — ce qui n'a de sens que si le tableau suit l'ordre de la page. Or
// il vient d'un `Object.entries(catRefs.current)`, et un objet JavaScript garde
// l'ordre de la PREMIÈRE insertion de chaque clé. Les refs se posent au premier
// rendu, quand les articles arrivent encore : cet ordre-là se fige, et les
// rendus suivants, eux, réordonnent les sections sans jamais le corriger.
//
// ✅ ON NE DÉPEND PLUS D'UN ORDRE QU'ON NE MAÎTRISE PAS. La seule chose qui
// décrit la page, c'est `offsetTop` : on trie dessus. La fonction devient vraie
// quel que soit l'ordre qu'on lui donne, ce qui est exactement ce qu'une règle
// doit être.
export function categorieAtteinte({ scrollTop = 0, hauteurEntete = 0, ancres = [] } = {}) {
  if (!Array.isArray(ancres) || ancres.length === 0) return null

  // ⚠️ UNE ANCRE SANS POSITION N'EST PAS UNE ANCRE À ZÉRO. Un élément pas encore
  // monté rendrait `offsetTop` absent : le compter comme 0 le placerait en tête
  // de page et allumerait le mauvais onglet. On l'écarte, on ne la corrige pas.
  const placees = ancres
    .filter(a => a && Number.isFinite(a.offsetTop))
    .sort((a, b) => a.offsetTop - b.offsetTop)
  if (placees.length === 0) return null

  const ligne = scrollTop + hauteurEntete + MARGE_LECTURE
  let courante = placees[0].cat
  for (const ancre of placees) {
    if (ligne >= ancre.offsetTop) courante = ancre.cat
  }
  return courante
}

// L'ombre sous la barre n'apparaît qu'une fois l'en-tête dépassé : avant, la
// barre n'est posée sur rien et l'ombre flotterait sans raison.
export function barreDetachee({ scrollTop = 0, hauteurEntete = 0 } = {}) {
  return scrollTop > hauteurEntete - 60
}
