...
// ⚠️ LE DIAGNOSTIC DOIT TENIR SUR UNE LIGNE D'ÉCRAN. Une adresse de fonte fait
// facilement 90 caractères : entière, elle noierait le reste du message.
const abreger = (u) => {
  const s = String(u || '')
  return s.length <= 52 ? s : `${s.slice(0, 24)}…${s.slice(-24)}`
}
...
export async function policesEmbarquees(familleRecherchee) {
  ...
  for (const r of retenues) {
    ...
    // 🔴 UNE ADRESSE RELATIVE SE RÉSOUT CONTRE LA FEUILLE DE STYLE, PAS CONTRE
    // LA PAGE. C'est le défaut vu par Alex le 29/08 : dix-sept règles trouvées,
    // et un 404 sur chacune. Le CSS de `next/font` écrit `../media/x.woff2`,
    // relatif à `/_next/static/css/…`. Résolu contre `/brand-kit/commercant`,
    // ça donnait `/media/x.woff2`, qui n'existe pas.
    //
    // ⚠️ On garde la page comme SECOND essai : une feuille posée en ligne n'a
    // pas de `href`, et il faut bien une base.
    const base = r.parentStyleSheet?.href || location.href
    const candidats = [new URL(adresse, base).href]
    const viaPage = new URL(adresse, location.href).href
    if (viaPage !== candidats[0]) candidats.push(viaPage)

    try {
      let rep = null
      let dernierStatut = 0
      for (const u of candidats) {
        // ⚠️ `fetch` NE REJETTE PAS SUR UN CODE HTTP. Sans cette lecture, un
        // 404 produirait une fonte faite de la page d'erreur, et le SVG
        // s'ouvrirait en police de substitution sans que rien ne le signale.
        const essai = await fetch(u)
        if (essai.ok) { rep = essai; break }
        dernierStatut = essai.status
      }
      if (!rep) { echecs.push(`HTTP ${dernierStatut} sur ${abreger(candidats[0])}`); continue }
...
// Rend une toile peinte, dont on tire ensuite un PNG ou une page de PDF.
// ⚠️ UNE SEULE RASTÉRISATION POUR LES DEUX SORTIES : deux chemins de dessin
// finiraient par diverger, et le PDF ne ressemblerait plus au PNG.
function svgEnToile(svgTexte, largeurMm, hauteurMm, dpi) {
...
export async function svgEnPng(svgTexte, largeurMm, hauteurMm, dpi = DPI_IMPRESSION) {
  const toile = await svgEnToile(svgTexte, largeurMm, hauteurMm, dpi)
  return new Promise((resolve, reject) => {
    toile.toBlob(
...
// ─── Les deux faces dans un seul PDF ────────────────────────────────────────
//
// ⚠️ POURQUOI UN PDF ET PAS DEUX IMAGES DANS UN ZIP. C'est le format qu'un
// imprimeur attend pour un recto/verso : deux pages A4 dans l'ordre, un seul
// fichier à déposer, et le pilote d'impression sait faire le retournement.
// Deux PNG séparés obligent à les rouvrir un par un et à espérer le bon ordre.
//
// ⚠️ `jspdf` EST CHARGÉ À LA DEMANDE. Il pèse plusieurs centaines de kilo-
// octets : au sommet du fichier, il partirait dans le bundle de la page pour
// tout le monde, y compris ceux qui ne cliqueront jamais.
export async function feuillesEnPdf(noeuds, largeurMm, hauteurMm, dpi = DPI_IMPRESSION) {
  const utiles = (noeuds || []).filter(Boolean)
  if (utiles.length === 0) throw new Error('Aucune page à assembler.')

  const { jsPDF } = await import('jspdf')
  const doc = new jsPDF({ unit: 'mm', format: [largeurMm, hauteurMm], orientation: 'portrait' })

  for (let i = 0; i < utiles.length; i++) {
    const svg = await feuilleEnSvg(utiles[i], largeurMm, hauteurMm)
    const toile = await svgEnToile(svg, largeurMm, hauteurMm, dpi)
    // ⚠️ LA PAGE N'EST AJOUTÉE QU'À PARTIR DE LA DEUXIÈME. `jsPDF` en crée
    // déjà une à la construction : un `addPage()` inconditionnel donnerait un
    // PDF dont la PREMIÈRE PAGE EST BLANCHE, et ça ne se voit qu'à l'impression.
    if (i > 0) doc.addPage([largeurMm, hauteurMm], 'portrait')
    doc.addImage(toile.toDataURL('image/png'), 'PNG', 0, 0, largeurMm, hauteurMm)
  }
