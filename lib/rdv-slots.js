...
export function ajusterPlagePourJour(plage, horaireJour) {
  ...
  if (!(f > d)) return { statut: 'ignoree', raison: 'heures_invalides', debut: null, fin: null }

  // ⚠️ HORAIRES INCONNUS : ON NE TOUCHE À RIEN. Ajuster sur une ignorance
  // raccourcirait des plages parfaitement valables.
  if (!horaireJour) return { statut: 'inchangee', raison: null, debut: minutesToTime(d), fin: minutesToTime(f) }
  if (horaireJour.ouvert === false) return { statut: 'ignoree', raison: 'jour_ferme', debut: null, fin: null }
  ...
  if (plages.length === 0) return { statut: 'inchangee', raison: null, debut: minutesToTime(d), fin: minutesToTime(f) }

  // On retient l'ouverture qui recouvre le plus : sur un commerce à deux
  // services, une plage à cheval appartient à celui où elle tient le mieux.
  let meilleur = null
  for (const [a, b] of plages) {
    const debut = Math.max(d, a)
    const fin = Math.min(f, b)
    if (fin <= debut) continue
    if (!meilleur || (fin - debut) > (meilleur[1] - meilleur[0])) meilleur = [debut, fin]
  }
  ...
  const [debut, fin] = meilleur
  return {
    statut: (debut === d && fin === f) ? 'inchangee' : 'raccourcie',
    raison: null,
