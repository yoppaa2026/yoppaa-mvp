...
import { libelleLancement, phraseEssai } from './lancement'
import { FRAIS_STRIPE_TEXTE } from './frais-paiement'
...
<body style="margin:0;padding:0;background:${C.bg};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:${C.ink};">
  <div style="max-width:560px;margin:0 auto;padding:24px 16px;">
    <div style="background-color:${C.panel};background-image:linear-gradient(135deg,${C.panel} 0%,#2D0F6B 60%,${C.ink} 100%);border-radius:18px 18px 0 0;padding:32px 28px 26px;">
      ${logoEmailDark(36)}
    ...
    <div style="background:#fff;border-radius:0 0 18px 18px;padding:30px 28px;border:1px solid ${C.pale};border-top:none;">
      ...
      ${ctaUrl && ctaLabel ? `
        <p style="margin:26px 0 0;text-align:center;">
          <a href="${ctaUrl}" style="display:inline-block;padding:14px 30px;background-color:${C.panel};background-image:linear-gradient(135deg,${C.panel},${C.main});color:#fff;text-decoration:none;border-radius:100px;font-weight:800;font-size:15px;letter-spacing:-0.2px;">
            ${ctaLabel} →
          </a>
        </p>
      ` : ''}
...
export function emailMerciPreinscription({ mode_landing = 'teasing', type_utilisateur = 'curieux', slug_kit = null } = {}) {
  ...
  const kitBlock = slug_kit ? `
        <div style="margin:24px 0 0;padding:18px 18px 20px;border-radius:14px;background:${C.pale};border:1px solid ${C.light};">
          ...
          <a href="https://www.yoppaa.app/kit/${slug_kit}" style="display:inline-block;padding:12px 26px;background-color:${C.panel};background-image:linear-gradient(135deg,${C.panel},${C.main});color:#fff;text-decoration:none;border-radius:100px;font-weight:800;font-size:14px;">
            Ouvrir mon kit →
          </a>
        </div>` : ''
  ...
  // PASSE ENSUITE (l'ouverture publique, cf. lib/lancement.js), ce que la personne pourra
  ...
  const encartPartage = `
        <div style="margin:24px 0 0;padding:18px 18px 20px;border-radius:14px;background:${C.bg};border:1px solid ${C.pale};">
          ...
          <a href="https://www.yoppaa.app" style="display:inline-block;padding:12px 26px;background-color:${C.panel};background-image:linear-gradient(135deg,${C.panel},${C.main});color:#fff;text-decoration:none;border-radius:100px;font-weight:800;font-size:14px;">
            Partager Yoppaa →
          </a>
        </div>`
  ...
  return layout({
    ...
    body: `
      ...
      <div style="margin:0 0 18px;padding:16px 18px;border-radius:14px;background:${C.bg};border:1px solid ${C.pale};">
        ...
        <p style="margin:0 0 6px;font-size:13.5px;line-height:1.6;color:${C.ink};">Yoppaa ne prend aucune commission sur tes ventes, une formule gratuite pour toujours, et ${phraseEssai()} sur les formules payantes.</p>
        <p style="margin:0;font-size:12.5px;line-height:1.6;color:${C.muted};">Sur les paiements en ligne, seuls les frais de Stripe, notre prestataire de paiement, s'appliquent : ${FRAIS_STRIPE_TEXTE}.</p>
        ` : `
...
export function emailRevealLaunch() {
  return layout({
    ...
    body: `
      ...
      <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${C.ink};">
        Sans commission Yoppaa. Belge. Bootstrap. Pour ton quartier.
      </p>
      <p style="margin:0;font-size:15px;line-height:1.65;color:${C.ink};">
