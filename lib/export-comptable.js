...
export function construireLignes({ commandes = [], rdvs = [], abonnements = [], tauxDefaut = null, articlesParId = {} }) {
  ...
  for (const c of commandes) {
    ...
    lignes.push({
      ...
      bonCadeau: parBon,
      fraisStripe: c.stripe_frais == null ? null : arrondi(c.stripe_frais),
  ...
  for (const r of rdvs) {
    ...
    lignes.push({
      ...
      bonCadeau: 0,
      fraisStripe: r.stripe_frais == null ? null : arrondi(r.stripe_frais),
  ...
  for (const a of abonnements) {
    ...
    lignes.push({
      ...
      bonCadeau: 0,
      fraisStripe: a.stripe_frais == null ? null : arrondi(a.stripe_frais),
...
export function csvDetail({ lignes, commercant, du, au, assujetti = true, avertissementTaux = false }) {
  ...
  for (const l of lignes) {
    ...
    cells.push(nombre(l.enLigne), nombre(l.comptoir), nombre(l.bonCadeau), l.fraisStripe == null ? '' : nombre(l.fraisStripe), l.netStripe == null ? '' : nombre(l.netStripe))
    out.push(ligneCsv(cells))
  }
