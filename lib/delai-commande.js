...
export function delaiDuPanier(lignes) {
  ...
  for (const ligne of liste) {
    ...
    if (d > minutes) {
      minutes = d
      // ⚠️ DEUX NOMS DE CHAMP, ET C'EST DÉLIBÉRÉ. La ligne du panier porte
      // `nom` ; la ligne construite par le serveur porte `article_nom`, parce
      // qu'elle décrit une ligne de commande et pas un article. Ajouter un
      // alias côté serveur fabriquerait deux sources de vérité pour la même
      // chaîne ; le module lit donc les deux formes, une fois, ici.
      nom = ligne?.nom || ligne?.article_nom || null
    }
  }
  return { minutes, nom }
}
...
//
// ⚠️ IL NE SAISIT PAS DES MINUTES. Un boulanger pense « 48 h », pas « 2880 ».
// Un champ libre en minutes est du vocabulaire de programmeur, et il autorise
// les valeurs absurdes : 20 000 minutes rendraient l'article commandable par
// personne, en silence, puisque les deux calculs n'explorent que quatorze
// jours. Une liste ferme la porte à la faute de frappe.
//
// ═══════════════════════════════════════════════════════════════════════════
// 🔴 QUATRE CHOIX, ET PAS UN DE PLUS. ARBITRAGE D'ALEX, 04/09.
//
// Ma première liste proposait aussi 30 min, 1 h, 2 h et 4 h. Elle recréait
// EXACTEMENT le défaut qu'on venait de retirer le matin même : deux réglages
// voisins qui semblent dire la même chose, dont un seul agit vraiment.
//
// Les deux notions ne se recouvrent pas, elles se partagent le temps :
//
//   • LA CLÔTURE DU CRÉNEAU, en heures, réglée sur la ligne de créneau, vaut
//     pour TOUT le catalogue. Elle dit « laisse-moi le temps de préparer ».
//     C'est elle qui fait marcher le sandwich commandé à 11 h pour midi, et
//     elle est en production depuis le 09/08.
//
//   • LE DÉLAI DE L'ARTICLE, en JOURS, vaut pour CET article seul. Il dit
//     « celui-là, je dois le produire ». La tarte, le gâteau d'anniversaire,
//     le plateau du traiteur.
//
// Mettre « 2 h » sur un article, c'est faire le travail de la clôture au
// mauvais endroit, et fabriquer deux sources de vérité pour une seule idée.
// ═══════════════════════════════════════════════════════════════════════════
export const DELAIS_PROPOSES = [0, 1440, 2880, 4320]

/**
 * Les choix à proposer, la valeur actuelle comprise.
 *
 * 🔴 UNE VALEUR HORS LISTE NE DOIT JAMAIS DISPARAÎTRE EN SILENCE. Un article
 * réglé à 36 h, ouvert dans le formulaire, verrait la liste retomber sur le
 * premier choix : le commerçant enregistrerait son prix et perdrait son délai
 * sans qu'aucun écran ne le lui dise. On l'ajoute donc à la liste, à sa place.
 */
export function choixDeDelai(actuel) {
  const m = Number(actuel)
  const liste = [...DELAIS_PROPOSES]
  if (Number.isFinite(m) && m > 0 && !liste.includes(m)) liste.push(m)
  return liste.sort((a, b) => a - b)
}

/** Ce que le commerçant lit dans la liste déroulante. */
export function libelleChoixDelai(minutes) {
  const duree = libelleDuree(minutes)
  return duree ? `Commande ${duree} à l'avance` : 'Aucun délai, disponible tout de suite'
}
