...
export { FUSEAU, heureNormalisee, minutesLocales } from './heure-belge.js'
import { minutesLocales } from './heure-belge.js'
...
// ─── COMBIEN IL EN RESTE, ET CE QU'IL FAUT ÉCRIRE ───────────────────────────
//
// 🔴 LE PIÈGE QUI AURAIT CASSÉ TOUTE LA FONCTION EN SILENCE.
//
// `articles.stock_jour` n'est PAS le stock restant : c'est le stock du DÉBUT DE
// JOURNÉE. Le serveur calcule ensuite (voir `lib/lignes-commande.js`) :
//
//     disponible = stock du jour − déjà commandé aujourd'hui − réservations
//
// Le boulanger qui a mis 20 ce matin, en a vendu 14, et déclare « il m'en reste
// 6 » verrait donc écrire 6, et le serveur calculerait 6 − 14 = **négatif**.
// Son invendu ne serait commandable par personne, et RIEN ne le lui dirait.
//
// ⚠️ CE QU'IL DÉCLARE EST UN RESTE. CE QU'IL FAUT ÉCRIRE EST UN TOTAL.
//
// ⚠️ ET LE CALCUL VIT CÔTÉ SERVEUR, jamais dans l'écran : « l'écran calcule, le
// serveur décide » est le méta-défaut de ce projet, relevé quatre fois.
export function stockJourAEcrire(dejaConsomme, resteAnnonce) {
  // 🔴 `Number(null)` VAUT ZÉRO, ET C'EST LE PIÈGE QUI M'A REPRIS ICI MÊME.
  // Un « déjà consommé » absent serait devenu zéro, on aurait écrit un stock
  // trop bas, et l'offre aurait été invisible SANS QUE RIEN NE LE DISE. On
  // écarte les deux formes de l'absence AVANT de convertir. Sixième fois que ce
  // piège se présente sur ce projet.
  const absent = (v) => v === null || v === undefined || v === ''
  if (absent(dejaConsomme) || absent(resteAnnonce)) return null
  const deja = Number(dejaConsomme)
  const reste = Number(resteAnnonce)
  if (!Number.isFinite(reste) || reste <= 0) return null
  if (!Number.isFinite(deja) || deja < 0) return null
  return Math.round(deja) + Math.round(reste)
}
