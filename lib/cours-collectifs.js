...
//
// Le repas se règle à la caisse du restaurant, pas dans Yoppaa. Une table qui
// porte un prix le fait payer en ligne, en réclame un acompte en pourcentage,
// accepte un bon cadeau ou une récompense dessus, et l'agenda annonce « À
// payer » au restaurateur sur chaque table.
//
// ⚠️ LA RÈGLE S'APPLIQUE À LA LECTURE, pas seulement à la saisie : une table
// réglée avant cette décision garde un prix en base, et ce prix ne doit plus
// rien décider nulle part. Le forfait par personne viendra avec le lot 4, sur
// ses propres réglages, jamais sur ce prix-là.
export function sansPrixSiTable(prestation) {
  if (!estParCouverts(prestation)) return prestation
  return { ...prestation, prix: null, acompte_pourcent: 0 }
}
...
// 🔴 CE QUE LA RÈGLE DES COUVERTS LIT, DÉCLARÉ PAR LA RÈGLE ELLE-MÊME
// (16/09, essai E2 d'Alex : « Ce nombre de personnes n'est pas accepté »).
//
// `capacite` manquait dans DEUX selects de prestation. Sans elle,
// `capacitePrestation` retombe sur 1, `bornesCouverts` rend { min: 1, max: 1 },
// et toute table de deux personnes ou plus devient invalide :
//   • sur la route de l'empreinte, le client était BLOQUÉ NET devant sa carte ;
//   • sur la route gratuite, `couvertsValides` rendait `null`, ce qui DÉSARMAIT
//     en silence la garde qui devait justement exiger la carte. Une table de six
//     s'y réservait donc sans rien demander : le défaut du 15/09, revenu par une
//     autre porte, avec un banc vert parce qu'il ne regardait que les colonnes
//     du COMMERÇANT.
//
// ⚠️ UNE COLONNE ABSENTE D'UN SELECT, SEPTIÈME FOIS DANS CE PROJET. Le remède
// n'est pas de l'ajouter à deux endroits : c'est que la règle DISE ce qu'elle
// lit, et qu'un banc exige cette liste de chaque appelant.
export const COLONNES_COUVERTS = 'capacite, par_couverts, couverts_min, couverts_max'
...
// Reste-t-il une place à ce cours, ce jour-là, à cette heure-là, d'après les
// inscriptions connues de l'écran ?
//
// 🔴 LES HEURES « LIBRES » PROPOSAIENT UN COURS COMPLET (10/09 tard). Elles ne
// regardaient que les horaires : la pastille de 10h s'affichait sur un cours de
// douze déjà plein, et le refus n'arrivait qu'au clic.
//
// ⚠️ L'ÉCRAN PROPOSE, LA BASE TRANCHE : la place se relit en base au moment
// d'écrire, avec `premierePlaceLibre`, la même fonction qu'ici. Ceci empêche
// seulement de proposer d'un tap ce qui sera refusé. `exclureId` : un inscrit
// qu'on déplace à l'intérieur de son propre cours ne se prend pas sa place.
export function coursAPlace(prestation, reservations = [], { dateStr, heure, exclureId = null } = {}) {
  const h = String(heure || '').slice(0, 5)
  const prises = (reservations || [])
    .filter(r => r
      && String(r.prestation_id) === String(prestation?.id)
      && r.date_rdv === dateStr
      && String(r.heure_debut || '').slice(0, 5) === h
      && ['confirme', 'honore'].includes(r.statut)
      && (exclureId == null || String(r.id) !== String(exclureId)))
    .map(r => r.place_no)
  return premierePlaceLibre(prestation, prises) !== null
}
