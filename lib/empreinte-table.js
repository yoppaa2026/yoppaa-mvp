...
// « CE COMPTE ENCAISSE-T-IL ? », ÉCRIT UNE SEULE FOIS.
//
// 🔴 L'ÉCRAN DE RÉGLAGE LE DEMANDAIT AUTREMENT QUE LA RÈGLE (16/09). Il lisait
// « un identifiant Stripe est posé ET l'encaissement n'est pas FAUX ». Un
// restaurateur en cours d'inscription chez Stripe (identifiant créé, formulaires
// pas terminés, donc `charges_enabled` à `null`) ne voyait AUCUN avertissement :
// il cochait « demander une empreinte », lisait « Empreinte enregistrée : dès
// 6 personnes, 20 € par personne », et plus bas la règle ne demandait jamais la
// moindre carte. Il l'aurait appris le soir du premier no-show.
//
// ⚠️ `=== true`, ET RIEN D'AUTRE. `null` (inscription commencée) et une colonne
// absente d'un select sont des ABSENCES, pas des comptes en ordre : les deux
// formes de l'absence, encore elles.
export function compteEncaisse(commercant) {
  return commercant?.stripe_account_charges_enabled === true
}
...
export function empreinteRequise(commercant, prestation, couverts) {
  ...
  // ⚠️ SANS COMPTE STRIPE QUI ENCAISSE, PAS D'EMPREINTE. Le SetupIntent naît sur
  // le compte du restaurateur : sans lui, la demande échouerait au moment précis
  // où le client donne sa carte, c'est-à-dire au pire moment possible.
  //
  // 🔴 LA RÈGLE NE LIT QUE CE QUE LA FICHE PUBLIQUE CONNAÎT (15/09, essai E2
  // d'Alex : une table de 6 réservée sans que rien ne soit demandé). Elle
  // testait aussi `stripe_account_id`, que `commercants_public` n'expose pas, et
  // à raison. Sur la fiche, elle rendait donc TOUJOURS faux : aucune carte n'a
  // jamais été demandée en ligne, et le banc restait vert parce que son
  // restaurant d'essai portait la colonne. Les routes qui ouvrent Stripe
  // vérifient l'identifiant elles-mêmes, puisqu'elles en ont besoin.
  //
  // ⚠️ `!== true` ET NON `=== false` : une colonne absente d'un select ne vaut
  // pas un compte en ordre. La question a désormais UNE définition, partagée
  // avec l'écran de réglage, qui la posait plus largement (voir `compteEncaisse`).
  if (!compteEncaisse(commercant)) return false
...
// CE QUE L'AGENDA DIT D'UN LIEN DÉJÀ PARTI.
//
// 🔴 « LIEN DÉJÀ ENVOYÉ » NE DISAIT PAS COMBIEN DE FOIS (demande d'Alex,
// 16/09). Le restaurateur ne savait donc pas s'il relançait pour la première ou
// la quatrième fois, et un client relancé quatre fois sans réponse n'appelle pas
// la même décision qu'un client prévenu une seule fois : au bout d'un moment,
// on décroche son téléphone au lieu de renvoyer un lien.
//
// ⚠️ LE COMPTEUR COMPTE LES ENVOIS RÉUSSIS, jamais les tentatives : un SMS qui
// n'est pas parti n'est pas une relance, et le compter ferait croire le client
// prévenu. C'est exactement le défaut corrigé le même jour sur la trace d'envoi.
//
// ⚠️ ET ON N'ÉCRIT PAS « 1 fois » : la première fois se dit sans chiffre, comme
// on parle.
export function libelleRelance({ envois = 0, canal = null } = {}) {
  const parQuoi = canal === 'sms' ? 'SMS' : 'email'
  const n = Math.floor(Number(envois))
  if (!Number.isFinite(n) || n <= 1) return `Lien envoyé par ${parQuoi}, pas encore confirmé.`
  return `Lien envoyé ${n} fois, le dernier par ${parQuoi}, pas encore confirmé.`
}
...
// 🔴 CE QUE L'ÉCRAN DOIT SAVOIR AVANT D'OFFRIR LE BOUTON (16/09, Alex : « SMS
// et email ne partent plus »). `raisonDemandeImpossible` ne connaît que l'heure
// du SERVICE ; le lien, lui, meurt plus tôt, au moment où la table n'est plus
// annulable sans frais. Entre les deux, l'écran proposait un bouton que le
// serveur refusait à tous les coups : sur une table de 19 h avec un délai de
// 3 h, c'est une fenêtre de trois heures où rien ne peut partir.
//
// ⚠️ UNE SEULE DÉFINITION, LUE PAR L'ÉCRAN ET PAR LA ROUTE. C'est la règle du
// jour : le serveur décide, et l'écran pose exactement la même question.
export function raisonLienImpossible(rdv, commercant, maintenant = new Date()) {
  const raison = raisonDemandeImpossible(rdv, maintenant)
  if (raison) return raison
  // ⚠️ `echeanceLien` s'arrête au plus tôt de sept jours et de la limite
  // d'annulation. Au-delà, poser le lien serait un piège, pas un service.
  return echeanceLien(rdv, commercant, maintenant) <= maintenant ? 'delai_passe' : null
}

// Ce que le restaurateur lit à la place des boutons, quand le lien ne peut plus
// servir. ⚠️ ON NE CACHE PAS SANS DIRE : un bouton qui disparaît sans raison
// laisse croire à une panne, et le restaurateur recharge sa page pour rien.
export function messageLienImpossible(raison) {
  return {
    delai_passe: 'Trop tard pour garantir cette table : le délai d’annulation est passé. Un lien envoyé maintenant piégerait ton client.',
    ...
    pas_confirmee: 'Cette réservation n’est plus confirmée.',
...
// 🔴 ON NE FACTURE QU'UNE ABSENCE (15/09, trouvé en écrivant la liste des
// essais). La règle vérifiait l'empreinte et la fenêtre, JAMAIS LE STATUT : une
// table marquée « Honorée », ou annulée à temps par le client, restait
// facturable jusqu'au lendemain soir. Il suffisait d'un mauvais clic pour
// débiter quelqu'un qui avait dîné.
//
// Deux cas, et deux seulement :
//   • `no_show` : le restaurateur a DÉCLARÉ l'absence avant de facturer ;
//   • `annule_client` + `annulation_tardive` : le client a prévenu trop tard
//     pour que la table soit reprise.
// ⚠️ `confirme` N'EN FAIT PAS PARTIE, et c'est délibéré : une table qu'on n'a
// pas encore pointée peut très bien être venue. Déclarer l'absence d'abord,
// facturer ensuite, c'est deux gestes et c'est voulu.
export function estAbsenceFacturable(rdv) {
  return rdv?.statut === 'no_show'
    || (rdv?.statut === 'annule_client' && rdv?.annulation_tardive === true)
}

// Ce qu'une annulation annonce au client comme encore facturable.
// 🔴 SEULEMENT une annulation TARDIVE d'une table GARANTIE. Une table annulée à
// temps ne coûte rien, une table tardive sans carte non plus : leur annoncer un
// montant ferait peur pour rien, et ferait croire à un débit qui n'aura jamais
// lieu. Sorti de la route le 15/09 pour être EXÉCUTÉ au banc : la ligne qu'une
// garde cherchait dans la route avait un jumeau dans la condition voisine, et
// restait verte quand on la cassait.
export function montantAnnulationFacturable(rdv, tardive) {
  if (!tardive || rdv?.empreinte_statut !== 'posee') return 0
  return Math.round((Number(rdv?.empreinte_montant) || 0) * 100) / 100
}

export function raisonDebitImpossible(rdv, maintenant = new Date()) {
  // 🔴 « DÉJÀ FACTURÉE » SE DIT AVANT « PAS D'EMPREINTE » (Alex, 16/09, G6).
  // Une table facturée passe en `debitee`, donc son statut n'est plus `posee` :
  // le premier test l'attrapait et répondait « cette réservation n'a pas
  // d'empreinte bancaire », juste sous une carte qui affiche « Garantie :
  // 120,00 € ». Le restaurateur lisait deux choses contraires sur le même
  // écran, et pouvait croire sa garantie perdue.
  if (rdv?.empreinte_statut === 'debitee' || rdv?.empreinte_debit_pi_id) return 'deja_debitee'
  ...
  // ⚠️ EN DERNIER, pour que l'écran puisse distinguer « le service n'a pas
  // commencé » et « trop tard » de « elle n'est pas déclarée absente ».
  if (!estAbsenceFacturable(rdv)) return 'pas_absente'
  return null
}
