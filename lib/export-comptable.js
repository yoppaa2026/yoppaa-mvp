...
//
// `venteHorsPeriode` contient les identifiants des ventes chargées UNIQUEMENT
// parce qu'elles ont été remboursées pendant la période : leur vente appartient
// à un export précédent, seul leur remboursement sort ici.
export function construireLignes({
  commandes = [], rdvs = [], abonnements = [], tauxDefaut = null, articlesParId = {},
  retoursBons = [], periode = null, venteHorsPeriode = null,
}) {
  const lignes = []
  const horsPeriode = venteHorsPeriode instanceof Set
    ? venteHorsPeriode
    : new Set(venteHorsPeriode || [])
  ...
  const poser = (ligne, { id = null, remboursements = [] } = {}) => {
    // La vente elle-même ne sort que si elle appartient à cette période. Son
    // remboursement, lui, est jugé sur SA propre date, juste en dessous : c'est
    // tout l'intérêt de charger ces ventes-là.
    if (!horsPeriode.has(id)) lignes.push(ligne)
  ...
  for (const c of commandes) {
    ...
    poser({
      ...
      referenceIncomplete: !/-S\d{2}$/.test(referenceQualifiee(referenceCommande(c), c.numero_semaine) || ''),
