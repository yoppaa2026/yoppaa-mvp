...
// 🔴 UN TAUX SE LIT « 1,5 % », JAMAIS « 1.5% » (trouvé par Alex le 17/09, sur une
// capture destinée aux stores). La carte de fidélité annonçait « Gagne 1.5% de
// chaque achat » : le point venait de JavaScript, qui écrit ses nombres à
// l'anglaise, et l'espace manquait devant le symbole.
//
// ⚠️ ON NE FORCE PAS LES DÉCIMALES, et c'est la différence avec `euros()`. Un
// taux rond s'écrit « 10 % » ; « 10,00 % » donnerait une fausse impression de
// précision sur un chiffre que le commerçant a choisi à la louche.
//
// ⚠️ L'ESPACE EST INSÉCABLE, pour la raison exacte du montant : un « % » qui
// tombe seul en début de ligne ne veut plus rien dire.
//
// ⚠️ ET ELLE EST NOMMÉE, PAS TAPÉE. Une espace insécable ressemble trait pour
// trait à une espace ordinaire : on ne la voit pas à la relecture, un
// copier-coller la remplace en silence, et l éditeur qui la convertit ne
// prévient personne. Nommée, elle se lit, se cherche, et se mesure.
// ⚠️ EXPORTÉE, parce que d'autres écrans en ont besoin sans passer par
// `euros()` : un bouton de montant rond affiche « 20 € » et non « 20,00 € »,
// deux décimales inutiles sur une pastille de trois centimètres.
export const INSECABLE = String.fromCharCode(160)

export function pourcent(valeur) {
  const n = Number(valeur)
  const arrondi = Math.round((Number.isFinite(n) ? n : 0) * 100) / 100
  return `${String(arrondi).replace('.', ',')}${INSECABLE}%`
}
