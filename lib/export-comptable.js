...
export function construireLignes({ commandes = [], rdvs = [], abonnements = [], tauxDefaut = null, articlesParId = {} }) {
  ...
  for (const c of commandes) {
    ...
    lignes.push({
      ...
      total: totalNet,
      parTaux,
      enLigne: c.paye_en_ligne ? arrondi(totalNet - parBon) : 0,
      comptoir: encaissementDeclare ? aRegler : 0,
      resteAEncaisser: encaissementDeclare ? 0 : aRegler,
      bonCadeau: parBon,
      fraisStripe: montantStripe(c.paye_en_ligne ? totalNet - parBon : 0, c.stripe_frais),
      netStripe: montantStripe(c.paye_en_ligne ? totalNet - parBon : 0, c.stripe_net),
      statut: c.statut,
    })
  }
...
function entete({ commercant, du, au, assujetti, avertissementTaux, tauxManquants, refsIncompletes = 0 }) {
  const l = [
